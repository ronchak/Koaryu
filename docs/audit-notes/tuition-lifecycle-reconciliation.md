...
| Refund | `billing_refunds` plus cumulative successful amount on payment | Stripe Refund status | Hidden Admin route and signed Connect events | Upsert by provider refund ID; terminal-status regression guard; recompute from all succeeded refund rows | No live refund control; projected status is visible indirectly through payment/invoice totals |
