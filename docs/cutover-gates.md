...
The completed V50 release is recorded in [PRODUCTION-RELEASE.md](remediation/PRODUCTION-RELEASE.md) and [verification](remediation/production-release-verification.md). Production and staging are V50, 145 migrations; both application pairs now serve Google branding candidate `f1320ee179f28df54b85616ae7752a14823b4db7`, with no migration added. See [Google SSO verification](google-sso-setup.md#verification-status). The staging billing cron remains suspended. Future releases need a new exact-candidate packet and fresh target evidence. Do not reuse completed inspection tokens or recovery evidence as approval of a new state.
