...
| Supabase Auth/backup control inventory | `npm run check:supabase-controls` and `git diff --check` | Run `npm run check:supabase-controls:live` only with a current private Management API token; run `npm run verify:supabase-auth-revocation` only against the pinned staging project with the explicit disposable acknowledgement |
