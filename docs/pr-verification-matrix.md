...
| Production-data classification policy, schemas, classifier, or runbook | `npm run audit:production-classification` and `git diff --check` | Verify a real private manifest only after the exact read-only extraction package, snapshot binding, target, and private output location receive explicit approval |
