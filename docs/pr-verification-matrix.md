...
| Stripe billing or webhooks | `npm run verify:tuition-lifecycle` for the fake-only lifecycle, role, ordering, retry, and UI proof | Run `npm run dev:stripe-connect-smoke -- --confirm-stateful-target --account acct_...` only against a confirmed disposable/local target |
