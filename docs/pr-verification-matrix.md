...
| Dependency manifests or lockfiles | Reinstall from the lock/requirements files and run the affected package tests | Frontend: `npm audit --omit=dev` and `npm run build`; backend: `backend/venv/bin/python -m pip check` and the broader pytest suite for shared dependency changes |
