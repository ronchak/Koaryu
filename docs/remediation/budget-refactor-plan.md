...
2. **Remove shared back-reference routes.** PR184 completed the next cut, removing 35 of the 68 remaining facade methods and four unused enrollment forwarders. Six facade aliases were test-only, 27 lacked BillingService receiver callers, and two more belonged solely to a dead enrollment forwarding chain. Existing implementations stay with their actual managers/projectors. PR185 made payer operations concrete and relocated their four existing fact/balance functions, eliminating manager construction for those facts and the reverse service import. PR186 gave the three provider projectors concrete dependencies and removed their service back-references. PR187 made Connect/autopay commands concrete, reused existing redirect rules and consolidated the ordinary billing audit insert. PR188 consolidated invoice commands into one concrete owner and deleted the separate workflow. PR189 made payment/reconciliation concrete and removed their projection forwarding routes. The final enrollment change removes that remaining family's opaque owners, deletes the facade and removes the payment/landing import cycle. Follow actual dependency direction through account/payer facts and provider projections. Move existing rules to their real owners or call existing pure functions directly. Remove forwarding methods at their callers. Do not create a replacement common facade or new behavior. Projection must not need to call command managers through BillingService.
...
## Reassessment after ownership removal

BB1-09 is fixed by the final concrete enrollment/record implementation. This closes one architectural finding, not the broader audit. The original counts become 79 fixed, 2 resolved indirectly, 184 pending, 11 intentionally deferred, 1 owner action and 1 rejected. Program findings remain separate: three fixed and four pending, including the newly verified refund completion fingerprint risk. No database fix is authorized in this run.

Next, return to the six selected user-visible defects, followed by the operator/runbook corrections including DOC1-02, then remaining delegated work by value. Recheck each recipe against current main and the remaining weekly budget; do not treat this sequence as a quota. Draft PR180 and blocked database prerequisites stay parked until their applicable scope is selected. Preserve the explicit 45/50-point stopping gates.
