...
The September 14 run completed the refund correction in PR207/PR209. PR208 contains the owner-authorization policy and tool change. The owner approved the one-migration extension on September14. Its implementation and verification are recorded in [the plan](one-migration-plan.md); hosted execution still requires its reviewed merge and the phase-three safety evidence. No hosted migration, backup, backend deployment, frontend promotion or live billing activation has run.

## Main and budget

Main at this checkpoint is `ea41d8604e255de9869b05c5e84e4363d2d432b9`, the PR208 merge. Its exact-head Release candidate run `34822452411` passed. Later documentation or governance merges will have their own SHA and CI; use Git and the closing task report for the final head.

The run began at 51% weekly usage, with authority to spend at most 35 percentage points. Stop new work and release execution at 81% used, reserving five points for verification/recovery; hard ceiling 86%. The continuation read at 2026-09-14 11:16 UTC was 65% used, or 14 points spent. This is a preparation reading; the closing report records the final meter. The execution block, not budget exhaustion, prevents later phases. The window resets September 19 at 10:16:57 UTC. Do not reuse the prior run's 45% baseline or 55% ceiling.
...
The owner approved `--one-migration` on September14. The implementation selects the first remaining file, verifies both the full and singleton CLI dry-runs, restores its temporary file scope, and requires the exact declared successor. The default production bulk refusal remains. No new migration or schema change is part of this extension. See [verification](one-migration-verification.md). After its fresh review, CI and merge, begin hosted staging rehearsal from a newly pinned candidate; the original 35-point budget does not reset on this continuation.

The next work is the hosted staging rehearsal, fresh production backup and verified disposable restore, plus a written recovery action for every forward-only migration. Only after all four exist may production proceed database first, backend second, frontend last. The owner explicitly allows the recorded old-frontend/new-backend pair only during that planned transition; every unexpected SHA mismatch still halts.
