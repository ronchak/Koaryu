...
- These two findings alone move to fixed, with derived ledger counts now 142 fixed and 120 pending. They are merged into main, **not deployed**. No production or staging deployment, database apply, migration, backup/restore, billing activation, or hosted fixture mutation occurred. GitHub/Vercel PR checks created ordinary preview builds. Production auto-deploy remains off. Release needs separate owner authorization after the postmortem.
...
- [PR243](https://github.com/ronchak/Koaryu/pull/243), FSH2-07 + FC1-03, remains open and unmerged. Coordinator initial review missed duplicate post-create materialization, which GitHub review caught after green CI. A correction brief is retained but was not dispatched after the 98% cutoff. Both ledger findings remain pending.
