...
Staging is exact V47, 142 migrations, head `20260914055301`. Its web service and billing cron remain suspended. No application deployment or staging branch move occurred. Restoring staging service is a separate announced release action after the failed gate is resolved; deploy the reviewed candidate and keep the cron paused. Complete all 53 hosted contracts and authenticated application rehearsal before production. The stored staging-owner password still needs its correct account email from the owner.
