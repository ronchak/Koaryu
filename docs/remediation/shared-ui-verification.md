...
The coordinator caught and corrected changed click cancellation, lost React 19 ref cleanup and ignored wrapper tab order before independent review. Role authorization, sign-out, report permissions, schedule behavior, legal content and routing remain unchanged.

Verification: 40 focused tests pass, plus TypeScript, narrow lint and a production build with 62 static pages. Direct built-output checks cover legal paragraphs, notices, anchors, labels and six static routes' metadata. The new mounted cases exercise actual Button composition and theme media changes with blocked storage. The earlier full frontend run passed 917 cases before final test consolidation; exact-head CI verifies the final tree.

Associated tests, including the new mounted file, shrink from 1,672 lines and 41 cases to 1,648 lines and 40 cases. Removed checks duplicated legal text, matched exact renderer/CSS/metadata spelling or prohibited already-retired source names. Existing financial, authorization and schedule assertions remain. The small isolated UI loader stays local because existing loaders bind unrelated store or billing dependencies; wider fixture consolidation remains FT1-07 work.
