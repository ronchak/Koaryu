...
[Recipe14](14-dead-ui-interface-closeout.md) is the completed six-finding subset of batch08, merged in PR180. It is not another independent audit batch. The original cohort is reproducible from ledger revision `30891b4d0796172ad93f20cd29ff47c90375899e`: select entries with `disposition=pending`, `track=Sol` and a nonempty `delegated_batch`. Of those 55 findings, 49 are fixed, one is obsolete and five remain pending. [Recipe15](15-staff-export-auth-hydration.md) completed OPS1-09 and BT5-05 together in PR205; OPS1-09 was outside that original 55. The ledger also contains broader application findings outside this original batch index.
