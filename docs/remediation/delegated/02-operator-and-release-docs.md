...
Prerequisites: the attestation generator is the next Astra implementation. Hold `DOC1-03` and copied active release-version edits that depend on it until that lands. Independent historical-label, token-placeholder, report-path, and sequence cleanup may proceed. Read the private release and authentication runbooks required by root `AGENTS.md`, but never copy credential values, private evidence or dumps into the repository.
