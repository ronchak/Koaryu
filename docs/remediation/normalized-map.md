...
| --- | --- | --- |
| Release attestation authority | PROGRAM-ATTESTATION-01, OS2-01/02, OS3-01, DM4-08; BT4-08 is historical supporting evidence already fixed | Merged in PR167, with Node compatibility corrected in PR168. V42 uses the generated release bodies and restore checks from declared state. Preserve historical bytes and distinct canonical/restored evidence. Update active version instructions DOC1-03 afterward. |
| Independent student program dates | DM2-02 | PR170 contains the verified forward SQL correction plus preview parity. Preserve paused status, rank and explicit program editing. Update tests that deliberately preserved the now-rejected date propagation. |
...
The generator dependency is satisfied by PR167. V42 provides the first real forward authoring use: declared state emits the preflight, manifest and restore checker while historical reproduction remains byte-identical. Canonical/restored expectations and business preservation still require independent proof.
