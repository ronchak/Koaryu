...
The repository contains durable, counts-only episodes, a receipt-bearing HTTPS primary/backup transport, acknowledgement/escalation/resolution lifecycle, a five-minute evaluator cadence, and exact-allowlisted evaluator/deletion-worker dead-man check-ins. The primary trigger is an external scheduler on the director-operated home server; the committed Vercel cron is a daily 09:00 UTC backup. This changes the trigger source, not the required five-minute cadence. Production remains `OPERATIONAL_ALERTS_ENABLED=false`; no real destination, owner, token, or dead-man value is committed. The evaluator endpoint returns `204` and performs no backend or provider call while disabled.
...
The provider-independent external trigger contract is: call `https://<production-frontend-origin>/api/cron/operational-alerts/evaluate` every five minutes with method `GET`, header `Authorization: Bearer $CRON_SECRET`, and a 35-second request timeout. Expect `204` while disabled and `200` when enabled. No retry may start concurrently with an in-flight evaluation: retries must wait for the preceding request to finish or time out, and scheduler executions must be serialized. The external scheduler's restricted secret store is an additional custody location for `CRON_SECRET` and must be included in every `CRON_SECRET` rotation.
...
3. Configure the director-operated home server's external scheduler to invoke the provider-independent trigger contract every five minutes, install only `CRON_SECRET` in the scheduler's restricted secret store while retaining the existing worker configuration on the frontend, verify the daily 09:00 UTC Vercel backup, and keep the switch false until rehearsal.
