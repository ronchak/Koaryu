...
Status: **staging at exact migration 108; migration 109 rehearsal pending; production human apply locked**

This packet reconciles the production and staging 100-migration V7 baseline
with the immutable 109-migration release candidate. It is specialized to this rollout, not a
...
- the release authority is the operator-side raw-catalog verifier plus its
  repository-pinned SHA-256 manifests; the database V16 readiness signal, backed
...
`20260814183000`, `20260814200000`, and `20260814213000`.
The only certifiable post-state is migration count 109 at the latter head. Its
V16 readiness signal retains the complete migration-85-through-109 sequence:
...
`integration_complete=true` only when those nine files are the release
pending migrations and the 109-state readiness contract contains the twenty-five
...
migrations 101 through 109; staging, already at exact migration 108,
must dry-run and apply only migration 109.
...
critical state 107, or column-attested state 108 may enter apply after a
matching inspection token and dry-run; post-state 109 is inspect-only and has
...
| 105 | Adds the V12 return-contract manifest and replaces readiness. No table data or trigger changes. | Read-only attestation change; application-compatible. | One migration transaction. Exact V12 state is guarded and may resume with migrations 106-109 after fresh inspection and dry-run. |
...
213000 halts before credentials are used.
...
`column-attested`, or `post`), its corresponding readiness
result, the complete
...
`20260814213000_lock_core_trial_decision_to_reservation.sql` with its final candidate hash.
...
1. require count 109, head 213000, the exact twenty-five-version sequence, and the
...
database-first: Render `/health/ready` remains 503 until the exact 109 head and
