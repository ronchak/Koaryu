...
Status: **staging at exact migration 108; migrations 109-110 rehearsal pending; production human apply locked**

This packet reconciles the production and staging 100-migration V7 baseline
with the immutable 110-migration release candidate. It is specialized to this rollout, not a
...
- the release authority is the operator-side raw-catalog verifier plus its
  repository-pinned SHA-256 manifests; the database V17 readiness signal, backed
  ...
  the V15 checkout-binding/promotion-column manifest, and the V16 critical-surface
  manifest, exposed through the candidate V3 RPC is an
...
`20260814183000`, `20260814200000`, `20260814213000`, and `20260815220402`.
The only certifiable post-state is migration count 110 at the latter head. Its
V17 readiness signal retains the complete migration-85-through-110 sequence:
...
`integration_complete=true` only when those ten files are the release
pending migrations and the 110-state readiness contract contains the twenty-six
...
exact V17 after the reviewed staff identity work. Never certify an earlier head;
...
migrations 101 through 110; staging, already at exact migration 108,
must dry-run and apply only migrations 109 and 110.
...
enter apply after a matching inspection token and dry-run; final post-state 110
...
`20260814213000_lock_core_trial_decision_to_reservation.sql` and
`20260815220402_staff_identity_name_model.sql` with their final candidate hashes.
...
3. invoke the service-role-only V3 readiness RPC during every apparent-post
   ...
   `release-db-attestation-v17`; a missing, malformed, stale, or failing result
   ...
   report ready only when the V3 check proves exact V17;
...
database-first: Render `/health/ready` remains 503 until the exact 110 head and
