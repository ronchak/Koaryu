...
The current `koaryu-staging` project is isolated: the repository migrations were replayed, the production backup was **not** restored into it, and it contains only the synthetic `River City Martial Arts` fixture. Verified on 2026-08-08 with `node scripts/studio-comp-migration-rollout.mjs --mode diagnose --target staging`, staging holds exactly **84** migrations with newest version `20260713173000`, matching production. Its `supabase_migrations.schema_migrations` additionally carries the `created_by`, `idempotency_key`, and `rollback` bookkeeping columns added by Supabase CLI releases after the pinned `2.95.4`; these carry no migration identity and are tolerated by name in the history-shape check. The dedicated staging backend API is `https://koaryu-staging.onrender.com/api/v1`, and the protected durable staging frontend alias is `https://koaryu-git-staging-ronakchak2569-8303s-projects.vercel.app`. The temporary hosted restore target used for the July 10 drill was `zmmacdleiaohvxdubrav`; it was deleted after validation.
...
The record-classification manifest is a separate inventory artifact, not an output of `supabase db dump`. Follow the [production data classification runbook](audit-notes/production-data-classification.md): classify the disposable restore of the exact verified dump using only aggregate evidence and classification-specific HMAC references, then encrypt the private input and manifest with the same GPG AEAD controls. Raw identifiers, names, addresses, and unkeyed email hashes are prohibited. Record only aggregate counts, policy/classifier digests, ciphertext hashes, and pass/fail results in the ledger. No classification approves deletion or any other production mutation.
