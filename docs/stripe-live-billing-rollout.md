...
For a mid-flight readiness or authorization flip, never issue a second mutation. Preserve the deterministic idempotency key, read the provider object and matching event ID, allow webhook reconciliation to converge local state, and stop for operator review if the outcome stays unknown. An uncertain initial Account Link call may retry only under its still-live bootstrap with the same stored idempotency key and exact context; it does not receive a new permit. Branding upload has no automatic retry.

The first live Connect onboarding request uses a five-minute database bootstrap bound to one studio, one account generation, the exact deployed SHA/readiness checkpoint, exact account-create and initial-link payload digests, and stored idempotency keys. Its only evidence exception is the newly created account generation's initial hosted Account Link. Every pre-existing mapping and every later Account Link remains checkpoint-bound. A claimed request may repeat only with the same bootstrap token, account, generation, payload, candidate, and stored idempotency key; this reuses the original permit and never authorizes a second mutation. Expiry fails closed permanently for that studio generation: an uncertain account-create must be reconciled before an explicitly reviewed generation rollover, not replaced with a new bootstrap permit.
