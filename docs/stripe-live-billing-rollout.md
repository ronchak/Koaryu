...
For a mid-flight readiness or authorization flip, never issue a second mutation. Preserve the deterministic idempotency key, read the provider object and matching event ID, allow webhook reconciliation to converge local state, and stop for operator review if the outcome stays unknown. Account Link creation and branding upload have no automatic retry; a timeout is a stop/readback/restart boundary, not permission to duplicate the request.
