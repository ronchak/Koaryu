...
- The local verifier replays all 140 migrations and 52 verification files. It checks the V38-to-V39 membership, V39-to-V40 rank-history, V40-to-V41 payer-balance, V41-to-V42 independent program-date, V42-to-V43 external-payment, V43-to-V44 local-plan, and V44-to-V45 import-ownership backup/restore upgrades with synthetic rows and retained backend contracts. The V43-to-V44 proof compares canonical and logically restored evidence and checks atomic plan, program-link, audit, and committed-snapshot continuation without a backfill. The former payer-only runner is now `scripts/verify-billing-command-concurrency.py`; it checks payer, external-payment, and local-plan concurrency in 15 cases using the existing process helpers. The former rank runner is now `scripts/verify-student-write-concurrency.py`; it retains five rank cases and adds eight import/clear/Auth/progress cases. V45 restore reuses the maintained import contracts from frozen bytes and records their hashes. Incomplete legacy imports fail closed; completed caches remain readable. The full suite includes the new import receipt/marker/Auth-helper negative checks. Restore and concurrency tools share strict PostgreSQL 17 target validation and accept only the verifier's disposable Unix-socket cluster. Never pass them a hosted target or a production dump.
