...
- The local verifier replays all 138 migrations and 51 verification files. It checks actual V38-to-V39 membership, V39-to-V40 rank-history, V40-to-V41 payer-balance, V41-to-V42 independent program-date, and V42-to-V43 external-payment backup/restore upgrades with synthetic rows and retained backend contracts. The V42-to-V43 proof compares canonical and logically restored evidence and checks atomic payment/audit continuation, replay, and balance completion without a backfill. The restore, rank-concurrency, and payer-concurrency tools share strict PostgreSQL 17 target validation and accept only the verifier's disposable Unix-socket cluster. Never pass them a hosted target or a production dump.
