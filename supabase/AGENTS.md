...
- The local verifier replays all 136 migrations and 51 verification files. It checks actual V38-to-V39 membership, V39-to-V40 rank-history, and V40-to-V41 payer-balance backup/restore upgrades with synthetic rows and retained backend contracts. The restore, rank-concurrency, and payer-concurrency tools share strict PostgreSQL 17 target validation and accept only the verifier's disposable Unix-socket cluster. Never pass them a hosted target or a production dump.
