...
BEGIN
    ...
    IF v_v6.ready IS TRUE
       ...
       OR v_v2.ready IS TRUE THEN
        RAISE EXCEPTION 'Readiness accepted substituted migration history: v6=%, v5=%, v4=%, v2=%',
            row_to_json(v_v6), row_to_json(v_v5), row_to_json(v_v4), row_to_json(v_v2);
    END IF;
...
BEGIN
    ...
    SELECT * INTO v_v5 FROM public.koaryu_release_schema_preflight_v5();
    SELECT * INTO v_v4 FROM public.koaryu_release_schema_preflight_v4();
    IF v_v6.ready IS DISTINCT FROM true
       OR v_v6.migration_count <> 119
       OR v_v6.migration_head <> '20260826030249'
       OR v_v6.security_failures IS DISTINCT FROM ARRAY[]::TEXT[]
       OR v_v6.manifest_version <> 'release-db-attestation-v26'
       OR v_v5.ready IS DISTINCT FROM true
       OR v_v5.migration_count <> 118
       OR v_v5.migration_head <> '20260826030234'
       ...
       OR v_v4.manifest_version <> 'release-db-attestation-v24' THEN
        RAISE EXCEPTION 'V26 readiness or compatibility drifted: v6=%, v5=%, v4=%',
            row_to_json(v_v6), row_to_json(v_v5), row_to_json(v_v4);
    END IF;
END $$;
...
BEGIN
    ...
    SELECT * INTO v_v5 FROM public.koaryu_release_schema_preflight_v5();
    SELECT * INTO v_v4 FROM public.koaryu_release_schema_preflight_v4();
    IF v_v6.ready IS DISTINCT FROM false
       OR NOT (ARRAY[
       ...
       OR NOT (ARRAY[
        ...
        RAISE EXCEPTION 'V27 compatibility accepted a tampered predecessor operational contract: v6=%, v5=%, v4=%',
            row_to_json(v_v6), row_to_json(v_v5), row_to_json(v_v4);
    END IF;
END $$;
