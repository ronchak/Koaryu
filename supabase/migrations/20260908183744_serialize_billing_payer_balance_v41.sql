-- Serialize payer balance recomputation without changing invoice or billing policy.
-- All earlier migrations remain immutable. This migration performs no data repair.
DO $predecessor$
DECLARE v RECORD;
BEGIN
    IF encode(extensions.digest(convert_to(pg_catalog.pg_get_functiondef(
        pg_catalog.to_regprocedure('public.koaryu_release_schema_preflight_v21()')),'UTF8'),'sha256'),'hex')
        IS DISTINCT FROM '9821befb5ff670cece891b73e2271f31384a5e7dc35bb87e2055b5ad2aa45987' THEN
        RAISE EXCEPTION 'V41 requires the reviewed full V40 readiness definition.';
    END IF;
    SELECT * INTO v FROM public.koaryu_release_schema_preflight_v21();
    IF v.ready IS DISTINCT FROM TRUE OR v.migration_count IS DISTINCT FROM 135
       OR v.migration_head IS DISTINCT FROM '20260908133504'
       OR v.manifest_version IS DISTINCT FROM 'release-db-attestation-v40'
       OR cardinality(v.security_failures) IS DISTINCT FROM 0
       OR cardinality(v.pending_versions) IS DISTINCT FROM 51
       OR v.pending_versions[51] IS DISTINCT FROM '20260908133504' THEN
        RAISE EXCEPTION 'V41 requires a fully verified V40 predecessor.';
    END IF;
END;
$predecessor$;

CREATE FUNCTION public.recompute_billing_payer_balance_v1(
    p_studio_id UUID,
    p_payer_id UUID
)
RETURNS VOID
...
BEGIN
    ...
    -- Lock before taking the invoice snapshot. Other recomputations must wait
    -- through both the calculation and its update of this payer.
    ...
    -- This separate READ COMMITTED query sees changes committed during a lock
    -- wait. It takes no invoice row locks, preserving invoice-to-payer ordering.
    SELECT COALESCE(SUM(GREATEST(0::BIGINT,
        COALESCE(invoice.amount_remaining_cents::BIGINT,
            GREATEST(0::BIGINT,
                COALESCE(invoice.amount_due_cents::BIGINT, 0)
                - COALESCE(invoice.amount_paid_cents::BIGINT, 0)
            )
        )
    )), 0)
    INTO v_balance
    ...
    WHERE invoice.studio_id = p_studio_id
      AND invoice.payer_id = p_payer_id
      AND invoice.status IN ('draft', 'open', 'uncollectible', 'partially_refunded');

    UPDATE public.billing_payers
    SET balance_cents = v_balance,
        billing_status = CASE WHEN v_balance = 0 THEN 'current' ELSE 'past_due' END
    WHERE id = p_payer_id AND studio_id = p_studio_id;
END;
$function$;

ALTER FUNCTION public.recompute_billing_payer_balance_v1(UUID, UUID) OWNER TO postgres;
REVOKE ALL ON FUNCTION public.recompute_billing_payer_balance_v1(UUID, UUID)
    FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.recompute_billing_payer_balance_v1(UUID, UUID)
    TO service_role;

CREATE FUNCTION public.koaryu_release_schema_preflight_v22()
...
BEGIN
    ...
    IF v_count <> 136 OR v_head <> '20260908183744' THEN
        v_failures := array_append(v_failures, 'migration_history_v41');
    END IF;
    IF COALESCE(v_pending, ARRAY[]::TEXT[]) IS DISTINCT FROM ARRAY[
        ...
        '20260826073728','20260826102840','20260826155911','20260826185651','20260830065627','20260830082610','20260830151714','20260831022021','20260831054918','20260902001000','20260905022339','20260908080420','20260908133504','20260908183744'
    ]::TEXT[] THEN
 ...
 RETURN QUERY SELECT cardinality(v_failures) = 0,
        ...
        'release-db-attestation-v41'::TEXT;
END;
$function$;

CREATE OR REPLACE FUNCTION public.koaryu_release_schema_preflight_v21()
...
BEGIN
    ...
    IF v.ready IS TRUE AND v.migration_count = 136 AND v.migration_head = '20260908183744'
       AND v.manifest_version = 'release-db-attestation-v41'
       AND cardinality(v.security_failures) = 0 AND cardinality(v.pending_versions) = 52
       AND v.pending_versions[cardinality(v.pending_versions)] = '20260908183744' THEN
        RETURN QUERY SELECT TRUE, 135, '20260908133504'::TEXT,
            v.pending_versions[1:cardinality(v.pending_versions)-1],
            ARRAY[]::TEXT[], 'release-db-attestation-v40'::TEXT;
        RETURN;
    END IF;
    RETURN QUERY SELECT FALSE, v.migration_count, v.migration_head,
        v.pending_versions, v.security_failures, 'release-db-attestation-v40'::TEXT;
END;
$function$;
...
BEGIN
    IF private.koaryu_release_critical_surface_manifest_v18() IS DISTINCT FROM '0:ffffe870f71abab5b1def36d5a496d2d5efa508204a773992c6d5fbe7aecae80' THEN
        RAISE EXCEPTION 'V41 additive critical_surface_manifest_v18 does not match the reviewed state.';
    END IF;
    IF private.koaryu_release_operational_manifest_v11() IS DISTINCT FROM '2efb0b2cf73beabfb219dd3642a824714c1b0619c5498ecf195071f929e651f6' THEN
        RAISE EXCEPTION 'V41 additive operational_manifest_v11 does not match the reviewed state.';
    END IF;
    IF private.koaryu_release_resource_ownership_manifest_v31() IS DISTINCT FROM '0:fda64c2e37a0e6efd3c69b1167fc2056fc53b85042ced46d511312c886a961a4' THEN
        RAISE EXCEPTION 'V41 additive resource_ownership_manifest_v31 does not match the reviewed state.';
    END IF;
    IF private.koaryu_release_operational_contract_v31() IS DISTINCT FROM '0:9f037cc4464637876016f47584d44910a6e175b2358eecb44486779273e75510' THEN
        RAISE EXCEPTION 'V41 additive operational_contract_v31 does not match the reviewed state.';
    END IF;
    IF private.koaryu_release_operational_manifest_v12() IS DISTINCT FROM '27cb3807a6ff60653fc0c7423428710a2c01d9254c1e5f59c0e2cb16c37f4d26' THEN
        RAISE EXCEPTION 'V41 additive operational_manifest_v12 does not match the reviewed state.';
    END IF;
    IF private.koaryu_release_student_rank_writer_manifest_v11() IS DISTINCT FROM '0:ba8bf9c6e32248c80d8621c29e4d9c062bb99bbc8e69da1533765a05931a26a9' THEN
        RAISE EXCEPTION 'V41 additive student_rank_writer_manifest_v11 does not match the reviewed state.';
    END IF;
    IF private.koaryu_release_student_rank_writer_manifest_v13() IS DISTINCT FROM '0:3201f4586df300ab113e7db82b6cead45d3ea3bc1add1ebc473625deaf80a702' THEN
        RAISE EXCEPTION 'V41 additive student_rank_writer_manifest_v13 does not match the reviewed state.';
    END IF;
END;
$derived$;
