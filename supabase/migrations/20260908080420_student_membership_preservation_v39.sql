-- Preserve retained membership status/dates and attest one coherent V39 state.
-- Apply through the guarded rollout after its exact raw-catalog inspection.
-- The migration and its history registration must commit in one transaction.
DO $predecessor$
DECLARE previous RECORD;
BEGIN
    IF (SELECT encode(extensions.digest(convert_to(prosrc, 'UTF8'), 'sha256'), 'hex')
        FROM pg_catalog.pg_proc WHERE oid = 'public.koaryu_release_schema_preflight_v19()'::REGPROCEDURE)
       IS DISTINCT FROM '8dc88fe13096cc71d8fc688cd7f266705bb81f7bbee94c44c7109d8ab28f0109' THEN
        RAISE EXCEPTION 'Membership preservation V39 requires the reviewed full V38 preflight.';
    END IF;
    -- The pinned full V38 body also verifies its exact pending-version sequence.
    SELECT * INTO previous FROM public.koaryu_release_schema_preflight_v19();
    IF previous.ready IS DISTINCT FROM TRUE
       OR previous.migration_count IS DISTINCT FROM 133
       OR previous.migration_head IS DISTINCT FROM '20260905022339'
       OR previous.manifest_version IS DISTINCT FROM 'release-db-attestation-v38'
       OR cardinality(previous.security_failures) IS DISTINCT FROM 0 THEN
        RAISE EXCEPTION 'Membership preservation V39 requires exact ready V38.';
    END IF;
END;
$predecessor$;
...
DECLARE
    ...
    v_membership_start_changed BOOLEAN := FALSE;
...
BEGIN
    ...
    ELSE
        v_membership_start_changed := (p_student ? 'membership_start_date')
            AND (NULLIF(p_student->>'membership_start_date', '')::DATE
                IS DISTINCT FROM v_existing.membership_start_date);
            ...
            IF FOUND THEN
                UPDATE public.student_program_memberships AS membership
                   SET status = CASE WHEN membership.status = 'paused' THEN 'paused' ELSE 'active' END,
                       ended_at = NULL,
                       started_at = CASE WHEN v_membership_start_changed
                           THEN COALESCE(v_membership_started_at, membership.started_at)
                           ELSE membership.started_at END,
    ...
    SELECT
        ...
        count(*) FILTER (
            WHERE manifest.v11_manifest IS DISTINCT FROM
              '0:ba8bf9c6e32248c80d8621c29e4d9c062bb99bbc8e69da1533765a05931a26a9'
        ) AS invalid_count
...
BEGIN
    ...
    SET expected_sha256 = 'fcaa04476824143c2aa69fc5d1722dbdd5b4459e0edb0249fbc24e89c0af417c'
    WHERE expectation_key = 'operational_contract_v31'
      AND expected_sha256 = 'abfd3d70c27d61b7be33193069739ddaef7db8e8a4cc591be1aeeebe130b64cc';
    GET DIAGNOSTICS changed = ROW_COUNT;
    IF changed <> 1 THEN RAISE EXCEPTION 'V39 requires the exact V38 operational expectation singleton.'; END IF;
END;
$expectation$;

-- Full current attestation, copied from the body-hash-checked V38 definition.
-- Only the history/version and two affected manifest checks advance here.
CREATE FUNCTION public.koaryu_release_schema_preflight_v20()
...
BEGIN
    ...
    IF v_count <> 134 OR v_head <> '20260908080420' THEN
        v_failures := array_append(v_failures, 'migration_history_v39');
    END IF;
    IF COALESCE(v_pending, ARRAY[]::TEXT[]) IS DISTINCT FROM ARRAY[
        ...
        '20260826073728','20260826102840','20260826155911','20260826185651','20260830065627','20260830082610','20260830151714','20260831022021','20260831054918','20260902001000','20260905022339','20260908080420'
    ]::TEXT[] THEN
    ...
    IF private.koaryu_release_resource_ownership_manifest_v31()
       IS DISTINCT FROM '0:daf85d2ad59a6853e872eebedbac8b21b960635cd3a7dec6e304cee2a79e465b' THEN
    ...
    IF private.koaryu_release_operational_manifest_v11()
       <> '43bb07edfbd3c6ee1431b6abda46ca1785b8ea117de2a13f95636fc7a3c3b263' THEN
    ...
    IF private.koaryu_release_operational_manifest_v12()
       IS DISTINCT FROM 'ead2be833206bfdadb455dfd41eaaae1b3d1d00b1ac2efabaaab6dd6ca4c5edf' THEN
 ...
 RETURN QUERY SELECT cardinality(v_failures) = 0,
        ...
        'release-db-attestation-v39'::TEXT;
END;
$function$
;

-- V19 retains the old response only after full V20 succeeds. V18 remains unchanged.
CREATE OR REPLACE FUNCTION public.koaryu_release_schema_preflight_v19()
...
DECLARE v RECORD;
BEGIN
    SELECT * INTO v FROM public.koaryu_release_schema_preflight_v20();
    IF v.ready IS TRUE
       AND v.migration_count = 134
       AND v.migration_head = '20260908080420'
       AND v.manifest_version = 'release-db-attestation-v39'
       AND cardinality(v.security_failures) = 0
       AND cardinality(v.pending_versions) = 50
       AND v.pending_versions[cardinality(v.pending_versions)] = '20260908080420' THEN
        RETURN QUERY SELECT TRUE, 133, '20260905022339'::TEXT,
            v.pending_versions[1:cardinality(v.pending_versions)-1],
            ARRAY[]::TEXT[], 'release-db-attestation-v38'::TEXT;
        RETURN;
    END IF;
    RETURN QUERY SELECT FALSE, v.migration_count, v.migration_head,
        v.pending_versions, v.security_failures, 'release-db-attestation-v38'::TEXT;
END;
$function$;
...
-- Definition/semantic checks do not assume the new history row already exists.
DO $installed$
BEGIN
    IF private.koaryu_release_student_rank_writer_manifest_v11() IS DISTINCT FROM '0:ba8bf9c6e32248c80d8621c29e4d9c062bb99bbc8e69da1533765a05931a26a9' THEN
        RAISE EXCEPTION 'V39 student_rank_writer_manifest_v11 did not match the reviewed state.';
    END IF;
    IF private.koaryu_release_student_rank_writer_manifest_v13() IS DISTINCT FROM '0:3201f4586df300ab113e7db82b6cead45d3ea3bc1add1ebc473625deaf80a702' THEN
        RAISE EXCEPTION 'V39 student_rank_writer_manifest_v13 did not match the reviewed state.';
    END IF;
    IF private.koaryu_release_resource_ownership_manifest_v31() IS DISTINCT FROM '0:daf85d2ad59a6853e872eebedbac8b21b960635cd3a7dec6e304cee2a79e465b' THEN
        RAISE EXCEPTION 'V39 resource_ownership_manifest_v31 did not match the reviewed state.';
    END IF;
    IF private.koaryu_release_operational_contract_v31() IS DISTINCT FROM '0:fcaa04476824143c2aa69fc5d1722dbdd5b4459e0edb0249fbc24e89c0af417c' THEN
        RAISE EXCEPTION 'V39 operational_contract_v31 did not match the reviewed state.';
    END IF;
    IF private.koaryu_release_operational_manifest_v12() IS DISTINCT FROM 'ead2be833206bfdadb455dfd41eaaae1b3d1d00b1ac2efabaaab6dd6ca4c5edf' THEN
        RAISE EXCEPTION 'V39 operational_manifest_v12 did not match the reviewed state.';
    END IF;
    IF EXISTS (
        SELECT 1 FROM (VALUES
            ('private.write_student_profile_atomic(uuid,uuid,uuid,jsonb,uuid[],jsonb,boolean,text)', '0d082598efc20157eda601183a6e66a84b17a139203ded1116cda656d73a6618'),
            ('private.koaryu_release_student_rank_writer_manifest_v13()', '1b1fd5a68f186481961bb4e03d3580d261421f5f2c9134fef255ec4a570c346c'),
            ('public.koaryu_release_schema_preflight_v20()', '0c3314d60a2ace254e0754a6ade3e1e61d0957dd3ea5cf096c408581e257cd4e'),
            ('public.koaryu_release_schema_preflight_v19()', 'ae516f5b78f018d7f5bff1b619080d68fa0242d82c35f60fbff78c6e183cc013'),
            ('public.koaryu_release_schema_preflight_v18()', 'a066fe2bd2a64f4c28561d3a9e5ec39adb2645efd711d01f46af21a96688990c')
        ) expected(signature, body_sha256)
        LEFT JOIN pg_catalog.pg_proc function ON function.oid = to_regprocedure(expected.signature)
        WHERE function.oid IS NULL
           OR encode(extensions.digest(convert_to(function.prosrc, 'UTF8'), 'sha256'), 'hex') IS DISTINCT FROM expected.body_sha256
    ) THEN RAISE EXCEPTION 'V39 installed function definitions did not match reviewed bodies.'; END IF;
END;
$installed$;
