-- A confirmed refund keeps its claim version until its receipt completes. No backfill.
-- Register this migration in the same transaction.
DO $predecessor$
DECLARE v RECORD;
BEGIN
    IF encode(extensions.digest(convert_to(pg_catalog.pg_get_functiondef(
        pg_catalog.to_regprocedure('public.koaryu_release_schema_preflight_v26()')),'UTF8'),'sha256'),'hex')
        IS DISTINCT FROM '503ef2b488e7c638a9bc3ea9ae7076b52354f42d5fa6bf0530a7c0b0391891aa' THEN
        RAISE EXCEPTION 'V46 requires the reviewed full V45 readiness definition.';
    END IF;
    SELECT * INTO v FROM public.koaryu_release_schema_preflight_v26();
    IF v.ready IS DISTINCT FROM TRUE OR v.migration_count IS DISTINCT FROM 140
       OR v.migration_head IS DISTINCT FROM '20260910185031'
       OR v.manifest_version IS DISTINCT FROM 'release-db-attestation-v45'
       OR cardinality(v.security_failures) IS DISTINCT FROM 0
       OR cardinality(v.pending_versions) IS DISTINCT FROM 56
       OR v.pending_versions[56] IS DISTINCT FROM '20260910185031' THEN
        RAISE EXCEPTION 'V46 requires a fully verified V45 predecessor.';
    END IF;
END;
$predecessor$;

CREATE OR REPLACE FUNCTION private.billing_operation_resource_version_v31(
    ...
    SELECT encode(extensions.digest(convert_to(
        CASE p_operation_type
            WHEN 'payment.refund' THEN jsonb_build_object(
                ...
                'refunded_amount_cents', p_payment.refunded_amount_cents - COALESCE((
                    -- Keep the original version until this operation finishes its own projection.
                    ...
                    JOIN public.billing_provider_operations AS operation
                     ...
                     AND operation.studio_id = resource.studio_id
                     AND operation.operation_type = resource.operation_type
                    JOIN public.billing_refunds AS refund
                      ON refund.studio_id = resource.studio_id
                     AND refund.payment_id = resource.resource_id
                     AND refund.stripe_refund_id = operation.provider_object_id
                    WHERE resource.studio_id = p_payment.studio_id
                      AND resource.operation_type = 'payment.refund'
                      AND resource.resource_type = 'payment'
                      AND resource.resource_id = p_payment.id
                      AND resource.payer_id = p_payment.payer_id
                      AND operation.state IN ('provider_succeeded', 'projected')
                      AND operation.stripe_connected_account_id = p_stripe_connected_account_id
                      AND operation.connect_account_generation = p_connect_account_generation
                      AND operation.result_summary = 'amount_cents:' || refund.amount_cents::TEXT
                      AND refund.stripe_account_id = p_stripe_connected_account_id
                      AND refund.connect_account_generation = p_connect_account_generation
                      AND refund.stripe_charge_id = p_payment.stripe_charge_id
                      AND refund.status = 'succeeded'
                      AND NOT refund.reconciliation_required
                      AND NOT p_payment.adjustment_reconciliation_required
                ), 0)
            )::TEXT
...
BEGIN
    IF (SELECT count(*) FROM private.koaryu_release_v31_expectations) IS DISTINCT FROM 1
       OR (SELECT expected_sha256 FROM private.koaryu_release_v31_expectations WHERE expectation_key='operational_contract_v31')
          IS DISTINCT FROM '9a317cbe7f22b280c63e2b52632b836fe526343460fc895b8115dffc38d5ed76'
       OR private.koaryu_release_resource_ownership_manifest_v31()
          IS DISTINCT FROM '0:5e96551e3a25a9ca17803c04afdd640771c6074b927d01941a45358d7e3ba733'
       OR private.koaryu_release_operational_contract_v31()
          IS DISTINCT FROM '0:b20e8b04bb440f43cfd1bdc8ae8b011a217b0ee102f5729bc1caf837b133e1b8' THEN
        RAISE EXCEPTION 'V46 requires the reviewed refund contract and original V31 expectation.';
    END IF;
    UPDATE private.koaryu_release_v31_expectations
    SET expected_sha256='b20e8b04bb440f43cfd1bdc8ae8b011a217b0ee102f5729bc1caf837b133e1b8'
    WHERE expectation_key='operational_contract_v31'
      AND expected_sha256='9a317cbe7f22b280c63e2b52632b836fe526343460fc895b8115dffc38d5ed76';
    GET DIAGNOSTICS changed = ROW_COUNT;
    IF changed IS DISTINCT FROM 1 OR private.koaryu_release_operational_manifest_v12()
       IS DISTINCT FROM 'ada17d08e26869bdf66723570e72c439facf96d671e7bab8826229949c4a9e87' THEN
        RAISE EXCEPTION 'V46 guarded expectation correction did not verify.';
    END IF;
END;
$expectation$;

CREATE FUNCTION public.koaryu_release_schema_preflight_v27()
...
BEGIN
    ...
    IF v_count <> 141 OR v_head <> '20260914033337' THEN
        v_failures := array_append(v_failures, 'migration_history_v46');
    END IF;
    IF COALESCE(v_pending, ARRAY[]::TEXT[]) IS DISTINCT FROM ARRAY[
        ...
        '20260826073728','20260826102840','20260826155911','20260826185651','20260830065627','20260830082610','20260830151714','20260831022021','20260831054918','20260902001000','20260905022339','20260908080420','20260908133504','20260908183744','20260910084231','20260910093958','20260910135133','20260910185031','20260914033337'
    ]::TEXT[] THEN
    ...
    IF private.koaryu_release_resource_ownership_manifest_v31()
       IS DISTINCT FROM '0:5e96551e3a25a9ca17803c04afdd640771c6074b927d01941a45358d7e3ba733' THEN
 ...
 RETURN QUERY SELECT cardinality(v_failures) = 0,
        ...
        'release-db-attestation-v46'::TEXT;
END;
$function$;

CREATE OR REPLACE FUNCTION public.koaryu_release_schema_preflight_v26()
...
BEGIN
    ...
    IF v.ready IS TRUE AND v.migration_count = 141 AND v.migration_head = '20260914033337'
       AND v.manifest_version = 'release-db-attestation-v46'
       AND cardinality(v.security_failures) = 0 AND cardinality(v.pending_versions) = 57
       AND v.pending_versions[cardinality(v.pending_versions)] = '20260914033337' THEN
        RETURN QUERY SELECT TRUE, 140, '20260910185031'::TEXT,
            v.pending_versions[1:cardinality(v.pending_versions)-1],
            ARRAY[]::TEXT[], 'release-db-attestation-v45'::TEXT;
        RETURN;
    END IF;
    RETURN QUERY SELECT FALSE, v.migration_count, v.migration_head,
        v.pending_versions, v.security_failures, 'release-db-attestation-v45'::TEXT;
END;
$function$;
...
BEGIN
    SELECT * INTO v FROM public.koaryu_release_schema_preflight_v27();
    IF v.migration_count IS DISTINCT FROM 140 OR v.migration_head IS DISTINCT FROM '20260910185031'
       OR v.security_failures IS DISTINCT FROM ARRAY['migration_history_v46',
           'migration_history_sequence_v31','migration_history_sequence_v30']::TEXT[] THEN
        RAISE EXCEPTION 'V46 installed contracts did not verify before history registration: %', row_to_json(v);
    END IF;
END;
$installed$;
