-- Make existing-key invoice closeout retries follow the invoice writer's lock order.
DO $predecessor$
DECLARE v RECORD;
BEGIN
    IF encode(extensions.digest(convert_to(pg_catalog.pg_get_functiondef(
        pg_catalog.to_regprocedure('public.koaryu_release_schema_preflight_v31()')),'UTF8'),'sha256'),'hex')
        IS DISTINCT FROM 'afb73a78c07b6095abbd29f5ede17a7a4cba77238f041cc746a7f0ec3faa1242' THEN
        RAISE EXCEPTION 'V51 requires the reviewed full V50 readiness definition.';
    END IF;
    SELECT * INTO v FROM public.koaryu_release_schema_preflight_v31();
    IF v.ready IS DISTINCT FROM TRUE OR v.migration_count IS DISTINCT FROM 145
       OR v.migration_head IS DISTINCT FROM '20260920154441'
       OR v.manifest_version IS DISTINCT FROM 'release-db-attestation-v50'
       OR cardinality(v.security_failures) IS DISTINCT FROM 0
       OR cardinality(v.pending_versions) IS DISTINCT FROM 61
       OR v.pending_versions[61] IS DISTINCT FROM '20260920154441' THEN
        RAISE EXCEPTION 'V51 requires a fully verified V50 predecessor.';
    END IF;
END;
$predecessor$;

-- An existing-key retry used to lock its alias, resource and operation before the
-- invoice, so it could deadlock with a new-key claim that already held the invoice.
-- It now takes the writer's leading invoice, payer and mutation-owner locks first.
-- Replay and claim validation, errors and results are unchanged. No rows are rewritten.
DO $v51_closeout_lock_order$
DECLARE v_definition TEXT;
        v_declare_old CONSTANT TEXT := $old$        v_result JSONB;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM public.staff_roles AS membership$old$;
        v_declare_new CONSTANT TEXT := $new$        v_result JSONB;
        v_invoice_payer_id UUID;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM public.staff_roles AS membership$new$;
        v_lock_old CONSTANT TEXT := $old$    -- Certified terminal aliases replay before the fresh-mutation function
    -- checks the invoice's current draft/open state.
$old$;
        v_lock_new CONSTANT TEXT := $new$    -- Take the V31 writer's leading locks before any alias, resource or operation
    -- lock: the invoice, its matching payer, then the invoice mutation owner. Missing
    -- or mismatched rows lock nothing extra and fail validation below, as before.
    SELECT payer_id INTO v_invoice_payer_id
      FROM public.billing_invoices
     WHERE id=p_resource_id AND studio_id=p_studio_id
     FOR UPDATE;
    IF FOUND AND v_invoice_payer_id IS NOT DISTINCT FROM p_payer_id THEN
        PERFORM 1 FROM public.billing_payers
         WHERE id=p_payer_id AND studio_id=p_studio_id
         FOR UPDATE;
    END IF;
    PERFORM 1 FROM public.billing_invoice_mutation_owners
...
BEGIN
    SELECT pg_get_functiondef(
      ...
      INTO v_definition;
    IF (length(v_definition)-length(replace(v_definition,v_declare_old,'')))/length(v_declare_old)<>1
       OR (length(v_definition)-length(replace(v_definition,v_lock_old,'')))/length(v_lock_old)<>1 THEN
        RAISE EXCEPTION 'V51 closeout lock order requires the exact V34 closeout body.';
    END IF;
    EXECUTE replace(replace(v_definition,v_declare_old,v_declare_new),v_lock_old,v_lock_new);
END;
$v51_closeout_lock_order$;

-- The closeout function is part of the inherited operational contract.
DO $expectation$
BEGIN
    UPDATE private.koaryu_release_v31_expectations
    SET expected_sha256 = 'f821468c4e9e1aa4540b0f3f070d290fd136ade5536c0ff87efd86144def8fa2'
    WHERE expectation_key = 'operational_contract_v31'
      AND expected_sha256 = 'cf96bff39d8cd5a30cf3378222544644f733f300ea435df6de03ea5e176a9cf6';
    IF NOT FOUND THEN
        RAISE EXCEPTION 'V51 operational expectation does not match its reviewed predecessor.';
    END IF;
END;
$expectation$;

CREATE FUNCTION public.koaryu_release_schema_preflight_v32()
...
BEGIN
    ...
    IF v_count <> 146 OR v_head <> '20260925030000' THEN
        v_failures := array_append(v_failures, 'migration_history_v51');
    END IF;
    ...
    IF private.koaryu_release_resource_ownership_manifest_v31()
       IS DISTINCT FROM '0:7eeca9f2e7003fd235eb60c70b65afd3ac17b9893dd582c6c9b6c9ab13ba61f2' THEN
    ...
    IF private.koaryu_release_operational_manifest_v11()
       <> 'fd84fac705af709620f3529f5769bb171cd21ece256c9862cf42152733703ad5' THEN
    ...
    IF private.koaryu_release_operational_manifest_v12()
       IS DISTINCT FROM '9c715f1b254ed82df63e645327be0da48e5ef0d5a8d6f44a2eeb27d687fa8e58' THEN
 ...
 RETURN QUERY SELECT cardinality(v_failures) = 0,
        ...
        'release-db-attestation-v51'::TEXT;
END;
$function$;

CREATE OR REPLACE FUNCTION public.koaryu_release_schema_preflight_v31()
...
DECLARE v RECORD;
BEGIN
    SELECT * INTO v FROM public.koaryu_release_schema_preflight_v32();
    IF v.ready IS TRUE AND v.migration_count = 146 AND v.migration_head = '20260925030000'
       AND v.manifest_version = 'release-db-attestation-v51'
       AND cardinality(v.security_failures) = 0 AND cardinality(v.pending_versions) = 62
       AND v.pending_versions[cardinality(v.pending_versions)] = '20260925030000' THEN
        RETURN QUERY SELECT TRUE, 145, '20260920154441'::TEXT,
            v.pending_versions[1:cardinality(v.pending_versions)-1],
            ARRAY[]::TEXT[], 'release-db-attestation-v50'::TEXT;
        RETURN;
    END IF;
    RETURN QUERY SELECT FALSE, v.migration_count, v.migration_head,
        v.pending_versions, v.security_failures, 'release-db-attestation-v50'::TEXT;
END;
$function$;

ALTER FUNCTION public.koaryu_release_schema_preflight_v32() OWNER TO postgres;
REVOKE ALL ON FUNCTION public.koaryu_release_schema_preflight_v32() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.koaryu_release_schema_preflight_v32() TO service_role;
...
BEGIN
    SELECT * INTO v FROM public.koaryu_release_schema_preflight_v32();
    IF v.migration_count IS DISTINCT FROM 145 OR v.migration_head IS DISTINCT FROM '20260920154441'
       OR v.security_failures IS DISTINCT FROM ARRAY['migration_history_v51',
           'migration_history_sequence_v31','migration_history_sequence_v30']::TEXT[] THEN
        RAISE EXCEPTION 'V51 installed contracts did not verify before history registration: %',row_to_json(v);
    END IF;
END;
$installed$;
