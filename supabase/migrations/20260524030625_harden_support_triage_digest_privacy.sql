-- ==========================================
-- Koaryu v1 - Harden support triage digest privacy
-- ==========================================
--
-- Daily automation digests are broad operational summaries. Keep the response
-- shape stable, but never echo user-entered ticket subject/details into fields
-- that can be posted outside the private support queue.
    ...
    sanitized AS (
        SELECT
            ...
            CASE
                ...
                WHEN topic = 'billing' THEN 'billing support request'
                WHEN topic = 'account_access' THEN 'account access support request'
                WHEN topic = 'bug_report' THEN 'bug report support request'
                WHEN topic = 'product_question' THEN 'product question support request'
                ELSE 'general support request'
            END AS subject,
            CASE
                WHEN topic = 'student_records' THEN 'details withheld'
                WHEN topic = 'billing' THEN 'metadata only: ' || severity || ' billing support request is ' || status
                WHEN topic = 'account_access' THEN 'metadata only: ' || severity || ' account access support request is ' || status
                WHEN topic = 'bug_report' THEN 'metadata only: ' || severity || ' bug report support request is ' || status
                WHEN topic = 'product_question' THEN 'metadata only: ' || severity || ' product question support request is ' || status
                ELSE 'metadata only: ' || severity || ' general support request is ' || status
            END AS summary_seed,
            CASE
