-- A recovered provider subscription may not yet have confirmed billing terms.
DO $predecessor$
DECLARE v RECORD;
BEGIN
    IF encode(extensions.digest(convert_to(pg_catalog.pg_get_functiondef(
        pg_catalog.to_regprocedure('public.koaryu_release_schema_preflight_v29()')),'UTF8'),'sha256'),'hex')
        IS DISTINCT FROM 'ab774655f9c9b359d6f56ee7626ad7f871844c133e52e1012443402accdbe515' THEN
        RAISE EXCEPTION 'V49 requires the reviewed full V48 readiness definition.';
    END IF;
    SELECT * INTO v FROM public.koaryu_release_schema_preflight_v29();
    IF v.ready IS DISTINCT FROM TRUE OR v.migration_count IS DISTINCT FROM 143
       OR v.migration_head IS DISTINCT FROM '20260920035023'
       OR v.manifest_version IS DISTINCT FROM 'release-db-attestation-v48'
       OR cardinality(v.security_failures) IS DISTINCT FROM 0
       OR cardinality(v.pending_versions) IS DISTINCT FROM 59
       OR v.pending_versions[59] IS DISTINCT FROM '20260920035023' THEN
        RAISE EXCEPTION 'V49 requires a fully verified V48 predecessor.';
    END IF;
    IF (SELECT count(*) FROM pg_catalog.pg_attribute a
        ...
        WHERE a.attrelid='public.billing_subscriptions'::REGCLASS
          ...
          AND ((a.attname='currency' AND pg_get_expr(d.adbin,d.adrelid)='''usd''::text')
            OR (a.attname='billing_interval' AND pg_get_expr(d.adbin,d.adrelid)='''monthly''::text')))
       IS DISTINCT FROM 2 THEN
        RAISE EXCEPTION 'V49 subscription term columns do not match the reviewed predecessor.';
    END IF;
END;
$predecessor$;
...
CREATE FUNCTION public.koaryu_release_schema_preflight_v30()
...
BEGIN
    ...
    IF v_count <> 144 OR v_head <> '20260920052705' THEN
        v_failures := array_append(v_failures, 'migration_history_v49');
    END IF;
 ...
 RETURN QUERY SELECT cardinality(v_failures) = 0,
        ...
        'release-db-attestation-v49'::TEXT;
END;
$function$;

CREATE OR REPLACE FUNCTION public.koaryu_release_schema_preflight_v29()
...
BEGIN
    ...
    IF v.ready IS TRUE AND v.migration_count = 144 AND v.migration_head = '20260920052705'
       AND v.manifest_version = 'release-db-attestation-v49'
       AND cardinality(v.security_failures) = 0 AND cardinality(v.pending_versions) = 60
       AND v.pending_versions[cardinality(v.pending_versions)] = '20260920052705' THEN
        RETURN QUERY SELECT TRUE, 143, '20260920035023'::TEXT,
            v.pending_versions[1:cardinality(v.pending_versions)-1],
            ARRAY[]::TEXT[], 'release-db-attestation-v48'::TEXT;
        RETURN;
    END IF;
    RETURN QUERY SELECT FALSE, v.migration_count, v.migration_head,
        v.pending_versions, v.security_failures, 'release-db-attestation-v48'::TEXT;
END;
$function$;
...
BEGIN
    SELECT * INTO v FROM public.koaryu_release_schema_preflight_v30();
    IF v.migration_count IS DISTINCT FROM 143 OR v.migration_head IS DISTINCT FROM '20260920035023'
       OR v.security_failures IS DISTINCT FROM ARRAY['migration_history_v49',
           'migration_history_sequence_v31','migration_history_sequence_v30']::TEXT[] THEN
        RAISE EXCEPTION 'V49 installed contracts did not verify before history registration: %',row_to_json(v);
    END IF;
END;
$installed$;
