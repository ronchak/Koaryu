-- ==========================================
-- Koaryu v1 - Fail closed on cross-tenant import ID conflicts
-- ==========================================
--
-- Keep the atomic CSV import writer idempotent for same-studio retries, but do
-- not let an upstream/deterministic ID collision reassign student or guardian
-- ownership across studios.
...
BEGIN
    ...
    INSERT INTO public.students AS existing (
        id,
        studio_id,
        legal_first_name,
        legal_last_name,
    ...
    IF v_upsert_count <> 1 THEN
        RAISE EXCEPTION 'Student import id already belongs to another studio.'
            USING ERRCODE = 'P0001';
    END IF;
    ...
    IF v_guardian_name IS NOT NULL THEN
        ...
        INSERT INTO public.guardians AS existing (
            id,
            studio_id,
            first_name,
            last_name,
        ...
        GET DIAGNOSTICS v_upsert_count = ROW_COUNT;
        IF v_upsert_count <> 1 THEN
            RAISE EXCEPTION 'Student import guardian id already belongs to another studio.'
                USING ERRCODE = 'P0001';
        END IF;

        INSERT INTO public.student_guardians AS existing (
            id,
            student_id,
            guardian_id
        )
        ...
        ON CONFLICT (id) DO UPDATE
         ...
         WHERE existing.student_id = EXCLUDED.student_id
           AND existing.guardian_id = EXCLUDED.guardian_id;

        GET DIAGNOSTICS v_upsert_count = ROW_COUNT;
        IF v_upsert_count <> 1 THEN
            RAISE EXCEPTION 'Student import guardian link id already points at another relationship.'
                USING ERRCODE = 'P0001';
        END IF;
