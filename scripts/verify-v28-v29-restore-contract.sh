...
predecessor_readiness="$("$psql_bin" "${restored_args[@]}" --tuples-only --no-align --command="SELECT ready::TEXT || '|' || migration_count::TEXT || '|' || migration_head || '|' || cardinality(security_failures)::TEXT || '|' || manifest_version FROM public.koaryu_release_schema_preflight_v8();" | tr -d '\r\n')"
if [[ "$predecessor_readiness" != "true|121|20260826073728|0|release-db-attestation-v28" ]]; then
...
readiness="$("$psql_bin" "${restored_args[@]}" --tuples-only --no-align --command="SELECT ready::TEXT || '|' || migration_count::TEXT || '|' || migration_head || '|' || cardinality(security_failures)::TEXT || '|' || COALESCE(array_to_string(security_failures,','),'') || '|' || manifest_version FROM public.koaryu_release_schema_preflight_v9();" | tr -d '\r\n')"
compat_readiness="$("$psql_bin" "${restored_args[@]}" --tuples-only --no-align --command="SELECT ready::TEXT || '|' || migration_count::TEXT || '|' || migration_head || '|' || cardinality(security_failures)::TEXT || '|' || COALESCE(array_to_string(security_failures,','),'') || '|' || manifest_version FROM public.koaryu_release_schema_preflight_v8();" | tr -d '\r\n')"
...
if [[ "$operational_contract" != "0:acb02796bef50ae55a9201315769fec5702de102fb251747f57d6a46cba71407" ]]; then
  echo "Restored V29 operational contract mismatch: $operational_contract" >&2
  exit 1
fi
if [[ "$operational_manifest" != "cf3ce387638a39bb70488a2bfd2c1e3b419df373d3f00dc28a4d371864e76abb" ]]; then
  echo "Restored V29 operational manifest mismatch: $operational_manifest" >&2
  exit 1
fi
if [[ "$v28_canonical_manifest" != "1cc3f8d95e1b41cd0e0bc4674c7461d456ee2aca53229b6c512630aee0249285" ]]; then
  echo "Restored post-V29 V28 manifest mismatch: $v28_canonical_manifest" >&2
  exit 1
fi
if [[ "$readiness" != "true|122|20260826102840|0||release-db-attestation-v29" ]]; then
  echo "Restored V29 readiness mismatch: $readiness" >&2
  exit 1
fi
if [[ "$compat_readiness" != "true|121|20260826073728|0||release-db-attestation-v28" ]]; then
  echo "Restored V28 compatibility readiness mismatch: $compat_readiness" >&2
  exit 1
fi
if [[ "$expectation_state" != "1:d42018ef129f0f7efcf405f5c77936172a31afd09ba6d94cae31398192cd8505" ]]; then
...
echo "PASS: V28 dump/restore then migration 122 produced the V29 transition contract."
