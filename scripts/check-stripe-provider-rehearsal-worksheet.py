...
def validate_template(template: dict[str, Any]) -> list[str]:
    ...
    if _exact_keys(errors, "supplemental evidence", supplemental, VALIDATOR.SUPPLEMENTAL_KEYS):
        ...
        readbacks = (
            ...
            (supplemental.get("external_payment", {}).get("provider_operation_inventory_readback"), "external payment inventory readback", "external_payment.inventory", "zero", VALIDATOR.EXTERNAL_INVENTORY_KEYS, "status"),
            (supplemental.get("external_payment", {}).get("local_readback"), "external payment local readback", "external_payment.local", "externally_recorded", VALIDATOR.EXTERNAL_LOCAL_KEYS, "payment_status"),
