...
if [[ $# -eq 3 ]]; then
  ...
  if [[ -z "$PSQL_BINARY" ]]; then
    echo "PostgreSQL psql is required for the Core checkout concurrency check." >&2
    exit 127
  fi
...
')" || {
  ...
  echo "Usage: scripts/verify-core-checkout-accept-reserve-concurrency.sh [psql host port]" >&2
  exit 2
fi
