...
PROVIDER_ALLOWED = {"id", "object", "livemode", "status", "created", "customer", "subscription", "subscription_item", "setup_intent", "payment_method", "payment_intent", "latest_charge", "charge", "invoice", "refund", "amount", "amount_refunded", "application_fee_amount", "quantity", "product", "price", "test_clock", "account", "type", "metadata", "last_payment_error", "canceled_at", "ended_at", "released_at"}
...
def _project_group_four_local(manifest: dict[str, Any], final: dict[str, Any], final_rows: list[dict[str, Any]], boundary: str, validator: Any) -> dict[str, Any]:
    ...
    if payment.get("status") != "externally_recorded" or payment.get("currency") is None or metadata.get("amount_cents") != payment.get("amount_cents") or metadata.get("external_method") != payment.get("external_method"):
        raise CollectorError("external payment and audit amount/method do not cross-bind")
    external = {"workflow_id": "payment.external.record", "local_payment_id": payment["id"], "local_status": payment["status"], "replay_payment_id": payment["id"], "caller_request_key_sha256": payment.get("caller_request_key_sha256"), "replay_outcome": "same_row", "audit_count": 1, "invoice_id": payment.get("invoice_id"), "provider_mutation_count": 0, "provider_operation_inventory_readback": _readback(validator.SUPPLEMENTAL_SOURCES["external_payment.inventory"], "zero", boundary), "local_readback": _readback(validator.SUPPLEMENTAL_SOURCES["external_payment.local"], payment["status"], boundary)}
...
def assemble(manifest: dict[str, Any], values: list[Any]) -> dict[str, Any]:
    ...
    frozen = test_clock.get("created") or test_clock.get("frozen_time")
    if type(frozen) is not int or frozen <= 0:
        raise CollectorError("test clock final readback lacks one positive provider boundary")
    ambiguity = index["local:operations:ambiguity_parent"]
    supplemental = {
        "payer_setup_lifecycle": setup,
        **supplemental_local,
        **supplemental_three,
        "period_advancement": {"method": "stripe_test_clock.advance", "test_clock_id": test_clock["id"], "advances_to": frozen, "observed_provider_boundary": frozen, "direct_database_timestamp_edit": False, "provider_readback": _readback(validator.SUPPLEMENTAL_SOURCES["period_advancement.provider"], "advanced", boundary), "local_readback": _readback(validator.SUPPLEMENTAL_SOURCES["period_advancement.local"], "completed", boundary)},
        "ambiguity_recovery": {"workflow_id": "payer.sync", "durable_operation_id": ambiguity["id"], "provider_mutation_count": ambiguity["provider_request_attempt_count"], "automatic_retry_count": 0, "caller_request_key_sha256": facts_three["ambiguous_caller_key_sha256"], "mutation_step_name": "payer.customer_create", "provider_readback": _readback(validator.SUPPLEMENTAL_SOURCES["ambiguity.provider"], "found", boundary), "local_readback": _readback(validator.SUPPLEMENTAL_SOURCES["ambiguity.local"], "completed", boundary)},
