...
if [[ $# -ne 8 ]]; then
  echo "Usage: scripts/verify-v30-v37-restore-contract.sh pg_dump pg_restore createdb psql host port temp-dir repository-root" >&2
  exit 2
fi
...
# Match the existing Python restore tools: reject routing overrides and verify
...
assert_restored_preflight_rejects() {
  ...
  actual_ready="$(read_restored "BEGIN; $mutation_sql SELECT ready::TEXT FROM public.koaryu_release_schema_preflight_v12(); ROLLBACK;")"
...
restored_predecessor="$(read_restored "SELECT ready::TEXT || '|' || migration_count::TEXT || '|' || migration_head || '|' || cardinality(security_failures)::TEXT || '|' || COALESCE(array_to_string(security_failures,','),'') || '|' || manifest_version FROM public.koaryu_release_schema_preflight_v11();")"
if [[ "$restored_predecessor" != "true|125|20260826155911|0||release-db-attestation-v30" ]]; then
...
readiness="$(read_restored "SELECT ready::TEXT || '|' || migration_count::TEXT || '|' || migration_head || '|' || cardinality(security_failures)::TEXT || '|' || COALESCE(array_to_string(security_failures,','),'') || '|' || manifest_version FROM public.koaryu_release_schema_preflight_v12();")"
compat_readiness="$(read_restored "SELECT ready::TEXT || '|' || migration_count::TEXT || '|' || migration_head || '|' || cardinality(security_failures)::TEXT || '|' || COALESCE(array_to_string(security_failures,','),'') || '|' || manifest_version FROM public.koaryu_release_schema_preflight_v11();")"
...
drift_ready="$(read_restored "BEGIN; ALTER FUNCTION private.validate_billing_payment_identity_change() SET search_path=public; SELECT ready::TEXT FROM public.koaryu_release_schema_preflight_v12(); ROLLBACK;")"
if [[ "$drift_ready" != "false" ]]; then
  echo "Restored V31 preflight accepted payment-identity function drift." >&2
  exit 1
fi

schedule_body_ready="$(read_restored "BEGIN; UPDATE pg_proc SET prosrc=prosrc || chr(10) || '-- injected schedule-window drift' WHERE oid='public.schedule_window_read(uuid,date,date,text)'::REGPROCEDURE; SELECT ready::TEXT FROM public.koaryu_release_schema_preflight_v12(); ROLLBACK;")"
if [[ "$schedule_body_ready" != "false" ]]; then
  echo "Restored V31 preflight accepted schedule-window function-body drift." >&2
  exit 1
fi

schedule_acl_ready="$(read_restored "BEGIN; REVOKE EXECUTE ON FUNCTION public.schedule_window_read(UUID,DATE,DATE,TEXT) FROM service_role; SELECT ready::TEXT FROM public.koaryu_release_schema_preflight_v12(); ROLLBACK;")"
...
expectation_acl_ready="$(read_restored "BEGIN; GRANT UPDATE ON private.koaryu_release_v31_expectations TO service_role; SELECT ready::TEXT FROM public.koaryu_release_schema_preflight_v12(); ROLLBACK;")"
...
owner_acl_ready="$(read_restored "BEGIN; GRANT UPDATE ON public.billing_invoice_mutation_owners TO service_role; SELECT ready::TEXT FROM public.koaryu_release_schema_preflight_v12(); ROLLBACK;")"
if [[ "$owner_acl_ready" != "false" ]]; then
  echo "Restored V31 preflight accepted broadened invoice-owner ACLs." >&2
  exit 1
fi

owner_custom_acl_ready="$(read_restored "BEGIN; CREATE ROLE v31_owner_acl_probe NOLOGIN; GRANT SELECT ON public.billing_invoice_mutation_owners TO v31_owner_acl_probe; SELECT ready::TEXT FROM public.koaryu_release_schema_preflight_v12(); ROLLBACK;")"
if [[ "$owner_custom_acl_ready" != "false" ]]; then
  echo "Restored V31 preflight accepted a custom-role invoice-owner grant." >&2
  exit 1
fi

owner_extra_column_ready="$(read_restored "BEGIN; ALTER TABLE public.billing_invoice_mutation_owners ADD COLUMN unexpected_probe TEXT; SELECT ready::TEXT FROM public.koaryu_release_schema_preflight_v12(); ROLLBACK;")"
...
"$psql_bin" "${restored_args[@]}" --single-transaction \
  --file="$repository_root/supabase/migrations/20260830065627_release_invoice_retry_preread_lease_v32.sql" \
  --command="INSERT INTO supabase_migrations.schema_migrations(version,name) VALUES ('20260830065627','release_invoice_retry_preread_lease_v32');" >/dev/null

"$psql_bin" "${restored_args[@]}" --single-transaction \
  --file="$repository_root/supabase/migrations/20260830082610_invoice_retry_release_compatibility_v33.sql" \
  --command="INSERT INTO supabase_migrations.schema_migrations(version,name) VALUES ('20260830082610','invoice_retry_release_compatibility_v33');" >/dev/null

"$psql_bin" "${restored_args[@]}" --single-transaction \
  --file="$repository_root/supabase/migrations/20260830151714_invoice_retry_closeout_contract_v34.sql" \
  --command="INSERT INTO supabase_migrations.schema_migrations(version,name) VALUES ('20260830151714','invoice_retry_closeout_contract_v34');" >/dev/null

actual_catalog="$(read_restored "$catalog_sql")"
expected_catalog="$(cd "$repository_root" && node --input-type=module --eval "import { EXPECTED_V34_RESTORED_CATALOG_STATE } from './scripts/studio-comp-migration-rollout.mjs'; process.stdout.write(EXPECTED_V34_RESTORED_CATALOG_STATE);")"
echo "RESTORED_V34_CATALOG_STATE=$actual_catalog"

if [[ "$actual_catalog" != "$expected_catalog" ]]; then
  echo "Restored V34 catalog mismatch: $actual_catalog" >&2
  exit 1
fi

"$psql_bin" "${restored_args[@]}" --single-transaction \
  --file="$repository_root/supabase/migrations/20260831022021_stripe_rehearsal_evidence_rpc_v35.sql" \
  --command="INSERT INTO supabase_migrations.schema_migrations(version,name) VALUES ('20260831022021','stripe_rehearsal_evidence_rpc_v35');" >/dev/null

actual_catalog="$(read_restored "$catalog_sql")"
expected_catalog="$(cd "$repository_root" && node --input-type=module --eval "import { EXPECTED_V35_RESTORED_CATALOG_STATE } from './scripts/studio-comp-migration-rollout.mjs'; process.stdout.write(EXPECTED_V35_RESTORED_CATALOG_STATE);")"
echo "RESTORED_V35_CATALOG_STATE=$actual_catalog"

if [[ "$actual_catalog" != "$expected_catalog" ]]; then
  echo "Restored V35 catalog mismatch: $actual_catalog" >&2
  exit 1
fi

actual_readiness="$(read_restored "SELECT ready::TEXT || '|' || migration_count::TEXT || '|' || migration_head || '|' || cardinality(security_failures)::TEXT || '|' || COALESCE(array_to_string(security_failures,','),'') || '|' || manifest_version FROM public.koaryu_release_schema_preflight_v16();")"
echo "RESTORED_V35_READINESS=$actual_readiness"
if [[ "$actual_readiness" != "true|130|20260831022021|0||release-db-attestation-v35" ]]; then
  echo "Restored V35 readiness mismatch: $actual_readiness" >&2
  exit 1
fi

"$psql_bin" "${restored_args[@]}" --single-transaction \
  --file="$repository_root/supabase/migrations/20260831054918_payer_setup_recovery_v36.sql" \
  --command="INSERT INTO supabase_migrations.schema_migrations(version,name) VALUES ('20260831054918','payer_setup_recovery_v36');" >/dev/null

actual_catalog="$(read_restored "$catalog_sql")"
expected_catalog="$(cd "$repository_root" && node --input-type=module --eval "import { EXPECTED_V36_RESTORED_CATALOG_STATE } from './scripts/studio-comp-migration-rollout.mjs'; process.stdout.write(EXPECTED_V36_RESTORED_CATALOG_STATE);")"
echo "RESTORED_V36_CATALOG_STATE=$actual_catalog"

if [[ "$actual_catalog" != "$expected_catalog" ]]; then
  echo "Restored V36 catalog mismatch: $actual_catalog" >&2
  exit 1
fi

actual_readiness="$(read_restored "SELECT ready::TEXT || '|' || migration_count::TEXT || '|' || migration_head || '|' || cardinality(security_failures)::TEXT || '|' || COALESCE(array_to_string(security_failures,','),'') || '|' || manifest_version FROM public.koaryu_release_schema_preflight_v17();")"
echo "RESTORED_V36_READINESS=$actual_readiness"
if [[ "$actual_readiness" != "true|131|20260831054918|0||release-db-attestation-v36" ]]; then
  echo "Restored V36 readiness mismatch: $actual_readiness" >&2
  exit 1
fi

"$psql_bin" "${restored_args[@]}" --single-transaction \
  --file="$repository_root/supabase/migrations/20260902001000_fix_billing_adjustment_trigger_table_guards.sql" \
  --command="INSERT INTO supabase_migrations.schema_migrations(version,name) VALUES ('20260902001000','fix_billing_adjustment_trigger_table_guards');" >/dev/null

actual_catalog="$(read_restored "$catalog_sql")"
expected_catalog="$(cd "$repository_root" && node --input-type=module --eval "import { EXPECTED_V37_RESTORED_CATALOG_STATE } from './scripts/studio-comp-migration-rollout.mjs'; process.stdout.write(EXPECTED_V37_RESTORED_CATALOG_STATE);")"
echo "RESTORED_V37_CATALOG_STATE=$actual_catalog"

if [[ "$actual_catalog" != "$expected_catalog" ]]; then
  echo "Restored V37 catalog mismatch: $actual_catalog" >&2
  exit 1
fi

actual_readiness="$(read_restored "SELECT ready::TEXT || '|' || migration_count::TEXT || '|' || migration_head || '|' || cardinality(security_failures)::TEXT || '|' || COALESCE(array_to_string(security_failures,','),'') || '|' || manifest_version FROM public.koaryu_release_schema_preflight_v18();")"
echo "RESTORED_V37_READINESS=$actual_readiness"
if [[ "$actual_readiness" != "true|132|20260902001000|0||release-db-attestation-v37" ]]; then
  echo "Restored V37 readiness mismatch: $actual_readiness" >&2
  exit 1
fi

trigger_guard="$(read_restored 'SELECT private.koaryu_release_adjustment_trigger_guard_manifest_v37();')"

expected_trigger_guard="$(cd "$repository_root" && node --input-type=module --eval "import { EXPECTED_V37_TRIGGER_GUARD_MANIFEST } from './scripts/studio-comp-migration-rollout.mjs'; process.stdout.write(EXPECTED_V37_TRIGGER_GUARD_MANIFEST);")"
echo "RESTORED_V37_TRIGGER_GUARD=$trigger_guard"

if [[ "$trigger_guard" != "$expected_trigger_guard" ]]; then
  echo "Restored V37 adjustment trigger guard mismatch: $trigger_guard" >&2
  exit 1
fi
