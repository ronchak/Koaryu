...
export const ROLLOUT = Object.freeze({
  ...
  verifiedV24MigrationCount: 117,
  finalMigrationCount: 119,
...
const V24_FINAL_PENDING_VERSIONS = Object.freeze(
  ROLLOUT.finalPendingVersions.slice(0, -2),
);
...
export const EXPECTED_TRIAL_LOCKED_OPERATIONAL_READINESS =
  ...
  V24_FINAL_PENDING_VERSIONS.slice(
    0,
    V24_FINAL_PENDING_VERSIONS.length -
      (ROLLOUT.verifiedV24MigrationCount - ROLLOUT.trialLockedMigrationCount),
  ).join(",") +
...
export function validateOperationalReadiness(value) {
  if (value !== EXPECTED_OPERATIONAL_READINESS) {
    throw new RolloutError("V25 operational readiness did not match the exact release state.");
  }
  return value;
}
...
export function buildInspectionTokenForAcceptedState(packet, target, result) {
  if (
    ...
    throw new RolloutError("Inspection tokens require an accepted pre, intermediate, recovery, convergence, attested, return-attested, retained, critical, column-attested, trial-locked, staff-identity, restored-v22, canonical-v23, restored-v23-pending-v24, verified-v24, or post probe state.");
  }
...
export function verifySourceTree(sourceRoot, candidateSha, commandRunner = runCommand) {
  ...
  const verifiedV24History = `${ROLLOUT.verifiedV24MigrationCount}:${digest(
    "md5",
    filenames.slice(0, ROLLOUT.verifiedV24MigrationCount)
      .map((filename) => {
        const separator = filename.indexOf("_");
        return `${filename.slice(0, separator)}:${filename.slice(separator + 1, -4)}`;
      })
  ...
  return {
    ...
    canonicalV23TargetHistory: filenames.slice(84, ROLLOUT.canonicalV23MigrationCount)
      ...
      .join("|"),
    verifiedV24TargetHistory: filenames.slice(84, ROLLOUT.verifiedV24MigrationCount)
      .map((filename) => {
        const separator = filename.indexOf("_");
        return `${filename.slice(0, separator)}:${filename.slice(separator + 1, -4)}`;
      })
                    ...
                    : state === "staff-identity"
                      ...
                      : state === "restored-v22"
                        ...
                        : state === "restored-v23-pending-v24"
                          ? ROLLOUT.canonicalV23MigrationCount - ROLLOUT.baselineMigrationCount
                          : state === "verified-v24"
                            ? ROLLOUT.verifiedV24MigrationCount - ROLLOUT.baselineMigrationCount
                          : null;
  if (consumedMigrations === null) {
    throw new RolloutError("A migration packet can only be selected from pre, intermediate, recovery, convergence, attested, return-attested, retained, critical, column-attested, trial-locked, staff-identity, restored-v22, canonical-v23, restored-v23-pending-v24, or verified-v24 state.");
  }
...
export function classifyStateSnapshot(snapshot, packet, expectedProviderFingerprint = null) {
  ...
  if (history === packet.verifiedV24History) {
    if (targetHistory !== packet.verifiedV24TargetHistory || objectCounts !== "3:1") {
      throw new RolloutError("Migration history is verified-v24 but its exact V24 target history is missing.");
    }
    if (operationalReadiness !== EXPECTED_VERIFIED_V24_OPERATIONAL_READINESS) {
      throw new RolloutError("V24 operational readiness did not match the exact live pre-release state.");
    }
    return { state: "verified-v24", providerFingerprint: null };
  }
  if (history === packet.postHistory) {
    if (!packet.integrationComplete) {
      throw new RolloutError(
        "Candidate does not contain the exact final 119-migration sequence; post-state cannot be certified.",
      );
    }
    ...
    const providerFingerprint =
      `functions=${functionState};trigger=${triggerState};catalog=${catalogState};critical_surface=${criticalSurfaceManifest}`;
    if (
  ...
  throw new RolloutError(
    `Unexpected migration history ${history}; expected exact pre-, intermediate-, recovery-, convergence-, attested-, return-attested-, retained-, critical-, column-attested-, trial-locked-, staff-identity-, restored-v22-, canonical-v23-, restored-v23-pending-v24-, verified-v24-, or post-state.`,
  );
}
...
export function approvedProviderFingerprintVariants(stagingFingerprint) {
  ...
  const canonicalCatalog = `catalog=${EXPECTED_CATALOG_STATE}`;
  const expectedSuffix = `;critical_surface=${EXPECTED_CRITICAL_SURFACE_MANIFEST}`;
  if (
  ...
  try {
    ...
    if (
      ...
      (
        ...
        snapshot.history === packet.verifiedV24History ||
...
export async function main(
  ...
  try {
    ...
    if (config.mode !== "diagnose" && !packet.integrationComplete) {
      throw new RolloutError(
        "Provider inspection requires the exact final 119-migration candidate through 20260825043911.",
      );
    }
    commandRunner(
      "supabase",
    ...
    if (
      ...
      before.state !== "verified-v24"
    ) {
      throw new RolloutError(
        `${config.mode} requires an exact accepted pre-, intermediate-, recovery-, convergence-, attested-, return-attested-, retained, critical, column-attested, trial-locked, staff-identity, restored-v22, canonical-v23, restored-v23-pending-v24, or verified-v24 state.`,
      );
    }
