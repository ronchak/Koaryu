...
export const EXPECTED_OPERATIONAL_READINESS =
  "true|111|20260816012723|" +
  ROLLOUT.finalPendingVersions.join(",") +
  "|0||release-db-attestation-v18";
...
export const EXPECTED_CATALOG_STATE =
  ...
  "functions=68:164af3cd98d7f26bc74994b4f16529ea988ba0e760aa34d3cebddc4f97c4b625:0;" +
...
export function validateOperationalReadiness(value) {
  if (value !== EXPECTED_OPERATIONAL_READINESS) {
    throw new RolloutError("V18 operational readiness did not match the exact release state.");
  }
  return value;
}
...
export function validateCriticalSurfaceManifest(value) {
  if (value !== EXPECTED_CRITICAL_SURFACE_MANIFEST) {
    throw new RolloutError(
      "V17 critical-surface semantic manifest did not match the exact archive authorization state.",
    );
  }
  return value;
}
...
export function classifyStateSnapshot(snapshot, packet, expectedProviderFingerprint = null) {
  ...
  if (history === packet.postHistory) {
    if (!packet.integrationComplete) {
      throw new RolloutError(
        "Candidate does not contain the exact final 111-migration sequence; post-state cannot be certified.",
      );
    }
...
export async function main(
  ...
  try {
    ...
    if (config.mode !== "diagnose" && !packet.integrationComplete) {
      throw new RolloutError(
        "Provider inspection requires the exact final 111-migration candidate through 20260816012723.",
      );
    }
    commandRunner(
      "supabase",
