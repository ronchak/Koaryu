...
export const EXPECTED_OPERATIONAL_READINESS =
  "true|114|20260820060216|" +
  ROLLOUT.finalPendingVersions.join(",") +
  "|0||release-db-attestation-v21";
...
export function validateOperationalReadiness(value) {
  if (value !== EXPECTED_OPERATIONAL_READINESS) {
    throw new RolloutError("V21 operational readiness did not match the exact release state.");
  }
  return value;
}
...
export function classifyStateSnapshot(snapshot, packet, expectedProviderFingerprint = null) {
  ...
  if (history === packet.postHistory) {
    if (!packet.integrationComplete) {
      throw new RolloutError(
        "Candidate does not contain the exact final 114-migration sequence; post-state cannot be certified.",
      );
    }
...
export async function main(
  ...
  try {
    ...
    if (config.mode !== "diagnose" && !packet.integrationComplete) {
      throw new RolloutError(
        "Provider inspection requires the exact final 114-migration candidate through 20260820060216.",
      );
    }
    commandRunner(
      "supabase",
