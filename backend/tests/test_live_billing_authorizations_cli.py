...
class LiveBillingAuthorizationCliTest(unittest.TestCase):
    def test_grant_is_dry_run_by_default_and_requires_expiry(self):
        ...
        self.assertEqual(args.allowed_operations, ["connected_invoice.create"])
        self.assertEqual(
    ...
    def test_grant_dry_run_names_v30_writer_and_exact_operations(self):
        ...
        with (
            patch(
                "scripts.live_billing_authorizations._resolve_studio",
                return_value={"id": "studio_1", "slug": "test"},
            ),
            patch(
        ...
        plan = print_json.call_args.args[0]
        self.assertEqual(
            plan["write_boundary"],
            "set_studio_live_billing_authorization_operations_v1",
        )
        self.assertEqual(plan["allowed_operations"], args.allowed_operations)
