...
def test_pages_cover_large_timestamp_ties_without_truncation_or_tenant_leak(dataset):
    rows = [
        dict(
            id=str(UUID(int=i)),
            studio_id="studio",
            status="paid" if dataset == "invoices" else "succeeded",
            amount_cents=i,
    ...
    client = _PageClient(
        {f"billing_{dataset}": rows + [dict(rows[0], id=str(UUID(int=9999)), studio_id="other")]}
    )
    seen = []
    cursor = None
    ...
    for query in client.query_log[1:]:
        ...
        assert query["or_filters"][0].startswith(
            f"created_at.lt.{stamp},and(created_at.eq.{stamp},id.lt."
        )
...
def test_exact_payment_read_obeys_existing_billing_access_and_tenant_boundaries(
    role, entitled, expected
):
    own = dict(
        id="payment-1",
        ...
        refunded_amount_cents=1250,
        created_at="2026-09-01T00:00:00+00:00",
        updated_at="2026-09-08T00:00:00+00:00",
    )
    provider = _PageClient(
        {
            "staff_roles": [
    ...
    with patch(
        ...
        payment_queries = [
            query for query in provider.query_log if query["table"] == "billing_payments"
        ]
        if expected != 200:
        ...
        assert page.status_code == 200 and [row["id"] for row in page.json()["items"]] == [
            "payment-1"
        ]
        with patch.object(
            endpoints.BillingService,
            "current_month_payment_cohort_summary",
            new=AsyncMock(
                return_value={
