...
class Provider:
    def __init__(self, count, cap=500):
        self.rows = [{"id": f"{i:06}", "studio_id": "one", "created_at": f"{i:06}"} for i in range(count)]
        self.cap = cap
        self.calls = []
    ...
    def table(self, name):
        ...
        class Query:
            ...
            def select(self, _columns, **_options): return self
            ...
            def execute(self):
                ...
                return SimpleNamespace(count=len(provider.rows), data=[r for r in provider.rows if r["id"] > self.cursor][:provider.cap])
        return Query()
...
def test_oversized_collection_fails_before_unbounded_download():
    ...
    with pytest.raises(HTTPException) as error: fetch_lead_rows(client, "one")
