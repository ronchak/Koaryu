...
def run(app, *, budget=0.025, path="/api/v1/students/private-id"):
    ...
    async def invoke():
        scope = {
            ...
            "method": "GET",
            "path": path,
            "route": SimpleNamespace(path="/api/v1/students/{student_id}"),
        }
        try:
            await RequestDeadlineMiddleware(app, interactive_seconds=budget)(scope, receive, send)
        finally:
...
def test_total_deadline_covers_multiple_individually_fast_stages(caplog):
    ...
    with caplog.at_level("INFO", logger="uvicorn.error.request_timing"):
        asyncio.run(invoke())
...
def test_completed_body_allows_cleanup_and_preserves_existing_timing_headers():
        ...
        await send(
            {
                ...
                "status": 200,
                "headers": [(b"server-timing", b"koaryu_facts;dur=1")],
            }
        )
    ...
    assert any(
        value.startswith(b"koaryu_request;")
        for key, value in messages[0]["headers"]
        if key == b"server-timing"
    )
...
@pytest.mark.parametrize(
    "path,method,bulk",
    [
        ("/students/import/execute", "POST", True),
        ("/students/bulk/archive", "POST", True),
        ("/reports/exports/attendance", "GET", True),
        ("/schedule/window/materialize", "POST", True),
        ("/schedule/window", "GET", False),
        ("/billing/landing", "GET", False),
    ],
)
