...
class LandingRuntimeFixture:
    def __init__(
        ...
        *,
        ...
        self.runtime = SupabaseProviderRuntime(
            config,
            config,
            client_factory=Resource,
            client_closer=lambda client: client.touch("closed"),
        ...
        monkeypatch.setattr(
            billing_endpoint, "resolve_billing_manager_staff_role_for_user", membership
        )
    ...
    async def call(self):
        response = Response()
        result = await billing_endpoint.get_billing_landing(
            response, "user", "studio", self.runtime
        )
    ...
    async def close(self):
        ...
        created = {
            (client, thread) for operation, client, thread in self.events if operation == "created"
        }
        closed = {
            (client, thread) for operation, client, thread in self.events if operation == "closed"
        }
...
def test_stalled_diagnostics_preserve_healthy_fields_affinity_and_capacity(
    monkeypatch, role, required
):
    async def scenario():
        ...
        try:
            ...
            assert not fixture.release.is_set(), (
                "response must not depend on the stalled provider returning"
            )
            snapshot = fixture.runtime.interactive_snapshot()
            assert (snapshot.admitted, snapshot.active) == (1, 1), (
                "timed-out work still owns its capacity"
            )
            assert snapshot.submitted == (4 if role == "admin" else 3), (
                "one membership read and role-allowed projections only"
            )
            assert snapshot.cancelled >= 1
            assert snapshot.timed_out == 1, (
                "only the stalled projection exhausts its owned deadline"
            )
            assert ticks >= 5, "blocking provider work must not block the ASGI loop"
            owners = {
                operation: client
                for operation, client, _ in fixture.events
                if operation in {"financial", "platform", "diagnostics"}
            }
            ...
            assert (
                await fixture.runtime.run_interactive(
                    lambda client: client.touch("unrelated-request") or "healthy"
                )
                == "healthy"
            )
        finally:
    ...
    async def scenario():
        ...
        try:
            ...
            assert result.system_status is not None, (
                "no new short cutoff for previously supported slow reads"
            )
    ...
    async def scenario():
        ...
        try:
            ...
            assert fixture.runtime.interactive_snapshot().timed_out == 0, (
                "caller cancellation is not a deadline"
            )
            assert not fixture.release.is_set()
        finally:
            await fixture.close()
...
def test_membership_denial_and_unrelated_timeout_start_no_projections(monkeypatch):
    for failure in (
        HTTPException(403, "not a billing manager"),
        TimeoutError("unrelated application failure"),
    ):
        ...
        with pytest.raises(type(failure)) as raised:
            asyncio.run(
                billing_endpoint.get_billing_landing(Response(), "user", "studio", object())
            )
...
def test_preexpired_projection_deadlines_count_without_submitting_work(
    monkeypatch, role, projection_count
):
    async def scenario():
    ...
    async def scenario():
        ...
        try:
            if failure_scope == "membership":
                monkeypatch.setattr(
                    billing_endpoint, "resolve_billing_manager_staff_role_for_user", fail
                )
