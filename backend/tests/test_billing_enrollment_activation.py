...
def test_new_non_usd_activation_leaves_local_and_provider_state_unchanged(mode):
    ...
    with pytest.raises(HTTPException) as failure:
        asyncio.run(
            _manager(facade).activate_enrollment("enrollment_1", "studio_1", "actor_1", "new-eur")
        )
    ...
    assert not (
        _Stripe.create_subscription_calls or _Stripe.add_item_calls or _Stripe.update_item_calls
    )


@pytest.mark.parametrize(
    "stage,error", [("unattempted", 400), ("attempted", 409), ("confirmed", None)]
)
def test_historical_non_usd_activation_uses_saved_provider_outcome(stage, error):
    intent = {
        "version": 1,
        "operation_type": "enrollment.activate.invoice",
        "studio_id": "studio_1",
    ...
    tables = _tables(
        group=_group(currency="eur"),
        enrollment=_enrollment(metadata={"provider_activation_intent": intent}),
    )
    tables["billing_plans"] = [_plan(currency="eur")]
    tables["billing_plan_prices"] = [_price(currency="eur")]
    facade = _Facade(tables)
    ...
    if stage == "confirmed":
        operations.transition(
            ...
            operation,
            "provider_succeeded",
            provider_object_id="sub_1",
            provider_secondary_object_id="si_historical",
    ...
    if error:
        with pytest.raises(HTTPException) as failure:
            asyncio.run(
                _manager(facade).activate_enrollment(
                    "enrollment_1", "studio_1", "actor_1", "historical"
                )
            )
    ...
    assert _operation(facade)["provider_request_attempt_count"] == (
        0 if stage == "unattempted" else 1
    )
    assert _operation(facade)["caller_request_key"] == "historical"
    assert _operation(facade)["request_sha256"] == intent["desired_sha256"]
    assert all(
        facade.supabase.tables[name][0]["currency"] == "eur"
        for name in ("billing_plans", "billing_plan_prices", "billing_subscriptions")
    )
    assert not (
        _Stripe.create_subscription_calls or _Stripe.add_item_calls or _Stripe.update_item_calls
    )
...
def test_activation_branches_preserve_exact_owner_replay_and_release_lock(
    branch, subscription_id, item_id, key_suffix
):
    facade = (
        _Facade(_tables())
    ...
    assert (
        ...
        == replay.stripe_subscription_id
        == subscription_id
    )
    assert first.stripe_subscription_item_id == replay.stripe_subscription_item_id == item_id
    calls = {
        "create_subscription": _Stripe.create_subscription_calls,
        "add_item": _Stripe.add_item_calls,
        "update_quantity": _Stripe.update_item_calls,
    }
    assert len(calls[branch]) == sum(map(len, calls.values())) == 1
    assert len(facade.supabase.tables["audit_logs"]) == 1
    parent = _operation(facade)
    assert (parent["state"], parent["actor_id"], parent["caller_request_key"]) == (
        "completed",
        "actor_1",
        "activation-owner",
    )
    assert (
        facade.supabase.billing_provider_operation_aliases[
    ...
    expected = {
        "create_subscription": (None, None, 1),
    ...
    assert (
        ...
        == f"koaryu:enrollment-activate:{parent['id']}:{key_suffix}"
    )
    if branch == "update_quantity":
    ...
    assert (claim["p_resource_type"], claim["p_resource_id"], claim["p_payer_id"]) == (
        "enrollment",
        "enrollment_1",
        "payer_1",
    )
    names = [name for name, _ in facade.supabase.rpc_calls]
    assert names.index("claim_billing_subscription_quantity_sync") < names.index(
...
def test_provider_success_local_failure_reconciles_and_readback_never_mutates_again():
    ...
    with pytest.raises(HTTPException) as exc:
        asyncio.run(
            manager.activate_enrollment("enrollment_1", "studio_1", "actor_1", "projection-key")
        )
...
def test_balance_failure_stays_projected_and_same_actor_replay_completes():
    ...
    with pytest.raises(HTTPException) as failed_balance:
        asyncio.run(
            manager.activate_enrollment("enrollment_1", "studio_1", "actor_1", "balance-key")
        )
    parent = _operation(facade)
    ...
    replay = asyncio.run(
        manager.activate_enrollment("enrollment_1", "studio_1", "actor_1", "balance-adopter")
    )
...
def test_lost_policy_cleanup_response_same_key_never_recreates_group():
    ...
    assert (
        facade.supabase.tables["billing_subscriptions"][0]["stripe_subscription_id"]
        == "sub_created"
    )
...
def test_lost_provider_success_response_replays_without_second_mutation():
    ...
    with pytest.raises(HTTPException) as lost:
        asyncio.run(manager.activate_enrollment("enrollment_1", "studio_1", "actor_1", "lost-key"))
    assert lost.value.status_code == 503
    assert _operation(facade)["state"] == "provider_succeeded"

    result = asyncio.run(
        manager.activate_enrollment("enrollment_1", "studio_1", "actor_1", "lost-adopter")
    )
    assert result.status == "active"
    assert len(_Stripe.create_subscription_calls) == 1
    assert len(facade.supabase.tables["audit_logs"]) == 1
...
def test_contradictory_active_price_and_provider_readback_mismatch_fail_closed():
    ...
    try:
        with pytest.raises(HTTPException) as mismatch:
            asyncio.run(
                manager.activate_enrollment("enrollment_1", "studio_1", "actor_1", "readback-key")
            )
