...
class BillingPaymentIntentLifecycleTest(BillingPaymentsLifecycleTestBase):
    def payment_facts_fixture(self):
        ...
        service.supabase = _FakeSupabase({
            "studio_payment_accounts": [{
                ...
                "charges_enabled": True, "requirements_due": [],
                "updated_at": datetime.now(timezone.utc).isoformat(),
            }],
            "billing_payers": [{
                "id": "payer_1", "studio_id": "studio_1", "stripe_account_id": "acct_1",
                "stripe_customer_id": "cus_1", "balance_cents": 0,
            }],
            "billing_payments": [], "billing_invoices": [], "billing_refunds": [],
            "billing_disputes": [], "audit_logs": [],
        })
        intent = {
            "id": "pi_1", "status": "succeeded", "amount": 1000, "amount_received": 1000,
            "created": int(datetime(2026, 1, 1, tzinfo=timezone.utc).timestamp()),
            "currency": "usd", "customer": "cus_1", "latest_charge": "ch_1",
    ...
    def test_success_sources_keep_one_payment_in_its_original_utc_month(self):
        ...
        for first_source in ("payment_intent", "invoice"):
            with self.subTest(first_source=first_source):
                ...
                invoice = {
                    "id": "in_1", "payment_intent": "pi_1", "amount_paid": 1000,
                    "currency": "usd", "customer": "cus_1", "metadata": intent["metadata"],
                    "status_transitions": {"paid_at": int(collected.timestamp())},
                }
                local_invoice = {"stripe_account_id": "acct_1", "paid_at": collected.isoformat()}
                with patch("app.services.billing_payment_projection.datetime", wraps=datetime) as clock, patch(
                    ...
                    if first_source == "invoice":
                        service._project_payment_from_invoice(invoice, "acct_1", local_invoice, event_created=int(observed.timestamp()))
                    else:
                        service._project_payment_intent(intent, "acct_1", "payment_intent.succeeded", int(collected.timestamp()))
                    service._project_payment_from_invoice(invoice, "acct_1", local_invoice, event_created=int(observed.timestamp()))
                    service._project_payment_intent(intent, "acct_1", "payment_intent.succeeded", int(observed.timestamp()))
                ...
                june = asyncio.run(manager.current_month_payment_cohort_summary("studio_1", as_of=collected))
                july = asyncio.run(manager.current_month_payment_cohort_summary("studio_1", as_of=observed))
        ...
        for winner_event in (150, 300):
            with self.subTest(winner_event=winner_event):
                ...
                def concurrent_success(rows):
                    rows[0].update({
                        "processed_at": winner_time, "last_stripe_event_created": winner_event,
                        "status": "succeeded", "amount_cents": 1000,
                        "net_collected_amount_cents": 1000, "refundable_amount_cents": 1000,
                    })
                ...
                updates = [query for query in service.supabase.query_log if query["table"] == "billing_payments" and (query["update"] or {}).get("last_stripe_event_created") == 200]
    ...
    def test_broad_reconciliation_refuses_uncaptured_authorization(self):
        ...
        with patch("app.services.billing_service.StripeService") as provider:
            ...
            with self.assertRaises(HTTPException) as raised:
                asyncio.run(service.reconcile_stripe_object(
                    BillingReconcileRequest(object_type="payment_intent", stripe_object_id="pi_1"),
                    "studio_1", "user_1",
                ))
        self.assertEqual(raised.exception.status_code, 409)
        self.assertIn("captur", raised.exception.detail)
        self.assertEqual(provider.return_value.method_calls, [call.retrieve_account(account_id="acct_1"), call.retrieve_connected_payment_intent(
            account_id="acct_1", payment_intent_id="pi_1", expand=["latest_charge", "payment_method"],
        )])
    ...
    def test_success_without_provider_time_keeps_its_first_observation(self):
        ...
        with patch("app.services.billing_payment_projection.datetime", wraps=datetime) as clock:
            ...
            service._project_payment_intent(intent, "acct_1", "payment_intent.succeeded")
        self.assertEqual(service.supabase.tables["billing_payments"][0]["processed_at"], first_seen.isoformat())
    ...
    def test_explicit_zero_received_cannot_pay_a_positive_invoice(self):
        ...
        invoice = {
            ...
            "stripe_account_id": "acct_1", "stripe_invoice_id": "in_1",
            "status": "open", "amount_due_cents": 1000, "amount_paid_cents": 0,
            "amount_remaining_cents": 1000, "currency": "usd",
        }
                ...
                with self.subTest(terminal=terminal, incoming=incoming):
                    ...
                    if terminal == "refunded":
                        ...
                        service._project_dispute({
                            "id": "dp_1", "charge": "ch_1", "amount": 1000,
                            "currency": "usd", "status": "needs_response",
                            "metadata": {"studio_id": "studio_1"},
                        }, "acct_1")
                    payment = service.supabase.tables["billing_payments"][0]
                    expected = {key: payment[key] for key in (
                        "id", "status", "processed_at", "amount_cents", "refunded_amount_cents",
                        "disputed_amount_cents", "net_collected_amount_cents", "refundable_amount_cents",
                    )}
                    self.assertEqual(expected["status"], terminal)
                    self.assertEqual(expected["amount_cents"], 1000)
                    self.assertEqual(expected["net_collected_amount_cents"], 1000 if terminal == "succeeded" else 0)
                    service._project_payment_intent(
                        {**intent, "amount_received": 0, "status": incoming},
                        "acct_1", f"payment_intent.{incoming}", 200,
                    )
                    actual = service.supabase.tables["billing_payments"][0]
                    self.assertEqual({key: actual[key] for key in expected}, expected)
                    self.assertEqual(actual["last_stripe_event_created"], 200, "the newer observation must reach projection rather than pass by stale-event rejection")
    ...
    def test_invoice_payment_intent_retrieval_failure_projects_nonrefundable_fallback(self):
        ...
        with patch("app.services.billing_service.StripeService") as provider:
            provider.return_value.retrieve_connected_payment_intent.side_effect = RuntimeError("transient retrieval failure")
