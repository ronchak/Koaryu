...
def manifest():
    local = {
        owner: {role: f"{owner}_{index}" for index, role in enumerate(sorted(roles), 1)}
        for owner, roles in C.LOCAL_ROLE_SCHEMA.items()
    }
    for first, second in (
        ("plan_product_create", "plan_price_create"),
        ("invoice_link_invoice_create", "invoice_link_item_create"),
        ("invoice_link_finalize", "invoice_link_send"),
        ("automatic_invoice_create", "automatic_item_create"),
        ("enrollment_subscription_create", "enrollment_shared_quantity_update"),
    ...
    local["webhook_events"] = {
        "connect_checkout": "evt_connect",
        "dispute_created": "evt_disputecreated",
        "dispute_closed": "evt_disputeclosed",
        "platform_subscription": "evt_platform",
    }
                    ...
                    "entries": [
                        {
                            ...
                            "operation": operation,
                            "provider_created_at": f"2026-08-29T09:0{index * 2}:30Z",
                            "method": "POST",
                            ...
                            "request_facts": (
                                {"studio_id": "studio_1", "connect_account_generation": 1}
                                if account
                                else {"account_id": "acct_Test1", "studio_id": "studio_1"}
                            ),
                            "response_facts": (
                                {
                                    "object": "account",
                                    "account_id": "acct_Test1",
                                    "metadata_studio_id": "studio_1",
                                }
                                if account
                                else {
                                    "object": "account_link",
    ...
    provider_ids = {
        "account": "acct_Test1",
        "payer_customer": "cus_payer",
        "initial_checkout": "cs_initial",
        "initial_setup_intent": "seti_initial",
        "replacement_checkout": "cs_replacement",
        "replacement_setup_intent": "seti_replacement",
        "product": "prod_1",
        "price": "price_1",
        "shared_subscription": "sub_shared",
        "shared_subscription_item": "si_shared",
        "invoice_link": "in_link",
        "automatic_invoice": "in_auto",
        "automatic_payment_intent": "pi_auto",
        "automatic_charge": "ch_auto",
        "revoke_schedule": "sub_sched_revoke",
        "due_schedule": "sub_sched_due",
        "refund": "re_1",
        "dispute": "dp_1",
        "test_clock": "clock_1",
        "platform_customer": "cus_platform",
        "platform_subscription": "sub_platform",
        "connect_checkout_event": "evt_connect",
        "dispute_created_event": "evt_disputecreated",
        "dispute_closed_event": "evt_disputeclosed",
        "platform_subscription_event": "evt_platform",
    }
    return C.validate_manifest(
        {
            "manifest_schema_version": 1,
            "candidate_sha": "a" * 40,
            "readiness_origin": "https://staging.example.invalid",
            "studio_id": "studio_1",
            "stripe_account_id": "acct_Test1",
            "connect_account_generation": 1,
            "rehearsal_started_at": "2026-08-29T10:00:00Z",
            "local_ids": local,
            "provider_objects": {
                role: {
                    "id": provider_ids[role],
                    "kind": kind,
                    "context": context,
                    "phase": list(C.PHASES),
                }
                for role, (kind, context) in C.PROVIDER_ROLE_SCHEMA.items()
            },
            ...
            "workbench_delivery_attempts": [
                {
                    ...
                    "role": "original",
                    "surface": "connect",
                    "event_id": "evt_connect",
                    "event_type": "checkout.session.completed",
                    "checkout_session_id": "cs_replacement",
                    "endpoint_url": "https://staging.example.invalid/api/v1/webhooks/stripe/connect",
                    "delivery_status": "delivered",
                    "http_status": 200,
                    "delivered_at": "2026-08-29T10:02:00Z",
                },
                {
                    ...
                    "delivery_status": "delivered",
                    "http_status": 200,
                    "delivered_at": "2026-08-29T10:03:00Z",
                },
                {
                    ...
                    "endpoint_url": "https://staging.example.invalid/api/v1/webhooks/stripe/platform",
                    "delivery_status": "delivered",
                    "http_status": 200,
                    "delivered_at": "2026-08-29T10:04:00Z",
                },
            ],
        }
    )
...
def chain():
    ...
    rows = [
        {
            "owner": owner,
            **(
                {"stripe_event_id": identifier}
                if owner == "webhook_events"
                else (
                    {"studio_id": identifier}
                    if owner == "platform_core_rows"
                    else {"id": identifier}
                )
            ),
            "studio_id": "studio_1",
        ...
        for owner, roles in m["local_ids"].items()
        for identifier in dict.fromkeys(roles.values())
    ]
    for row in rows:
    ...
    for step_name, source_role in C.MUTATION_SOURCE_ROLES.items():
        row = next(
            row
            for row in rows
            if row["owner"] == "operations"
            and row["id"] == m["local_ids"]["operations"][source_role]
        )
        workflow, operation, scope, actor_role, _ = validator.REQUIRED_MUTATIONS[step_name]
        row.update(
            operation_type=C.PARENT_OPERATION_TYPES.get(workflow, workflow),
            state="completed",
            stripe_connected_account_id="acct_Test1",
            connect_account_generation=1,
            actor_id="actor_internal"
            if actor_role == "internal"
            else ("actor_2" if actor_role == "front_desk" else "actor_1"),
            request_sha256="c" * 64,
            caller_request_key=f"caller-{row['id']}",
            caller_request_key_sha256="d" * 64,
            provider_request_attempt_count=1,
            completed_at="2026-08-29T10:01:00Z",
        )
        if step_name == "payer.customer_create":
            row.update(
                recovery_outcome="provider_succeeded_reconcile_only", provider_object_id="cus_payer"
            )
        else:
            step = next(
                item
                for item in rows
                if item["owner"] == "steps" and item["id"] == m["local_ids"]["steps"][source_role]
            )
            step.update(
                operation_id=row["id"],
                ...
                connect_account_generation=1,
                request_sha256="e" * 64,
                stripe_idempotency_key=f"step-{step['id']}",
                caller_request_key_sha256="f" * 64,
            )
    due = next(
        row
        for row in rows
        if row["owner"] == "transitions" and row["id"] == m["local_ids"]["transitions"]["due"]
    )
    due.update(
        provider_operation_id=m["local_ids"]["operations"]["period_end_due_release"],
        transition_kind="execute_due",
        initiated_by="actor_internal",
    )
    ...
    for phase_name, pm in (("initial", "pm_initial"), ("replacement", "pm_replacement")):
        request = next(
            row
            for row in rows
            if row["owner"] == "setup_requests"
            and row["id"] == m["local_ids"]["setup_requests"][phase_name]
        )
        consent = next(
            row
            for row in rows
            if row["owner"] == "consents" and row["id"] == m["local_ids"]["consents"][phase_name]
        )
        request.update(
            stripe_checkout_session_id=m["provider_objects"][f"{phase_name}_checkout"]["id"]
        )
        consent.update(
            payer_id=payer["id"],
            stripe_connected_account_id="acct_Test1",
            ...
            stripe_checkout_session_id=m["provider_objects"][f"{phase_name}_checkout"]["id"],
            stripe_setup_intent_id=m["provider_objects"][f"{phase_name}_setup_intent"]["id"],
            terms_version=f"terms-{phase_name}",
    ...
    for number, role in enumerate(("student_one", "student_two"), 1):
        enrollment = next(
            row
            for row in rows
            if row["owner"] == "subscriptions"
            and row["id"] == m["local_ids"]["subscriptions"][role]
        )
        enrollment.update(
            student_id=f"student_{number}",
            payer_id=payer["id"],
            status="active",
    ...
    invoice_link = next(
        row
        for row in rows
        if row["owner"] == "invoices" and row["id"] == m["local_ids"]["invoices"]["invoice_link"]
    )
    invoice_link.update(
        stripe_invoice_id="in_link",
        finalized_at="2026-08-29T10:03:00Z",
        sent_at="2026-08-29T10:04:00Z",
    )
    automatic = next(
        row
        for row in rows
    ...
    payment = next(
        row
        for row in rows
        if row["owner"] == "payments" and row["id"] == m["local_ids"]["payments"]["automatic"]
    )
    payment.update(
        stripe_payment_intent_id="pi_auto",
        stripe_charge_id="ch_auto",
        amount_cents=10000,
        application_fee_cents=50,
        gross_paid_cents=10000,
        refunded_cents=1000,
        disputed_cents=0,
        net_collected_cents=9000,
        refundable_remaining_cents=9000,
        application_fee_amount_cents=50,
        gross_paid_amount_cents=10000,
        refunded_amount_cents=1000,
        disputed_amount_cents=0,
        net_collected_amount_cents=9000,
        refundable_amount_cents=9000,
    ...
    for role, state, schedule_id in (
        ("schedule", "scheduled", None),
        ("revoke", "revoked", "sub_sched_revoke"),
        ("due", "completed", "sub_sched_due"),
    ):
        transition = next(
            row
            for row in rows
            if row["owner"] == "transitions" and row["id"] == m["local_ids"]["transitions"][role]
        )
        transition.update(
            state=state,
    ...
    next(
        ...
        for row in rows
        if row["owner"] == "operations"
        and row["id"] == m["local_ids"]["operations"]["period_end_revoke_schedule_create"]
    )["provider_object_id"] = "sub_sched_revoke"
    next(
        row
        for row in rows
        if row["owner"] == "operations"
        and row["id"] == m["local_ids"]["operations"]["period_end_due_schedule_create"]
    )["provider_object_id"] = "sub_sched_due"
    ambiguity = next(
        row
        for row in rows
        if row["owner"] == "operations"
        and row["id"] == m["local_ids"]["operations"]["ambiguity_parent"]
    )
    ambiguity.update(
        actor_id="actor_1",
        state="completed",
        ...
        provider_request_attempt_count=1,
        provider_object_id="cus_payer",
        completed_at="2026-08-29T10:01:00Z",
        caller_request_key="ambiguity-key",
        caller_request_key_sha256="e" * 64,
    )
    resource = next(row for row in rows if row["owner"] == "resources")
    resource.update(
        operation_id=ambiguity["id"],
        resource_type="payer",
    ...
    void_op = next(
        ...
        for row in rows
        if row["owner"] == "operations"
        and row["id"] == m["local_ids"]["operations"]["invoice_void"]
    )
    void_op.update(
        actor_id="actor_1",
        state="completed",
        ...
        stripe_connected_account_id="acct_Test1",
        connect_account_generation=1,
        provider_object_id="in_link",
        provider_request_attempt_count=1,
        caller_request_key="void-key",
        caller_request_key_sha256="f" * 64,
    )
    immediate_op = next(
        row
        for row in rows
        if row["owner"] == "operations"
        and row["id"] == m["local_ids"]["operations"]["immediate_cancellation"]
    )
    immediate_op.update(
        actor_id="actor_1",
        state="completed",
        operation_type="enrollment.cancel.immediate",
        stripe_connected_account_id="acct_Test1",
        connect_account_generation=1,
        provider_object_id="sub_shared",
        provider_request_attempt_count=1,
        caller_request_key="immediate-key",
        caller_request_key_sha256="1" * 64,
    )
    immediate_transition = next(
        row
        for row in rows
        if row["owner"] == "transitions" and row["id"] == m["local_ids"]["transitions"]["immediate"]
    )
    immediate_transition.update(
        state="completed",
        transition_kind="immediate_cancel",
        provider_operation_id=immediate_op["id"],
        enrollment_id=m["local_ids"]["subscriptions"]["student_two"],
    )
    external = next(
        row
        for row in rows
        if row["owner"] == "payments"
        and row["id"] == m["local_ids"]["payments"]["payments.external"]
    )
    external.update(
        ...
        request_hash="3" * 64,
        caller_request_key_sha256="2" * 64,
        invoice_id=None,
    )
    rows.append(
        {
            ...
            "metadata": {"amount_cents": 2500, "external_method": "cash"},
            "created_at": "2026-08-29T10:01:00Z",
        }
    )
    for role, event_id, event_type, sequence in (
        ("dispute_created", "evt_disputecreated", "charge.dispute.created", 3),
        ("dispute_closed", "evt_disputeclosed", "charge.dispute.closed", 4),
    ):
        event = next(
            row
            for row in rows
            if row["owner"] == "webhook_events"
            and row["stripe_event_id"] == m["local_ids"]["webhook_events"][role]
        )
        event.update(
    ...
    for role, event_id, event_type, sequence in (
        ("connect_checkout", "evt_connect", "checkout.session.completed", 1),
        ("platform_subscription", "evt_platform", "customer.subscription.created", 5),
    ):
        event = next(
            row
            for row in rows
        ...
        event.update(
            ...
            processing_status="processed",
            livemode=False,
            stripe_account_id=None if role == "platform_subscription" else "acct_Test1",
            live_billing_ingest_sequence=sequence,
            created_at="2026-08-29T10:00:00Z",
        )
    core = next(row for row in rows if row["owner"] == "platform_core_rows")
    core.update(
        stripe_customer_id="cus_platform", stripe_subscription_id="sub_platform", status="active"
    )
    result = []
    for index, phase in enumerate(C.PHASES):
        ...
        provider = [
            ...
            for role, spec in m["provider_objects"].items()
        ]
        ...
        by_role["replacement_setup_intent"].update(
            payment_method="pm_replacement", status="succeeded"
        )
        ...
        by_role["platform_subscription"].update(
        ...
        if phase == "failed_before_retry":
            ...
            by_role["automatic_payment_intent"].update(
                status="requires_payment_method", last_payment_error_present=True
            )
        else:
            by_role["automatic_invoice"]["status"] = "paid"
            by_role["automatic_payment_intent"].update(
                status="succeeded", payment_method="pm_replacement"
            )
        ...
        for row in phase_rows:
            ...
            if (
                phase in {"final_local_1", "final_local_2"}
                and row["owner"] == "invoices"
                and row["id"] == invoice_link["id"]
            ):
                row["status"] = "void"
            if (
                phase in {"final_local_1", "final_local_2"}
                and row["owner"] == "subscriptions"
                and row["id"] == m["local_ids"]["subscriptions"]["student_two"]
            ):
                row["status"] = "canceled"
        if phase in {"final_provider_1", "final_provider_2"}:
            ...
            C.artifact(
                {
                    ...
                    "connect_account_generation": 1,
                    "observed_at": f"2026-08-29T10:0{index}:00Z",
                    "readiness": copy.deepcopy(READINESS),
                    "manifest_ids": {
                        "local": sorted(
                            v for values in m["local_ids"].values() for v in values.values()
                        ),
                        "provider": sorted(spec["id"] for spec in m["provider_objects"].values()),
                    },
                    "local_rows": phase_rows,
                    "provider_objects": sorted(provider, key=lambda row: row["role"]),
                }
            )
        )
...
def rpc_envelope(m, local_rows, boundary):
    ...
    return {
        ...
        "studio_id": m["studio_id"],
        "stripe_account_id": m["stripe_account_id"],
        "connect_account_generation": m["connect_account_generation"],
        "rehearsal_started_at": m["rehearsal_started_at"],
        "event_window_ended_at": boundary,
        "local_id_bindings": copy.deepcopy(m["local_ids"]),
        "local_rows": rows,
    }
    ...
    for index, phase in enumerate(C.PHASES):
        ...
        stripe = FakeStripe(
            {
                row["id"]: {
                    ...
                    if key not in {"role", "kind", "context"}
                }
        ...
        captures.append(
            C.capture_phase(
                ...
                phase,
                readiness=READINESS,
                local_reader=lambda observed, client=supabase: C.collect_local(
                    client, m, event_window_ended_at=observed
                ),
                provider_reader=lambda current, client=stripe: C.collect_provider(
                    client, m, current
                ),
                now=lambda boundary=boundary: boundary,
            )
        )
        supabase_clients.append(supabase)
        stripe_clients.append(stripe)
...
class CollectorArtifactTest(unittest.TestCase):
    def test_manifest_requires_exact_local_and_provider_roles(self):
        ...
        for owner in C.LOCAL_ROLE_SCHEMA:
            with self.subTest(owner=owner, case="missing"):
                ...
                with self.assertRaisesRegex(C.CollectorError, owner):
        ...
        for case in ("missing", "extra"):
            with self.subTest(provider=case):
                value = copy.deepcopy(valid)
                if case == "missing":
                    value["provider_objects"].pop("account")
                else:
                    value["provider_objects"]["arbitrary"] = {
                        "id": "obj_extra",
                        "kind": "customer",
                        "context": "connected",
                        "phase": list(C.PHASES),
    ...
    def test_projects_identity_steps_mutations_and_capabilities(self):
        ...
        self.assertEqual(
            [row["name"] for row in projected["steps"]], list(validator.REQUIRED_STEP_ORDER)
        )
        self.assertEqual(
            [row["step_name"] for row in projected["mutation_attempts"]],
            list(validator.REQUIRED_MUTATIONS),
        )
    ...
    def test_projects_retry_period_accounting_and_ambiguity(self):
        ...
        index = C._source_index(m, phases[4]["local_rows"], phases[5]["provider_objects"])
        validator, _ = C._load_contract()
        facts, supplemental = C._project_group_three(
            m, phases, index, phases[5]["observed_at"], validator
        )
        ...
        self.assertEqual(
            supplemental["failed_payment_retry"]["failed_provider_readback"]["invoice_status"],
            "open",
        )
            ...
            with self.subTest(label=label):
                ...
                for phase_index in (2, 4):
                    for row in artifacts[phase_index]["local_rows"]:
                        if row["owner"] == owner:
                            row[field] = value
                    artifacts[phase_index] = C.artifact(
                        {key: artifacts[phase_index][key] for key in C.ARTIFACT_BODY_KEYS}
                    )
    ...
    def test_projects_replay_deliveries_and_platform_fixture(self):
        ...
        final = C._source_index(m, phases[4]["local_rows"], phases[5]["provider_objects"])
        validator, _ = C._load_contract()
        replay, deliveries, platform = C._project_group_four_deliveries(
            m, final, phases[5]["observed_at"], validator
        )
        self.assertEqual(
            [row["role"] for row in replay["provider_replay"]["attempts"]],
            ["original", "manual_resend"],
        )
    ...
    def test_projects_void_immediate_external_and_unsupported(self):
        ...
        final = C._source_index(m, phases[4]["local_rows"], phases[5]["provider_objects"])
        validator, _ = C._load_contract()
        rows = C._project_group_four_local(
            m, final, phases[4]["local_rows"], phases[5]["observed_at"], validator
        )
        self.assertEqual(rows["invoice_void"]["provider_readback"]["status"], "void")
        self.assertEqual(
            rows["immediate_cancellation"]["local_readback"]["enrollment_status"], "canceled"
        )
        self.assertEqual(rows["external_payment"]["audit_count"], 1)
        self.assertEqual(
            rows["external_payment"]["provider_operation_inventory_readback"][
                "matching_provider_operation_count"
            ],
            0,
        )
        self.assertEqual(
            rows["external_payment"]["local_readback"]["audit_entity_id"],
            rows["external_payment"]["local_payment_id"],
        )
    ...
    def test_external_payment_projection_rejects_mixed_captured_sources(self):
        cases = {
            ...
            "mixed-account": lambda payment, audit, rows: payment.update(
                stripe_account_id="acct_Other1"
            ),
            "mixed-generation": lambda payment, audit, rows: payment.update(
                connect_account_generation=2
            ),
            "mixed-audit": lambda payment, audit, rows: audit.update(entity_id="payment_other"),
            "provider-operation": lambda payment, audit, rows: next(
                row for row in rows if row.get("owner") == "operations"
            ).update(provider_object_id=payment["id"]),
        }
    ...
    def test_assembles_validator_clean_packet_without_template(self):
        ...
        self.assertTrue(
            all(row["count"] == 0 for row in packet["terminal_counts"]["counts"].values())
        )
    ...
    def test_rejects_each_nonzero_terminal_count(self):
        def local(owner, **changes):
            def mutate(m, values):
                for phase in (2, 4):
                    ...
                    row.update(changes)
                    values[phase] = C.artifact(
                        {key: values[phase][key] for key in C.ARTIFACT_BODY_KEYS}
                    )
        ...
        cases = {
            ...
            "stuck": local(
                ...
                lease_expires_at="2026-08-29T09:00:00Z",
            ),
        ...
        for name, mutate in cases.items():
            with self.subTest(name=name):
                ...
                phases = C.validate_phase_chain(m, values)
                validator, _ = C._load_contract()
                counts = C._terminal_counts(
                    m,
                    phases[4]["local_rows"],
                    phases[5]["provider_objects"],
                    phases[5]["observed_at"],
                    validator,
                )
                self.assertGreater(counts["counts"][name]["count"], 0)
                with self.assertRaisesRegex(
                    C.CollectorError, f"terminal count {name} must be zero"
                ):
                    C.assemble(m, values)

    def test_terminal_count_compares_lease_instants_not_strings(self):
        m, values = assembled_chain()
        row = next(row for row in values[4]["local_rows"] if row["owner"] == "operations")
        row.update(state="provider_request_in_flight", lease_expires_at="2026-08-29T03:05:01-07:00")
        validator, _ = C._load_contract()
        counts = C._terminal_counts(
            m,
            values[4]["local_rows"],
            values[5]["provider_objects"],
            values[5]["observed_at"],
            validator,
        )
    ...
    def test_cli_sanitizes_unexpected_adapter_failures(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                mock.patch.object(
                    C, "assemble", side_effect=RuntimeError("PRIVATE_VALUE raw payload")
                ),
                contextlib.redirect_stderr(stderr),
            ):
                self.assertEqual(
                    C.main(["--manifest", str(manifest_path), "--assemble", *paths]), 1
                )
    ...
    def test_live_adapters_capture_and_assemble_without_mutations(self):
        ...
        validator, _ = C._load_contract()
        self.assertEqual(
            validator.validate_evidence(packet, m["candidate_sha"], m["readiness_origin"]), []
        )
        self.assertTrue(
            all(row["count"] == 0 for row in packet["terminal_counts"]["counts"].values())
        )
        ...
        self.assertEqual(
            packet["supplemental_evidence"]["ambiguity_recovery"]["provider_mutation_count"], 1
        )
        for index, client in enumerate(supabase_clients):
            ...
            self.assertEqual(
                params,
                {
                    ...
                    "p_stripe_account_id": "acct_Test1",
                    "p_connect_account_generation": 1,
                    "p_rehearsal_started_at": "2026-08-29T10:00:00Z",
                    "p_event_window_ended_at": f"2026-08-29T10:0{index}:00Z",
                    "p_local_ids": m["local_ids"],
                    "p_actor_ids": ["actor_1", "actor_2"],
                    "p_external_audit_id": "audit_1",
                    "p_connect_event_ids": [
                        "evt_connect",
                        "evt_disputeclosed",
                        "evt_disputecreated",
                    ],
                    "p_platform_event_ids": ["evt_platform"],
                },
            )
        for client in stripe_clients:
            ...
            for _owner, identifier, kwargs in client.calls:
                role = next(
                    role for role, spec in m["provider_objects"].items() if spec["id"] == identifier
                )
                self.assertEqual(
                    kwargs,
                    {"stripe_account": "acct_Test1"}
                    if m["provider_objects"][role]["context"] == "connected"
                    else {},
                )
        self.assertTrue(
            all(
                "studio_id" not in row
                for row in captures[4]["local_rows"]
                if row["owner"] == "webhook_events"
            )
        )
        ...
        operation = next(
            row
            for row in captures[4]["local_rows"]
            if row["owner"] == "operations" and row.get("caller_request_key_sha256")
        )
        self.assertRegex(operation["caller_request_key_sha256"], r"^[0-9a-f]{64}$")
        self.assertNotIn("caller_request_key", operation)
        self.assertEqual(
            m["local_ids"]["operations"]["plan_product_create"],
            m["local_ids"]["operations"]["plan_price_create"],
        )
        self.assertEqual(
            supabase_clients[0].calls[0][1]["p_local_ids"]["operations"]["plan_product_create"],
            supabase_clients[0].calls[0][1]["p_local_ids"]["operations"]["plan_price_create"],
        )
    ...
    def test_rpc_boundaries_accept_postgres_utc_offset_rendering(self):
        ...
        for row in client.envelope["local_rows"]:
            for key, value in list(row.items()):
                if (
                    (key.endswith("_at") or key == "period_boundary")
            ...
            stripe = FakeStripe(
                {
                    ...
                    for row in source[index]["provider_objects"]
                }
            )
            captures.append(
                C.capture_phase(
                    ...
                    now=lambda current=boundary: current,
                )
            )
    ...
    def test_rpc_timestamp_values_fail_closed(self):
        m, _captures, clients, _ = adapter_chain()
        client = copy.deepcopy(clients[4])
        next(row for row in client.envelope["local_rows"] if row.get("created_at"))[
            "created_at"
        ] = "not-an-instant"
    ...
    def test_live_adapter_state_drift_is_refused(self):
        ...
        for phase_index in (2, 4):
            operation = next(
                row for row in source[phase_index]["local_rows"] if row.get("owner") == "operations"
            )
            operation.update(
                state="definitive_failed",
                definitive_failed_at="2026-08-29T10:04:00Z",
                error_code="declined",
            )
    ...
    def test_rpc_envelope_and_row_drift_are_refused(self):
        ...
        cases = {
            ...
            "boundary": lambda envelope: envelope.update(
                event_window_ended_at="2026-08-29T10:03:00Z"
            ),
            "bindings": lambda envelope: envelope["local_id_bindings"]["payments"].update(
                automatic="wrong"
            ),
            "malformed-rows": lambda envelope: envelope.update(local_rows={}),
            "missing-row": lambda envelope: envelope["local_rows"].pop(
                next(
                    i
                    for i, row in enumerate(envelope["local_rows"])
                    if row.get("owner") == "payments"
                )
            ),
            "duplicate-row": lambda envelope: envelope["local_rows"].append(
                copy.deepcopy(
                    next(row for row in envelope["local_rows"] if row.get("owner") == "payments")
                )
            ),
        }
        for label, mutate in cases.items():
        ...
        for phase in (2, 4):
            event = next(
                row
                for row in artifacts[phase]["local_rows"]
                if row.get("stripe_event_id") == "evt_connect"
            )
            event.update(processing_status="failed", error="handler_failed")
            artifacts[phase] = C.artifact(
                {key: artifacts[phase][key] for key in C.ARTIFACT_BODY_KEYS}
            )
    ...
    def test_readiness_is_required_hash_bound_and_source_derived(self):
        ...
        with self.assertRaisesRegex(C.CollectorError, "capture readiness"):
            C.capture_phase(
                m,
                C.PHASES[0],
                readiness={**READINESS, "commit_sha": "0" * 40},
                local_reader=lambda _boundary: [],
                provider_reader=lambda _phase: [],
            )
        tampered = copy.deepcopy(artifacts)
        tampered[0]["readiness"]["commit_sha"] = "0" * 40
    ...
    def test_rpc_failure_is_sanitized_and_never_falls_back_to_table(self):
        m = manifest()
        client = FakeSupabase({}, error=RuntimeError("PRIVATE raw response secret"))
    ...
    def test_each_proof_step_requires_its_projected_predicate(self):
        ...
        mutations = {
            ...
            "operation_bounded_role_capabilities": lambda p: p["role_capabilities"].update(
                instructor=["bad"]
            ),
            ...
            "payer_consent_duplicate_replay": lambda p: p["supplemental_evidence"][
                "payer_setup_lifecycle"
            ]["replacement"].update(active=False),
            "shared_subscription_quantity_two": lambda p: p["workflow_facts"].update(
                shared_provider_quantity=1
            ),
            "invoice_link_finalize_send": lambda p: p["workflow_facts"].update(
                invoice_link_sent=False
            ),
            "automatic_payment_fee_50bps": lambda p: p["workflow_facts"].update(
                automatic_amount_cents=9999
            ),
            "failed_payment_named_retry": lambda p: p["supplemental_evidence"][
                "failed_payment_retry"
            ]["failed_provider_readback"].update(invoice_status="paid"),
            "period_end_schedule_revoke_due": lambda p: p["workflow_facts"].update(
                period_due_state="pending"
            ),
            "refund_dispute_convergence": lambda p: p["workflow_facts"].update(
                net_collected_cents=1
            ),
            "ambiguous_same_key_readback_recovery": lambda p: p["workflow_facts"].update(
                ambiguous_recovery_outcome="failed"
            ),
            "platform_webhook_delivery_readback": lambda p: p["webhook_delivery_evidence"][
                "platform"
            ].update(local_processing_status="failed"),
            "connect_webhook_delivery_readback": lambda p: p["webhook_delivery_evidence"][
                "connect"
            ].update(local_processing_status="failed"),
            "terminal_zero_counts": lambda p: p["terminal_counts"]["counts"]["failed"].update(
                count=1
            ),
        }
        packet = C.assemble(m, artifacts)
        for name in validator.REQUIRED_STEP_ORDER:
            with self.subTest(name=name):
                ...
                with self.assertRaisesRegex(C.CollectorError, name):
                    C._proof_steps(changed, validator)
    ...
    def test_shared_parents_keep_distinct_exact_steps(self):
        ...
        self.assertNotEqual(
            m["local_ids"]["steps"]["plan_product_create"],
            m["local_ids"]["steps"]["plan_price_create"],
        )
        for label, mutate, message in (
            (
                ...
                lambda rows: next(
                    row
                    for row in rows
                    if row.get("owner") == "steps"
                    and row.get("id") == m["local_ids"]["steps"]["plan_price_create"]
                ).update(operation_id="wrong"),
                "parent",
            ),
            (
                "duplicate_operation",
                lambda rows: next(
                    ...
                    if row.get("owner") == "steps"
                    and row.get("id") == m["local_ids"]["steps"]["plan_price_create"]
                ).update(provider_operation="connected_product.create"),
                "provider state",
            ),
        ):
            with self.subTest(label=label):
                ...
                for phase in (2, 4):
                    mutate(changed[phase]["local_rows"])
                    changed[phase] = C.artifact(
                        {key: changed[phase][key] for key in C.ARTIFACT_BODY_KEYS}
                    )
                phases = C.validate_phase_chain(m, changed)
                index = C._source_index(m, phases[4]["local_rows"], phases[5]["provider_objects"])
                validator, _ = C._load_contract()
                with self.assertRaisesRegex(C.CollectorError, message):
                    C._project_group_one(m, index, READINESS, validator)
    ...
    def test_test_clock_mode_is_kind_and_context_guarded_without_livemode(self):
        ...
        normalized = C.normalize_provider(
            {"id": "clock_1", "created": 300}, kind="test_clock", context="connected"
        )
        self.assertNotIn("livemode", normalized)
        for field, value in (("kind", "customer"), ("context", "platform")):
            with self.subTest(field=field):
                changed = copy.deepcopy(m)
                changed["provider_objects"]["test_clock"][field] = value
    ...
    def test_counted_owner_window_rejects_extra_and_missing_rows(self):
        ...
        for label, mutate in (
            ...
            with self.subTest(label=label):
                client = copy.deepcopy(clients[4])
                rows = [
                    row for row in client.envelope["local_rows"] if row.get("owner") == "payments"
                ]
                mutate(rows)
                client.envelope["local_rows"] = [
                    row for row in client.envelope["local_rows"] if row.get("owner") != "payments"
                ] + rows
