...
class ReleaseSchemaReadinessTest(unittest.TestCase):
    def test_v44_candidate_constants_are_exact(self):
        self.assertEqual(EXPECTED_RELEASE_MIGRATION_COUNT, 139)
        self.assertEqual(EXPECTED_RELEASE_MIGRATION_HEAD, "20260910135133")
        self.assertEqual(
            EXPECTED_RELEASE_MANIFEST_VERSION,
            "release-db-attestation-v44",
        )
        self.assertEqual(EXPECTED_RELEASE_PENDING_VERSIONS[-2:], ["20260910093958", "20260910135133"])
        self.assertEqual(len(EXPECTED_RELEASE_PENDING_VERSIONS), 55)

    def test_exact_preflight_is_ready(self):
        validate_release_schema_preflight(exact_preflight_row())

    def test_incomplete_or_malformed_preflight_shape_fails_closed(self):
        rows = (
            {
                key: value
    ...
    def test_coherent_v43_predecessor_cannot_satisfy_v44_readiness(self):
        previous = {
            "ready": True,
            "migration_count": 138,
            "migration_head": "20260910093958",
            "pending_versions": EXPECTED_RELEASE_PENDING_VERSIONS[:-1],
            "security_failures": [],
            "manifest_version": "release-db-attestation-v43",
        }
    ...
    def test_hosted_check_uses_required_rpc(self):
        ...
        with patch(
            ...
            assert_hosted_release_schema_ready()
        self.assertEqual(calls, [("koaryu_release_schema_preflight_v25", {})])
        ...
        def rpc(name, params):
            ...
            error = PostgrestAPIError({
                ...
                "message": (
                    ...
                    "public.koaryu_release_schema_preflight_v25 in the schema cache"
                ),
            })
            return SimpleNamespace(execute=lambda: (_ for _ in ()).throw(error))
