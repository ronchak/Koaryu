...
class ReleaseSchemaReadinessTest(unittest.TestCase):
    def test_v45_candidate_constants_are_exact(self):
        self.assertEqual(EXPECTED_RELEASE_MIGRATION_COUNT, 140)
        self.assertEqual(EXPECTED_RELEASE_MIGRATION_HEAD, "20260910185031")
        self.assertEqual(
            EXPECTED_RELEASE_MANIFEST_VERSION,
    ...
    def test_coherent_v44_predecessor_cannot_satisfy_v45_readiness(self):
        previous = {
            "ready": True,
            "migration_count": 139,
            "migration_head": "20260910135133",
            "pending_versions": EXPECTED_RELEASE_PENDING_VERSIONS[:-1],
            "security_failures": [],
            "manifest_version": "release-db-attestation-v44",
        }
