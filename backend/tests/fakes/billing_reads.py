...
class BillingReadRpcMixin:
    def _rpc_billing_payment_cohort(self, params):
        start, end = (
            datetime.fromisoformat(params[key]) for key in ("p_period_start", "p_period_end")
        )
        totals = dict(
            payment_count=0,
        ...
        for row in self.tables.get("billing_payments", []):
            ...
            if (
                ...
                or not stamp
                or row["status"] not in {"succeeded", "refunded", "disputed", "externally_recorded"}
            ):
                continue
            ...
            gross = max(
                ...
                if row.get("gross_paid_amount_cents") is not None
                else row.get("amount_cents", 0),
            )
            ...
            net = max(
                ...
                if row.get("net_collected_amount_cents") is not None
            ...
            totals[
                ...
                if row["status"] == "externally_recorded"
                else "stripe_net_amount_cents"
            ] += net
        return dict(
            totals, period_start=params["p_period_start"], period_end=params["p_period_end"]
        )
    ...
    def _rpc_billing_webhook_health(self, params):
        ...
        for account in [None] + ([params["p_account_id"]] if params["p_account_id"] else []):
            rows = [
                r
                for r in self.tables.get("stripe_events", [])
                if r.get("stripe_account_id") == account
            ]
            processed = sorted(
                [
                    r
                    for r in rows
                    if r.get("processing_status") == "processed" and r.get("processed_at")
                ],
            ...
            result[account or "platform"] = dict(
                ...
                stale_processing_count=sum(
                    r.get("processing_status") == "processing"
                    and bool(r.get("processing_started_at") or r.get("created_at"))
                    and datetime.fromisoformat(r.get("processing_started_at") or r["created_at"])
                    <= datetime.fromisoformat(params["p_stale_before"])
                    for r in rows
                ),
                mode_mismatch_count=sum(
                    r.get("livemode") is not None and r["livemode"] != params["p_expected_livemode"]
                    for r in rows
                )
                if params["p_expected_livemode"] is not None
                else 0,
            )
        return result
