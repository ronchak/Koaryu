...
class _BootstrapLifecycleSupabase(_FakeSupabase):
        ...
        return [
            {
                ...
                "account_create_idempotency_key": self.bootstrap[
                    "p_account_create_idempotency_key"
                ],
                ...
                "phase": (
                    "initial_link_delivery_pending"
                    if self.bootstrap_response
                    else "initial_link_retry"
                    if self.bootstrap.get("stripe_connected_account_id")
                    else "account_create"
                ),
            }
        ]
    ...
    def _rpc_bind_connect_onboarding_bootstrap_account_v2(self, params):
        ...
        if (
            ...
            or params["p_connect_account_generation"]
            != self.bootstrap["p_connect_account_generation"]
        ):
            return []
        row = next(
            row
            for row in self.tables["studio_payment_accounts"]
    ...
    def _rpc_authorize_connect_onboarding_bootstrap_initial_link_v2(self, params):
        if (
            not self.bootstrap_create
            or params["p_bootstrap_id"] != self.bootstrap_id
            or params["p_stripe_connected_account_id"]
            != self.bootstrap.get("stripe_connected_account_id")
            or params["p_initial_link_context_sha256"]
            != self.bootstrap["p_initial_link_context_sha256"]
            or params["p_initial_link_idempotency_key"]
            != self.bootstrap["p_initial_link_idempotency_key"]
        ):
            return []
    ...
    def _rpc_record_connect_onboarding_bootstrap_initial_link_response(self, params):
        if (
            ...
            or params["p_initial_link_context_sha256"]
            != self.bootstrap["p_initial_link_context_sha256"]
            or params["p_initial_link_payload_sha256"]
            != self.bootstrap_link["p_initial_link_payload_sha256"]
            or params["p_initial_link_idempotency_key"]
            != self.bootstrap["p_initial_link_idempotency_key"]
        ):
            return []
...
class BillingConnectLifecycleTest(BillingPaymentsLifecycleTestBase):
    def test_live_first_onboarding_runs_real_policy_rpc_chain_and_only_mocks_provider_transport(
        self,
    ):
        ...
        supabase = _BootstrapLifecycleSupabase(
            {
                ...
                "studios": [
                    {"id": studio_id, "name": "Policy Chain Studio", "owner_id": "owner_1"}
                ],
            }
        )
        provider_calls = []
        ...
        with (
            patch("app.services.billing_service.get_settings", return_value=settings),
            patch("app.services.stripe_service.get_settings", return_value=settings),
            patch("app.services.stripe_service.stripe_v2_request", side_effect=provider_transport),
            patch.dict("os.environ", {"RENDER_GIT_COMMIT": "a" * 40}, clear=False),
        ):
            service = BillingService(supabase)
            link = asyncio.run(
                service.create_connect_onboarding_link(
                    studio_id,
                    "actor_1",
        ...
        self.assertEqual(
            supabase.tables["studio_payment_accounts"][0]["stripe_connected_account_id"],
            "acct_BootstrapPolicy1",
        )
        self.assertEqual(len(provider_calls), 2)
        self.assertEqual(
            provider_calls[0][3], supabase.bootstrap_create["p_account_create_idempotency_key"]
        )
        self.assertEqual(
            provider_calls[1][3], supabase.bootstrap_link["p_initial_link_idempotency_key"]
        )
        self.assertEqual(
        ...
        for interrupted_phase in ("account_create", "bind", "initial_link"):
            with self.subTest(interrupted_phase=interrupted_phase):
                supabase = _BootstrapLifecycleSupabase(
                    {
                        ...
                        "studios": [
                            {"id": studio_id, "name": "Recovery Studio", "owner_id": "owner_1"}
                        ],
                    }
                )
                provider_calls = []
                ...
                with (
                    ...
                    patch("app.services.stripe_service.get_settings", return_value=settings),
                    patch(
                        "app.services.stripe_service.stripe_v2_request",
                        side_effect=provider_transport,
                    ),
                    patch.dict("os.environ", {"RENDER_GIT_COMMIT": "a" * 40}, clear=False),
                ):
                    service = BillingService(supabase)
                ...
                prepare_calls = [
                    ...
                    if name == "prepare_connect_onboarding_bootstrap_atomic"
                ]
                self.assertEqual(len(prepare_calls), 1)
                account_calls = [call for call in provider_calls if call[1] == "/v2/core/accounts"]
                link_calls = [
                    call for call in provider_calls if call[1] == "/v2/core/account_links"
                ]
                self.assertTrue(account_calls)
                self.assertTrue(link_calls)
                self.assertEqual(
                    {call[3] for call in account_calls},
                    {supabase.bootstrap["p_account_create_idempotency_key"]},
                )
                self.assertEqual(
                    {call[3] for call in link_calls},
                    {supabase.bootstrap["p_initial_link_idempotency_key"]},
                )
                self.assertEqual(
                    len(account_calls), 2 if interrupted_phase in {"account_create", "bind"} else 1
                )
                self.assertEqual(len(link_calls), 2 if interrupted_phase == "initial_link" else 1)
                self.assertEqual(
                    supabase.bootstrap["stripe_connected_account_id"], "acct_Recovered1"
                )
    ...
    def test_lost_initial_link_response_rotates_receipt_then_retires_to_ordinary_authorization(
        self,
    ):
        studio_id = "11111111-1111-4111-8111-111111111111"
        settings = type(
    ...
    def test_live_onboarding_expired_recovery_is_support_required_without_second_provider_call(
        self,
    ):
        studio_id = "11111111-1111-4111-8111-111111111111"
        settings = type(
    ...
    def test_standard_connect_account_uses_account_holder_dashboard_url(self):
        ...
        url = service.create_connect_dashboard_url(
            account_id="acct_connected", studio_id="studio_1"
        )
    ...
    def test_connect_dashboard_link_uses_account_holder_dashboard_url_for_full_dashboard_account(
        self,
    ):
        _FakeStripe.reset()
        service = StripeService()
        ...
        link = service.create_connect_dashboard_link(
            account_id="acct_connected", studio_id="studio_1"
        )
    ...
    def test_connect_account_creation_uses_accounts_v2_full_dashboard(self):
        ...
        self.assertEqual(
            payload["configuration"]["merchant"]["capabilities"]["card_payments"]["requested"], True
        )
    ...
    def test_bootstrap_connect_account_falls_back_to_accounts_v1_when_accounts_v2_blocked(self):
        ...
        self.assertEqual(
            calls[0]["idempotency_key"], bootstrap_context.account_create_idempotency_key
        )
    ...
    def test_connect_onboarding_link_uses_accounts_v2_account_links(self):
        ...
        def fake_v2_post(path, payload, *, idempotency_key=None, **_context):
            calls.append((path, payload, idempotency_key))
            return {
                "url": "https://connect.stripe.test/v2/acct_v2",
                "object": "v2.core.account_link",
            }

        service._stripe_v2_post = fake_v2_post
        service._stripe = lambda: self.fail(
            "Accounts v2 onboarding must not call Stripe v1 AccountLink APIs."
        )
        ...
        self.assertEqual(
            onboarding["refresh_url"], "https://app.koaryu.test/billing/connect/refresh"
        )
    ...
    def test_bootstrap_onboarding_link_falls_back_to_accounts_v1_with_stored_idempotency(self):
        ...
        self.assertEqual(link["url"], "https://connect.stripe.test/setup/acct_v1")
        self.assertEqual(
            calls[0]["idempotency_key"], bootstrap_context.initial_link_idempotency_key
        )
    ...
    def test_connected_account_branding_update_uses_accounts_v2(self):
        ...
        self.assertEqual(
            payload["configuration"]["merchant"]["branding"],
            {
                "primary_color": "#0B0D10",
                "secondary_color": "#D6B25E",
                "icon": "file_icon",
    ...
    def test_existing_connect_account_uses_default_refresh_and_return_urls_without_studio_lookup(
        self,
    ):
        ...
        self.assertEqual(
            _FakeStripeService.onboarding_calls[0]["refresh_url"],
            "https://app.koaryu.test/billing/connect/refresh",
        )
        self.assertEqual(
            _FakeStripeService.onboarding_calls[0]["return_url"],
    ...
    def test_connect_onboarding_validates_redirect_before_creating_account(self):
        ...
        self.assertEqual(_FakeStripeService.onboarding_calls, [])
        self.assertIsNone(
            service.supabase.tables["studio_payment_accounts"][0]["stripe_connected_account_id"]
        )
    ...
    def test_connect_reset_clears_stale_account_when_no_stripe_history_exists(self):
        ...
        self.assertEqual(
            service.supabase.tables["studio_payment_accounts"][0]["metadata"][
                "previous_stripe_connected_account_ids"
            ],
            ["acct_stale"],
        )
        self.assertEqual(
        ...
        service.supabase = _FakeSupabase(
            {
                ...
                "billing_plans": [
                    {"id": "plan_1", "studio_id": "studio_1", "stripe_price_id": "price_1"}
                ],
                "billing_payers": [],
                "billing_subscriptions": [],
                "billing_invoices": [],
    ...
    def test_stale_connected_account_returns_actionable_conflict(self):
        ...
        with self.assertRaises(HTTPException) as context:
            service.create_connect_dashboard_url(
                account_id="acct_from_other_platform", studio_id="studio_1"
            )
