...
def test_landing_authorizes_fields_independently(role, required):
    aggregates = dict(
        ...
        payment_cohort=dict(
            period_start="2026-12-01Z", period_end="2027-01-01Z", payment_count=1001
        ),
    )
    with (
        patch("app.services.billing_landing.BillingService") as billing,
        patch("app.services.billing_landing.PlatformBillingService") as platform,
        patch(
            "app.services.billing_landing.get_platform_subscription_access",
            return_value={"subscription_required": required},
        ),
        patch(
            "app.services.billing_landing.execute_required_rpc",
            return_value=SimpleNamespace(data=aggregates),
        ) as rpc,
    ):
...
def test_unverifiable_subscription_preserves_diagnostics_without_financial_query():
    with (
        ...
        patch(
            "app.services.billing_landing.get_platform_subscription_access",
            side_effect=HTTPException(503, "unavailable"),
        ),
        patch("app.services.billing_landing.execute_required_rpc") as rpc,
    ):
        billing.return_value.get_system_status = AsyncMock(return_value=billing_system_status())
        result = asyncio.run(
            get_billing_landing(Mock(), {"studio_id": "studio", "role": "front_desk"})
        )
...
def test_endpoint_resolves_membership_once_and_does_not_blanket_gate_subscription():
    async def run(provider, operation, **kwargs):
        return await operation(provider)

    with (
        patch("app.api.v1.endpoints.billing.run_supabase_operation", side_effect=run),
        patch(
            "app.api.v1.endpoints.billing.resolve_billing_manager_staff_role_for_user",
            return_value={"studio_id": "studio", "role": "front_desk"},
        ) as resolver,
        patch(
            "app.services.billing_landing.get_billing_landing",
            new=AsyncMock(return_value="landing"),
        ),
    ):
        response = Response()
...
def test_utc_period_rollover_and_naive_compatibility():
    assert payment_cohort_period(datetime(2026, 12, 31)) == (
...
def test_system_failure_does_not_erase_admin_platform_recovery():
    with (
        ...
        patch(
            "app.services.billing_landing.get_platform_subscription_access",
            return_value={"subscription_required": True},
        ),
    ):
        billing.return_value.get_system_status = AsyncMock(
            side_effect=RuntimeError("failed diagnostic read")
        )
        billing.return_value.get_payment_account = AsyncMock(
            return_value=StudioPaymentAccountResponse(studio_id="studio")
        )
        platform.return_value.get_status = AsyncMock(return_value=None)
