    ...
    def test_create_portal_returns_session_for_valid_customer(self):
        ...
        class FakeStripeService:
            def create_customer_portal_session(
                self, *, customer_id, return_url, studio_id=None, idempotency_key=None
            ):
                keys.append(idempotency_key)
                return {
                    "url": f"https://billing.stripe.test/session?customer={customer_id}&return={return_url}"
                }

        with patch("app.services.platform_billing_service.StripeService", FakeStripeService):
            response = asyncio.run(
                service.create_portal_link("studio_1", "user_1", "https://koaryu.test/billing")
            )
