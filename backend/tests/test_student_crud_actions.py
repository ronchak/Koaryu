...
class FakeStudentWriteSupabase(RpcBackedSupabase):
    def _rpc_write_student_profile_atomic(self, params):
        ...
        if params["p_replace_programs"]:
            ...
            for program_id in program_ids:
                ...
                if membership:
                    membership.update(
                        {"status": "active", "started_at": existing.get("membership_start_date")}
                    )
                else:
                    ladder_ids = {
                        row["id"]
                    ...
                    self.tables["student_program_memberships"].append(
                        {
                            ...
                            "ended_at": None,
                            "current_belt_rank_id": starting_rank.get("id")
                            if starting_rank
                            else None,
                            "created_at": "2026-05-20T00:00:00+00:00",
                            "updated_at": "2026-05-20T00:00:00+00:00",
    ...
    def _rpc_write_student_profile_v2_atomic(self, params):
        ...
        guardians = [dict(row) for row in self.tables["guardians"] if row["id"] in guardian_ids]
        memberships = [
            {
                **row,
            ...
            for row in self.tables["student_program_memberships"]
            if row.get("student_id") == student_id and row.get("studio_id") == params["p_studio_id"]
        ]
        return [
            {
...
class StudentCrudActionsTest(unittest.TestCase):
    ...
    def test_create_student_uses_atomic_rpc_for_student_memberships_guardians_and_audit(self):
        ...
        student = asyncio.run(
            actions.create_student(
                StudentCreate(
                    ...
                    guardians=[
                        GuardianCreate(
                            first_name="Hana", last_name="Tanaka", is_primary_contact=True
                        )
                    ],
                ),
                "studio-1",
        ...
        self.assertEqual(
            [name for name, _params in supabase.rpc_calls], ["write_student_profile_v2_atomic"]
        )
    ...
    def test_update_student_uses_atomic_rpc_for_profile_membership_and_audit(self):
        ...
        self.assertEqual(
            supabase.tables["student_program_memberships"][1]["program_id"], "program-2"
        )
        self.assert_no_direct_operational_writes(supabase)
    ...
    def test_partial_update_preserves_or_explicitly_replaces_existing_tags(self):
        for fields, expected_tags in [
            ...
            with self.subTest(fields=fields):
                ...
                result = asyncio.run(
                    build_actions(supabase).update_student(
                        "student-1",
