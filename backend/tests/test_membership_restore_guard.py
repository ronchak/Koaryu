...
spec = importlib.util.spec_from_file_location(
    "local_postgres_verification", ROOT / "scripts/local_postgres_verification.py"
)
...
def restore_pair_catalogs():
    ...
    source = {
        key: {**metadata, "hash": pair["source_hash"], "definition": pair["source_definition"]}
        for key, pair in pairs.items()
    }
    restored = {
        key: {**metadata, "hash": pair["restored_hash"], "definition": pair["restored_definition"]}
        for key, pair in pairs.items()
    }
...
def test_restore_normalization_accepts_only_the_reviewed_constraint_changes():
    ...
    assert all(
        expected[key] == (restored[key] if pair["replay_definition"] is None else source[key])
        for key, pair in pairs.items()
    )
    key = next(iter(pairs))
    for field, value in [
...
def test_restore_normalization_does_not_repair_extra_grants_or_ownership():
    ...
    statements, _ = guard.normalization_plan(
        source, restored, {identity: original}, {identity: target}, pairs
    )
    ...
    for field, value in [
        ...
        with pytest.raises(RuntimeError, match="Unapproved ACL change"):
            guard.normalization_plan(
                source, restored, {identity: original}, {identity: {**target, field: value}}, pairs
            )
...
def test_restore_helper_refuses_remote_or_unowned_targets_before_invoking_tools(tmp_path):
    ...
    for socket, port, temporary in [
        ...
        for script, arguments in [
            ...
            (
                ROOT / "scripts/verify-v39-v40-restore-contract.py",
                [str(executable)] * 4 + [socket, port, temporary, str(ROOT)],
            ),
            (ROOT / "scripts/verify-student-write-concurrency.py", [str(executable), socket, port]),
            (
                ROOT / "scripts/verify-v40-v41-restore-contract.py",
                [str(executable)] * 4 + [socket, port, temporary, str(ROOT)],
            ),
            (
                ROOT / "scripts/verify-v41-v42-restore-contract.py",
                [str(executable)] * 4 + [socket, port, temporary, str(ROOT)],
            ),
            (
                ROOT / "scripts/verify-v42-v43-restore-contract.py",
                [str(executable)] * 4 + [socket, port, temporary, str(ROOT)],
            ),
            (
                ROOT / "scripts/verify-v43-v44-restore-contract.py",
                [str(executable)] * 4 + [socket, port, temporary, str(ROOT)],
            ),
            (
                ROOT / "scripts/verify-v44-v45-restore-contract.py",
                [str(executable)] * 4 + [socket, port, temporary, str(ROOT)],
            ),
            (
                ROOT / "scripts/verify-billing-command-concurrency.py",
                [str(executable), socket, port],
            ),
        ]:
            ...
            assert (
                "Expected the local verifier" in result.stderr
                or "Unexpected local socket or port" in result.stderr
            )
