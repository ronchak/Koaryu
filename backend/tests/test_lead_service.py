...
class FakeLeadConversionSupabase(RpcBackedSupabase):
    def _rpc_convert_lead_to_student_atomic(self, params):
        ...
        if not any(
            row.get("id") == student_id and row.get("studio_id") == params["p_studio_id"]
...
class LeadServiceTest(unittest.TestCase):
    def test_convert_existing_deterministic_student_repairs_missing_membership(self):
        ...
        self.assertEqual(
            [name for name, _params in supabase.rpc_calls], ["convert_lead_to_student_atomic"]
        )
        ...
        self.assertEqual(
            {
                key: membership[key]
                for key in ("studio_id", "student_id", "program_id", "status", "started_at")
            },
            {
                "studio_id": "studio-1",
                "student_id": student_id,
        ...
        supabase = FakeLeadConversionSupabase(
            {
                "leads": [
                    lead_row(
                        ...
                        guardian_email="mina@example.test",
                    )
                ],
                "students": [],
                "student_program_memberships": [],
