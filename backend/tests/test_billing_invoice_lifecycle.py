        ...
        "billing_payments": [
            {
                ...
                "net_collected_amount_cents": 200,
...
class BillingInvoiceLifecycleTest(BillingPaymentsLifecycleTestBase):
                ...
                "billing_invoice_retry_operations": [
                    {
                        ...
                        "studio_id": "studio_1",
                        "invoice_id": "invoice_1",
                        "client_idempotency_key": "expired-key",
                        "stripe_idempotency_key": "stripe-expired-key",
        ...
        with patch("app.services.billing_service.StripeService", _FakeStripeService):
            for contender in ("contender-key-1", "contender-key-2"):
                with self.assertRaises(HTTPException) as in_flight:
                    asyncio.run(
                        service.retry_invoice_payment("invoice_1", "studio_1", "actor_1", contender)
                    )
    ...
    def test_create_invoice_reuses_matching_idempotency_key(self):
        ...
        with patch("app.services.billing_service.StripeService", _FakeStripeService):
            persisted = asyncio.run(
                service.create_invoice(data, "studio_1", "user_1", idempotency_key="invoice-key")
            )
            provider_counts = (
            ...
            replay = asyncio.run(
                restarted_service.create_invoice(
    ...
    def test_paid_in_full_enrollment_uses_invoice_idempotency_path(self):
        ...
        with patch("app.services.billing_service.StripeService", _FakeStripeService):
            enrollment = asyncio.run(
                service.add_student_billing_enrollment(
                    StudentBillingEnrollmentCreate(
                        student_id="student_1",
                        billing_plan_id="plan_1",
                        payer_id="payer_1",
            ...
            with self.assertRaises(HTTPException) as exc:
                asyncio.run(
                    service.create_invoice(
                        BillingInvoiceCreate(
                            payer_id="payer_1", amount_cents=8900, description="Changed tuition"
                        ),
                        "studio_1",
                        "user_1",
                        idempotency_key="invoice-key",
    ...
    def test_late_payment_intent_backlinks_existing_refund(self):
        ...
        self.assertEqual(
            service.supabase.tables["billing_payments"][0]["refunded_amount_cents"], 50
        )
    ...
    def test_terminal_won_dispute_does_not_regress_or_reverse_balance(self):
        ...
        self.assertEqual(
            service.supabase.tables["billing_invoices"][0]["amount_remaining_cents"], 0
        )
    ...
    def test_delayed_historical_adjustment_keeps_unknown_generation_reconciliation(self):
        ...
        payment["adjustment_reconciliation_reason_code"] = "historical_connect_generation_unknown"
        tables["billing_refunds"] = [
            {
