...
def config(*, workers=1, queue=0, caller=2, transport=0.12):
    return SupabaseLaneConfig(
        ...
        operation_wait_timeout=caller,
        postgrest_client_timeout=transport,
    )
...
def test_runtime_applies_independent_transport_budgets_and_normalizes_timeout(provider_server):
    async def scenario():
        ...
        try:
            interactive = await runtime.run_interactive(
                lambda client: client.postgrest.session.timeout.read
            )
            ...
            with pytest.raises(HTTPException) as error:
                await run_supabase_operation(
                    runtime, lambda client: client.table("stall_body").select("*").execute()
                )
...
def test_mixed_concurrency_records_bounded_metrics(provider_server, sessions, scope):
    async def scenario():
        runtime = SupabaseProviderRuntime(
            config(workers=4, queue=16, transport=1), config(transport=1)
        )
        latencies = []
        try:
            ...
            async def session(index):
                ...
                def read(client):
                    for _ in range(1 + index % 3):
                        assert client.table("healthy").select("*").eq(
                            "studio_id", studio
                        ).execute().data == [{"ok": True}]
            ...
            print(
                json.dumps(
                    {
                        "scenario": "synthetic_loopback_provider",
                        "scope": scope,
                        "sessions": sessions,
