    ...
    def seed(
        ...
        plan_specs = [
            (
                "kids-unlimited",
                "Kids Unlimited",
                "Unlimited youth classes with belt testing billed separately.",
                12900,
                "monthly",
            ...
            (
                "adult-unlimited",
                "Adult Unlimited",
                "Unlimited adult BJJ classes, no-gi, and open mat access.",
                14900,
                "monthly",
                [program_ids["bjj_core"]],
            ),
            (
                ...
                "Monthly Tae Kwon Do tuition for youth and adult fundamentals.",
                11900,
                "monthly",
            ...
            (
                "family-unlimited",
                "Family Unlimited",
                "Family tuition for multiple students across programs.",
                17900,
                "monthly",
                [program_ids["bjj_core"], program_ids["tae_kwon_do"]],
            ),
            (
                "belt-test",
                "Belt Testing Fee",
                "One-time exam charge collected when a promotion is approved.",
                3500,
                "paid_in_full",
                [program_ids["bjj_core"], program_ids["tae_kwon_do"]],
            ),
        ...
        for index, (key, name, description, amount, interval, attached_programs) in enumerate(
            plan_specs
        ):
            ...
            plan_rows.append(
                {
                    ...
                    "stripe_one_time_price_id": f"price_demo_{key}_one_time"
                    if interval == "paid_in_full"
                    else None,
                    "metadata": {"demo": True},
        ...
        self._insert_optional(
            ...
            [
                ...
                for index, (key, _name, _description, amount, interval, _programs) in enumerate(
                    plan_specs
                )
            ],
        )
            ...
            payer_rows.append(
                {
                    ...
                    if billing_status == "externally_paid"
                    else DEMO_CONNECT_ACCOUNT_ID,
                    "stripe_customer_id": None
                    if billing_status == "externally_paid"
                    else f"cus_demo_{key}",
        ...
        for (
            ...
            next_bill_on,
            stripe_subscription,
        ) in subscription_specs:
            subscription_id = self._id(
                studio_id, f"billing-subscription:{payer_key}:{collection_mode}:{billing_interval}"
            )
            ...
            subscription_rows.append(
                {
                    ...
                    "current_period_end": self._timestamp(7, 0)
                    if billing_interval != "paid_in_full"
                    else self._timestamp(2, 0),
                    "cancel_at_period_end": False,
                    "default_payment_method_id": None
                    if collection_mode != "autopay"
                    else f"pm_demo_{payer_key}",
            ...
            enrollment_rows.append(
                {
                    ...
                    if external
                    else f"si_demo_{payer_key}_{plan_key}",
                    "metadata": {"demo": True},
                    "created_at": self._timestamp(-30, 9),
                    "updated_at": now,
                }
            ...
            invoice_rows.append(
                {
                    ...
                    if external or invoice_status != "paid"
                    else f"pi_demo_{key}",
                    ...
                    "currency": "usd",
                    "hosted_invoice_url": f"https://dashboard.stripe.com/test/invoices/{stripe_invoice}"
                    if stripe_invoice
                    else None,
                    "due_date": due_date,
                    ...
                    "finalized_at": self._timestamp(-25, 8, 15)
            ...
            invoice_item_rows.append(
                {
                    ...
                    "student_id": student_ids[student_key],
                    "description": "Belt testing fee"
                    if invoice_type == "belt_test"
        ...
        payment_specs = [
            ...
            (
                ...
                "card",
                None,
                "Signup fee collected before monthly invoice balance.",
                -1,
            ),
            (
                "card-johnson-liam",
        ...
        for (
            ...
            external_request_key = (
                f"demo-external-payment:{payment_id}" if external_method else None
            )
            ...
            payment_rows.append(
                {
                    ...
                    "receipt_url": None
                    if external_method
                    else f"https://dashboard.stripe.com/test/payments/pi_demo_{key}",
                    "application_fee_amount_cents": 0 if external_method else round(amount * 0.005),
                    "status": payment_status,
                    "amount_cents": amount,
                    "disputed_amount_cents": 0,
                    "net_collected_amount_cents": amount,
                    "refundable_amount_cents": amount if not external_method else 0,
                    "currency": "usd",
