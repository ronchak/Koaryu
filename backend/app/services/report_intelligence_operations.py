...
def build_belt_momentum_testing_pipeline(
    ...
    for membership in memberships:
        ...
        if not student or not _is_active_student(student):
            continue
        current_rank = ranks_by_id.get(
            membership.get("current_belt_rank_id") or student.get("current_belt_rank_id")
        )
        program = (
            programs_by_id.get(membership.get("program_id") or student.get("program_id")) or {}
        )
        next_rank = None
        if current_rank:
            siblings = sorted(
                ranks_by_ladder.get(current_rank.get("ladder_id"), []),
                key=lambda row: int(row.get("display_order") or 0),
            )
            for rank in siblings:
                if int(rank.get("display_order") or 0) > int(
                    current_rank.get("display_order") or 0
                ):
                    next_rank = rank
                    break
        elif program.get("id") in ladders_by_program:
            siblings = sorted(
                ranks_by_ladder.get(ladders_by_program[program.get("id")].get("id"), []),
                key=lambda row: int(row.get("display_order") or 0),
            )
        ...
        time_met = (
            bool(next_rank) and isinstance(days_at_rank, int) and days_at_rank >= required_days
        )
        if not current_rank and not next_rank:
            ...
            status_text = (
                "ready_for_review" if next_rank.get("requires_approval") else "ready_to_test"
            )
        elif classes_met:
        ...
        rows.append(
            {
                ...
                "time_met": time_met,
                "requires_approval": bool(next_rank.get("requires_approval"))
                if next_rank
                else False,
    ...
    return sorted(
        rows, key=lambda row: (status_rank.get(row["pipeline_status"], 9), row["student_name"])
    )
...
def build_schedule_utilization_demand(
    ...
    for session in data.get("sessions", []):
        ...
        if (
            ...
            or session_date < today - timedelta(days=89)
        ...
        row = grouped.setdefault(
            key,
            {
                ...
                "program_name": (programs_by_id.get(session.get("program_id")) or {}).get("name")
                or "",
    ...
    return sorted(
        rows, key=lambda row: (row["recommendation"], int(row["total_attendance"])), reverse=True
    )
...
def build_instructor_staff_impact(
    ...
    for session in data.get("sessions", []):
        ...
        if (
            ...
            or session_date > today
            or session.get("deleted_at")
            or session.get("status") == "canceled"
        ):
            continue
    ...
    return sorted(
        rows,
        key=lambda row: (
            int(row["total_attendance_90_days"]),
            int(row["assigned_leads_enrolled_or_converted"]),
        ),
        reverse=True,
    )
...
def build_data_hygiene_readiness(
        ...
        if student.get("is_minor") and not guardians_by_student.get(student_id):
            rows.append(
                _hygiene_row(
                    ...
                    student_id,
                    student_id,
                    "Minor student has no linked guardian.",
                    "Add at least one guardian contact.",
                )
            )
        if active and not student.get("emergency_contact_name"):
            rows.append(
                _hygiene_row(
                    ...
                    student_id,
                    student_id,
                    "Active student has no emergency contact name.",
                    "Add emergency contact details.",
                )
            )
        if (
            active
            and not student.get("program_id")
            and not any(
                row.get("status") == "active" for row in memberships_by_student.get(student_id, [])
            )
        ):
            rows.append(
                _hygiene_row(
                    ...
                    student_id,
                    student_id,
                    "Active student is not clearly assigned to a program.",
                    "Assign a program membership.",
                )
            )
        has_membership_rank = any(
            row.get("current_belt_rank_id") for row in memberships_by_student.get(student_id, [])
        )
        if active and not student.get("current_belt_rank_id") and not has_membership_rank:
            rows.append(
                _hygiene_row(
                    "missing_rank_assignment",
                    "low",
                    "student",
                    student_id,
                    student_id,
                    "Active student has no current belt rank on the legacy student record.",
                    "Set the current rank or verify program membership rank.",
                )
            )
        if active and not billing_enrollments_by_student.get(student_id):
            rows.append(
                _hygiene_row(
                    "missing_billing_enrollment",
                    "high",
                    "student",
                    student_id,
                    student_id,
                    "Active student has no billing enrollment.",
                    "Attach a billing plan or mark billing as intentionally external.",
                )
            )
    ...
    for lead in data.get("leads", []):
        if lead.get("stage") in OPEN_LEAD_STAGES and not lead.get("follow_up_date"):
            rows.append(
                _hygiene_row(
                    "active_lead_without_follow_up",
                    "medium",
                    "lead",
                    lead.get("id"),
                    "",
                    "Active lead has no follow-up date.",
    ...
    for payer in data.get("billing_payers", []):
        if not payer.get("email") and not payer.get("phone"):
            rows.append(
                _hygiene_row(
                    "payer_without_contact_method",
                    "high",
                    "billing_payer",
                    payer.get("id"),
                    "",
                    "Payer has neither email nor phone.",
                    "Add at least one payer contact method.",
                )
            )
