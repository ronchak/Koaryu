...
def merge_invoice_identity_from_stored_event(
    invoice: dict[str, Any], stored_invoice: dict[str, Any]
) -> dict[str, Any]:
    merged = dict(invoice)
    if not invoice_parent_subscription_details(merged) and invoice_parent_subscription_details(
        stored_invoice
    ):
...
def invoice_subscription_id(invoice: Any) -> Optional[str]:
    ...
    line_subscriptions = {
        stripe_id(
            object_get(
                object_get(object_get(line, "parent") or {}, "subscription_item_details") or {},
                "subscription",
            )
        )
...
def subscription_period_bounds(subscription: dict[str, Any]) -> tuple[Optional[Any], Optional[Any]]:
    ...
    return (
        start or (min(item_starts) if item_starts else None),
        end or (max(item_ends) if item_ends else None),
    )
