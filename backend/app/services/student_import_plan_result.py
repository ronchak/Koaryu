...
def build_import_result(
    ...
    *,
    ...
    for row in rows:
        ...
        if row["issues"]:
            error_messages = [issue.message for issue in row["issues"] if issue.severity == "error"]
            warning_messages = [
    ...
    can_create_missing_belts = any(
        ...
        and (
            row.get("belt_creation_target_ladder_id")
            or row.get("belt_creation_requires_new_ladder")
        )
        for row in rows
    )
    ...
    if setup_buckets["missing_belt_ladder"]["row_numbers"]:
        ...
        if can_create_missing_belts:
            missing_belt_ladder_message = "It looks like some programs do not have belt ladders set up yet, but Koaryu can create them during import."
        elif severity == "error":
            missing_belt_ladder_message = "It looks like your belt ladder is not set up yet, so current belt values cannot be matched."
        else:
            missing_belt_ladder_message = "Your belt ladder is not set up yet. Students can still be imported without current belts."
        setup_issues.append(
            CsvImportSetupIssue(
    ...
    if setup_buckets["missing_belt"]["row_numbers"]:
        ...
        if can_create_missing_belts:
            missing_belt_message = "Some current belt values do not match their program ladders yet, but Koaryu can create them during import."
        elif severity == "error":
            missing_belt_message = (
                "Some current belt values do not match the belt ladder configured for this studio."
            )
        else:
            missing_belt_message = "Some current belt values do not match this studio's ladder, but those students can still import without belts."
        setup_issues.append(
            CsvImportSetupIssue(
                code="missing_belt",
                severity=severity,
