...
INVOICE_RETRY_PREREAD_UNAVAILABLE_DETAIL = (
    "Invoice payment was not attempted because provider evidence could not be read. "
    "Retry with the same Idempotency-Key."
)
INVOICE_RETRY_PREREAD_RELEASE_FAILED_DETAIL = (
    "Invoice payment was not attempted, but retry readiness could not be confirmed. "
    "Wait and retry with the same Idempotency-Key."
)
...
class BillingInvoiceManager:
    def __init__(
        ...
        *,
        ...
        self.projector = BillingWebhookProjector(
            supabase,
            connect_accounts,
    ...
    async def reconcile_invoice(
        ...
        invoice = self.projector.update_invoice_from_stripe(
        ...
        record_billing_audit(
            ...
            studio_id,
            actor_id,
            "billing.invoice_reconciled",
            invoice_id,
            {},
        )
        recompute_payer_balance(self.supabase, studio_id, invoice.get("payer_id"))
    ...
    async def create_invoice(
        ...
        return self.create_invoice_sync(data, studio_id, actor_id, idempotency_key)
    ...
    def create_invoice_sync(
        ...
        if data.collection_mode == "autopay":
            ...
            if not payer_autopay_authorized(self.supabase, self.connect_accounts, payer):
                raise HTTPException(
                    status_code=status.HTTP_409_CONFLICT,
        ...
        caller_hash = self._invoice_request_hash(data)
        existing_invoice = self._find_invoice_by_idempotency_key(
            studio_id,
            request_key,
        )
    ...
    def _require_future_invoice_due_date(self, due_date: str | None) -> None:
        if due_date is None:
            return
        if date.fromisoformat(due_date) <= self._utc_today():
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
    ...
    async def finalize_invoice(
        ...
        if state == "projected":
            ...
            recompute_payer_balance(self.supabase, studio_id, invoice["payer_id"])
    ...
    async def void_invoice(
        ...
        if not invoice.get("stripe_invoice_id") and not invoice.get("stripe_account_id"):
            ...
            recompute_payer_balance(self.supabase, studio_id, local_void.get("payer_id"))
        ...
        if state == "projected":
            ...
            recompute_payer_balance(self.supabase, studio_id, voided.get("payer_id"))
        ...
        if state == "provider_succeeded":
            ...
            try:
                provider_void = self.stripe_service_cls().void_connected_invoice(
                    ...
                    idempotency_key=build_idempotency_key("invoice-void", context.operation_id),
                )
    ...
    async def retry_invoice_payment(
        ...
        if retry_autopay_consent is not None:
            try:
                ...
                if current_retry_snapshot != retry_autopay_consent:
                    raise HTTPException(
                        status_code=409,
        ...
        try:
            pay_payload = {
                ...
                "studio_id": context.studio_id,
                "invoice_id": str(invoice["stripe_invoice_id"]),
                "idempotency_key": build_idempotency_key("invoice-retry", context.operation_id),
            }
            if retry_autopay_consent is not None:
    ...
    def _require_retry_autopay_consent(
        ...
        *,
        ...
        local_consent = self._read_autopay_consent(
            invoice,
            studio_id=studio_id,
            generation=generation,
            local_unavailable_is_retryable=True,
        ...
        try:
            ...
            raise _RetryProviderEvidenceUnavailable() from exc
        return self._require_autopay_consent(
            ...
            provider_unavailable_is_retryable=True,
        )

    @staticmethod
    def _release_retry_preread_lease(
        ...
        *,
        ...
        try:
            ...
            raise HTTPException(
                status_code=503,
                detail=INVOICE_RETRY_PREREAD_RELEASE_FAILED_DETAIL,
            ) from exc
        if not valid:
            raise HTTPException(
                status_code=503,
    ...
    def _require_autopay_consent(
        ...
        *,
        ...
        local_consent: tuple[dict[str, Any], dict[str, Any], str] | None = None,
        provider_unavailable_is_retryable: bool = False,
    ) -> dict[str, Any]:
        payer, consent, payment_method_id = local_consent or self._read_autopay_consent(
            invoice, studio_id=studio_id, generation=generation
        )
        detail = self._autopay_consent_detail()
        ...
        try:
            ...
            if provider_unavailable_is_retryable:
                raise _RetryProviderEvidenceUnavailable() from exc
    ...
    def _normalized_items(self, data: BillingInvoiceCreate, studio_id: str) -> list[dict[str, Any]]:
        ...
        for item in source:
            ...
            self._validate_invoice_item_refs(row, studio_id)
    ...
    def _invoice_generation(
        ...
        account = self.connect_accounts.by_stripe_account(invoice["stripe_account_id"])
        if (
            not account
    ...
    def _invoice_step_plan(
        ...
        *,
        ...
        invoice_step = {
            ...
            "stripe_idempotency_key": build_idempotency_key(
        ...
        *,
        ...
        try:
            provider_invoice = self.stripe_service_cls().create_connected_invoice(
                ...
                due_date=self._date_to_epoch(due_date) if due_date else None,
                days_until_due=7,
                metadata={
                    "studio_id": context.studio_id,
                    "payer_id": payer["id"],
        ...
        *,
        ...
        try:
            ...
            for item, provider_item_id in zip(items, item_ids, strict=True):
                ...
                self._insert_invoice_item_once(row)
                saved = self._local_item_by_provider_id(
                    context.studio_id,
                    provider_item_id,
                )
                self._verify_local_item(saved, row)
            projected = self.projector.update_invoice_from_stripe(
                invoice["id"],
                context.studio_id,
                provider_invoice,
                context.stripe_connected_account_id,
    ...
    def _finish_local_invoice(
        ...
        recompute_payer_balance(self.supabase, context.studio_id, invoice.get("payer_id"))
        ...
        *,
        ...
        try:
            provider = self.stripe_service_cls().finalize_connected_invoice(
                ...
                studio_id=context.studio_id,
                invoice_id=str(invoice["stripe_invoice_id"]),
                idempotency_key=build_idempotency_key("invoice-finalize", context.operation_id),
            )
        except StripeMutationBlocked:
            operations.transition(
                context,
