...
class BillingPlanManager:
    ...
    async def sync_plan(
        ...
        plan = self._get_plan_or_404(plan_id, studio_id)
        ...
        if disposition == "replay":
            ...
            return self._plan_response(projected, account)
    ...
    def _run_product_update_only(
        ...
        try:
            product = self.stripe_service_cls().update_connected_product(
                ...
                idempotency_key=build_idempotency_key("plan-sync", context.operation_id, "product"),
            )
    ...
    def _execute_price_step(
        ...
        *,
        ...
        recurring, _ = self._stripe_recurring_for_interval(
    ...
    def _two_step_plan(
        ...
        product_key = build_idempotency_key("plan-sync", context.operation_id, "step-1-product")
        price_key = build_idempotency_key("plan-sync", context.operation_id, "step-2-price")
        steps = [
            {
                "step_name": "product",
    ...
    def _project_two_step_result(
        ...
        *,
        ...
        recurring, interval_count = self._stripe_recurring_for_interval(
            plan.get("billing_interval") or "monthly"
        )
        price_payload = {
            "studio_id": context.studio_id,
    ...
    def _load_projected_plan(
        ...
        plan = self._get_plan_or_404(plan_id, context.studio_id)
        if (
            plan.get("status") != "active"
    ...
    async def archive_plan(
        ...
        self._get_plan_or_404(plan_id, studio_id)
        result = (
            self.supabase.table("billing_plans")
