...
def build_owner_kpi_summary(
    ...
    active_enrollments = [
        row for row in data.get("billing_enrollments", []) if row.get("status") == "active"
    ]
    ...
    capacity_total = sum(
        int(row.get("capacity") or 0) for row in sessions_30 if row.get("capacity")
    )
    capacity_attendance = sum(
        attendance_by_session.get(row.get("id"), 0) for row in sessions_30 if row.get("capacity")
    )
    mrr_cents = sum(
        _monthly_equivalent_cents(plans_by_id.get(row.get("billing_plan_id")))
    ...
    enrolled_leads = sum(
        1 for lead in leads if lead.get("stage") == "enrolled" or lead.get("converted_student_id")
    )
    ...
    return [
        {
            ...
            "value": len(active_students),
            "context": "Students with active or trialing status and no deleted_at.",
        },
        {
            "metric": "new_students_30_days",
            "value": new_students_30,
            "context": "Students whose membership_start_date or created_at is within 30 days.",
        },
        {
            "metric": "visits_30_days",
            "value": visits_30,
            "context": "Non-absent attendance records tied to sessions in the last 30 days.",
        },
        {
            "metric": "unique_attendees_30_days",
            "value": len(unique_attendees_30),
            "context": "Distinct students with at least one non-absent visit in the last 30 days.",
        },
        {
            "metric": "avg_visits_per_active_student_30_days",
            "value": round(visits_30 / len(active_students), 2) if active_students else 0,
            "context": "30-day visits divided by active student count.",
        },
        {
            "metric": "class_utilization_30_days",
            "value": round(capacity_attendance / capacity_total, 4) if capacity_total else "",
            "context": "Attendance divided by capacity for non-canceled sessions with capacity.",
        },
        {
            "metric": "active_pipeline_leads",
            "value": active_leads,
            "context": "Leads still before enrolled or closed_lost.",
        },
        {
            "metric": "new_leads_30_days",
            "value": new_leads_30,
            "context": "Leads created in the last 30 days.",
        },
        {
            "metric": "lead_conversion_rate",
            "value": round(enrolled_leads / total_leads, 4) if total_leads else "",
            "context": "Enrolled or converted leads divided by all leads.",
        },
        {
            "metric": "estimated_mrr_cents",
            "value": mrr_cents,
            "context": "Monthly-equivalent value of active billing enrollments.",
        },
        {
            "metric": "open_invoice_exposure_cents",
            "value": open_invoice_cents,
            "context": "Unpaid amount on open, uncollectible, or partially refunded invoices.",
        },
        {
            "metric": "failed_payment_amount_30_days_cents",
            "value": failed_payment_cents,
            "context": "Failed payment amount created in the last 30 days.",
        },
    ]
...
def build_first_90_days_onboarding(
    ...
    for student in data.get("students", []):
        ...
        visits_to_date = _count_events(
            events, student_id=student["id"], start=start_date, end=today
        )
        visits_first_7 = _count_events(
            events,
            student_id=student["id"],
            start=start_date,
            end=min(today, start_date + timedelta(days=6)),
        )
        visits_first_30 = _count_events(
            events,
            student_id=student["id"],
            start=start_date,
            end=min(today, start_date + timedelta(days=29)),
        )
        visits_first_90 = _count_events(
            events,
            student_id=student["id"],
            start=start_date,
            end=min(today, start_date + timedelta(days=89)),
        )
        status, action = _onboarding_status(
            days_since_start, visits_to_date, visits_first_7, visits_first_30
        )
...
def build_lead_quality_after_enrollment(
    ...
    for row in groups.values():
        ...
        row["lead_conversion_rate"] = (
            round(int(row["enrolled_or_converted_leads"]) / total, 4) if total else ""
        )
        row["avg_first_30_day_visits_per_converted_student"] = (
            round(int(row["first_30_day_visits"]) / converted, 2) if converted else ""
        )
        row["avg_lifetime_payment_per_converted_student_cents"] = (
            round(int(row["lifetime_payment_cents"]) / converted) if converted else ""
        )
        row["active_converted_student_rate"] = (
            round(int(row["active_converted_students"]) / converted, 4) if converted else ""
        )
        rows.append(row)
    return sorted(
        rows,
        key=lambda row: (
            int(row["lifetime_payment_cents"]),
            int(row["enrolled_or_converted_leads"]),
        ),
        reverse=True,
    )
