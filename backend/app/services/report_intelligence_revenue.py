...
def build_revenue_leakage(
    ...
    active_students = [
        student for student in data.get("students", []) if _is_active_student(student)
    ]
    ...
    for student in active_students:
        active_enrollments = [
            row
            for row in enrollments_by_student.get(student["id"], [])
            if row.get("status") in {"pending", "active", "paused"}
        ]
        if not active_enrollments:
            rows.append(
                _leakage_row(
                    "active_student_without_billing",
                    "high",
                    student,
                    detail="Active student has no active billing enrollment.",
                    action="Attach the student to a payer and billing plan.",
                )
            )
    ...
    for enrollment in data.get("billing_enrollments", []):
        ...
        if not payer:
            rows.append(
                _leakage_row(
                    "enrollment_without_payer",
                    "high",
                    student,
                    enrollment=enrollment,
                    detail="Billing enrollment has no payer.",
                    action="Assign a family payer before the next billing run.",
                )
            )
        if not enrollment.get("next_bill_on") and enrollment.get("status") == "active":
            rows.append(
                _leakage_row(
                    "active_enrollment_missing_next_bill_date",
                    "medium",
                    student,
                    enrollment=enrollment,
                    payer=payer,
                    detail="Active billing enrollment has no next_bill_on date.",
                    action="Review the billing schedule and set the next bill date.",
                )
            )
        ...
        if billing_status in BILLING_RISK_STATUSES:
            rows.append(
                _leakage_row(
                    "billing_status_needs_attention",
                    "high",
                    student,
                    enrollment=enrollment,
                    payer=payer,
                    detail=f"Billing status is {billing_status}.",
                    action="Follow up with payer and resolve payment method or balance.",
                )
            )
    ...
    for invoice in data.get("invoices", []):
        unpaid = max(
            0,
            int(invoice.get("amount_due_cents") or 0) - int(invoice.get("amount_paid_cents") or 0),
        )
...
def build_family_account_health(
    ...
    for payer in payers:
        student_ids = {
            row.get("student_id")
            for row in enrollments_by_payer.get(payer.get("id"), [])
    ...
    for guardian_id, links in links_by_guardian.items():
        ...
        rows.append(
            _family_row(
                key,
                f"{guardian.get('first_name') or ''} {guardian.get('last_name') or ''}".strip()
                or "Unnamed guardian",
                guardian,
                [
                    students_by_id[link.get("student_id")]
                    for link in links
                    if link.get("student_id") in students_by_id
                ],
                events,
                enrollments_by_student,
                payers_by_id,
    ...
    return sorted(
        rows,
        key=lambda row: (int(row["priority_score"]), int(row["active_students"])),
        reverse=True,
    )
