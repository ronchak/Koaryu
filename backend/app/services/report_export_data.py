...
INTELLIGENCE_INPUT_COLUMNS: Mapping[str, Mapping[str, tuple[str, ...]]] = _manifest(
    owner_kpi_summary=(
        (
            "students",
            _columns(
                "id", "studio_id", "status", "membership_start_date", "deleted_at", "created_at"
            ),
        ),
        ...
        (
            "invoices",
            _columns("id", "studio_id", "status", "amount_due_cents", "amount_paid_cents"),
        ),
    ...
    revenue_leakage=(
        ...
        (
            "payments",
            _columns(
                "id", "studio_id", "payer_id", "invoice_id", "status", "amount_cents", "created_at"
            ),
        ),
    ),
    ...
    instructor_staff_impact=(
        (
            "leads",
            _columns("id", "studio_id", "assigned_staff_id", "stage", "converted_student_id"),
        ),
        (
            "sessions",
            _columns(
    ...
    data_hygiene_readiness=(
        ...
        (
            "memberships",
            _columns("id", "studio_id", "student_id", "status", "current_belt_rank_id"),
        ),
    ...
    def fetch_intelligence_dataset(
        ...
        for source_key in report.source_keys:
            ...
            if columns is None or spec is None or spec.provider != "postgrest":
                raise RuntimeError(
                    f"Invalid intelligence source manifest entry: {report.id}/{source_key}"
                )
            if source_key == "student_guardians":
                student_ids = [row["id"] for row in dataset.get("students", []) if row.get("id")]
                dataset[source_key] = (
