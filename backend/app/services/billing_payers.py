...
class BillingPayerManager:
    ...
    def _sync_payer_customer(self, payer: dict[str, Any], account: dict[str, Any]) -> dict[str, Any]:
        """Legacy multi-call slice retained for implicit create/update, invoice, and enrollment callers.

        Workstream 3D migrates only the explicit payer.sync route. These callers
        must move to their owning planned workflows before customer mutations are
        exposed live; they must not call the V27 payer.sync parent from inside
        another workflow.
        """
