...
if (enabled && !["localhost", "127.0.0.1"].includes(new URL(origin).hostname))
  throw new Error("Recovery tests require loopback preview.");
...
check("a restored older build offers refresh while preserving the open form", async ({ page }) => {
  ...
  await page.route(`${origin}/api/version`, (route) =>
    route.fulfill({
      json: {
...
check("same-path history changes do not create a false navigation timeout", async ({ page }) => {
  ...
  await page.waitForFunction(() =>
    performance
      .getEntriesByName("koaryu.measured.navigation_complete")
      .some((entry) => (entry as PerformanceMark).detail?.route === "schedule"),
  );
  const failures = () =>
    page.evaluate(() =>
      performance
        .getEntriesByName("koaryu.measured.navigation_failure")
        .map((entry) => ({ at: entry.startTime, detail: (entry as PerformanceMark).detail })),
    );
  const before = await failures();
  const intents = () =>
    page.evaluate(() =>
      performance
        .getEntriesByName("koaryu.navigation.intent")
        .map((entry) => ({ at: entry.startTime, detail: (entry as PerformanceMark).detail })),
    );
