...
dashboardWidgetTest(
  "keeps a lifted widget under the pointer and settles into its placeholder",
  async ({ page }, testInfo) => {
    ...
    await expect
      .poll(async () => widget.evaluate((node) => getComputedStyle(node).position))
      .toBe("fixed");
    const lifted = await widget.boundingBox();
    const initialPlaceholder = await page
      .locator('[data-widget-placeholder="student_pulse"]')
      .boundingBox();
    ...
    const settledPlaceholder = await page
      .locator('[data-widget-placeholder="student_pulse"]')
      .boundingBox();
...
dashboardWidgetTest(
  "previews corner resize continuously and persists a supported footprint",
  async ({ page }, testInfo) => {
    await page.setViewportSize({ width: 1440, height: 1000 });
    await signInToPreview(page);
    await page.getByRole("button", { name: "Customize", exact: true }).click();
    ...
    const widget = page.locator('[data-widget-id="student_pulse"]');
    const storedLayoutBeforeSelection = await page.evaluate(() =>
      Object.entries(localStorage).filter(([key]) => key.startsWith("koaryu:dashboard-layout:")),
    );
      ...
      .poll(() =>
        page.evaluate(() =>
          Object.entries(localStorage).filter(([key]) =>
            key.startsWith("koaryu:dashboard-layout:"),
          ),
        ),
      )
      .toEqual(storedLayoutBeforeSelection);
...
dashboardWidgetTest(
  "keeps every resting widget body inside its footprint",
  async ({ page }, testInfo) => {
    await page.setViewportSize({ width: 1440, height: 1100 });
    await signInToPreview(page);
    const overflow = await page.locator("[data-widget-id]").evaluateAll((widgets) =>
      widgets
        .map((widget) => {
    ...
    await page.screenshot({
      path: testInfo.outputPath("resting-widget-matrix.png"),
      fullPage: true,
    });
    await page.getByRole("button", { name: "Customize", exact: true }).click();
    const bandHeights = await page
      ...
      .evaluateAll((widgets) =>
        widgets.map(
          (widget) => widget.querySelector("header")?.getBoundingClientRect().height ?? 0,
        ),
      );
    ...
    await expect
      .poll(async () =>
        page
          .locator('[data-widget-id="student_pulse"]')
          .evaluate((widget) => getComputedStyle(widget).touchAction),
      )
      .toBe("pan-y");
    await page.screenshot({
      path: testInfo.outputPath("customizing-widget-matrix.png"),
      fullPage: true,
    });
  },
);
...
dashboardWidgetTest(
  "fits widget content at every supported footprint",
  async ({ page }, testInfo) => {
    await page.setViewportSize({ width: 1440, height: 1100 });
    await signInToPreview(page);
    await page.getByRole("button", { name: "Customize", exact: true }).click();

    const widgetIds = await page
      .locator("[data-widget-id]")
      .evaluateAll((widgets) =>
        widgets
    ...
    for (const id of [
      ...
      if (!catalog || (await widget.count()) === 0 || !catalog.allowedSizes.includes("1x2"))
        continue;
      const handle = widget.getByRole("button", { name: new RegExp(`Resize ${catalog.title}`) });
...
dashboardWidgetTest(
  "maps tablet drops to the visual target and avoids hybrid resize footprints",
  async ({ page }) => {
    ...
    await page.mouse.move(startX, startY);
    await page.mouse.down();
    await page.mouse.move(startX + billingBefore.width, startY + billingBefore.height * 2, {
      steps: 8,
    });
