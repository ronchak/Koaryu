...
liveStatefulTest(
  "belt ladder sync stays single-request and preserves existing ranks",
  async ({ page }) => {
    ...
    expect(
      studioName,
      "expected KOARYU_E2E_STUDIO_NAME for a disposable live test studio",
    ).toBeTruthy();
    ...
    expect(
      nonSyncBeltMutations.length,
      "expected at most one ladder-creation mutation",
