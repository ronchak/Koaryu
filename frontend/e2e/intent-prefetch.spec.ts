...
if (enabled && !["localhost", "127.0.0.1"].includes(new URL(origin).hostname))
  throw new Error("Prefetch tests require loopback preview.");
const check = enabled ? test : test.skip;

for (const intent of ["hover", "focus"] as const) {
  check(
    `production navigation prefetches Billing on ${intent} without navigating`,
    async ({ page }) => {
      const requests: string[] = [];
      page.on("request", (request) => {
        if (new URL(request.url()).pathname === "/billing") requests.push(request.url());
      });
