...
previewTest(
  "serializes two same-tick attendance toggles from the latest committed state",
  async ({ page }) => {
    await page.addInitScript(() => {
      window.localStorage.removeItem("koaryu:attendance");
    });
...
previewTest(
  "clearing attendance removes all duplicate rows and stays cleared after reopen",
  async ({ page }) => {
    await page.addInitScript(() => {
    ...
    await expect
      .poll(async () =>
        page.evaluate(() => {
          ...
          return rows.filter(
            (row: { session_id?: string; student_id?: string }) =>
    ...
    expect(
      busyOnReopen,
      "same-session reopen must commit pending before refresh effects settle",
    ).toBe(true);
    await expect(page.getByTestId("attendance-summary")).toHaveAttribute("aria-busy", "false");
    await expect(page.getByRole("button").filter({ hasText: "Aiko Tanaka" })).toContainText(
      "Check in",
    );
  },
);
