...
export function useStoreProgramActions({
  ...
  const refreshPrograms = useCallback(async (
    ...
    const ownsRead = () => programScopeRef.current === scope && scope.sequence === sequence
    ...
    const pending = (async () => {
      ...
      const error = new Error("Programs changed while loading. Refresh to see the latest records.");
      if (ownsRead()) {
    ...
    usageRequestsRef.current.set(key, {
      identity, promise: pending,
      isCurrent: () => ownsRead() && owner.isCurrent() && revision === scope.revision,
    });
    return pending;
  }, [beginLiveAuthRequest, isPreviewMode, persistPrograms, programScopeRef, programsLoadedRef, setProgramsLoadError, setProgramsUsageLoaded, setProgramsUsageLoadError]);
  ...
  const commitLiveProgram = useCallback(async (
    ...
    return result;
  }, [beginLiveAuthRequest, programScopeRef, persistPrograms, programsRef, refreshBeltsRef, currentLadderIdRef]);
