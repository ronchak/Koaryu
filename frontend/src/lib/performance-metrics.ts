...
export function parseMetricBatch(value: unknown): MetricBatch | null {
  ...
  if (Object.keys(batch).some(key => !["version", "events"].includes(key))
    || typeof batch.version !== "string" || !/^[0-9a-f]{40}$/.test(batch.version)
    || !Array.isArray(batch.events) || !batch.events.length || batch.events.length > 10) return null;
  for (const event of batch.events) {
    if (!event || typeof event !== "object" || Object.keys(event).length !== 5
      || !METRIC_ROUTES.includes(event.route) || !METRIC_NAMES.includes(event.name)
      ...
      || event.value > (event.name === "CLS" ? 10 : 180_000)) return null;
  }
