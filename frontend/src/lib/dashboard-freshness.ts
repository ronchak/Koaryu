...
export function markDashboardFactsChanged(now = Date.now()) {
  changedAt = now;
  try { if (typeof window !== "undefined") window.localStorage.setItem(CHANGE_KEY, String(now)); } catch { /* Local protection still applies. */ }
}
...
export function needsFreshDashboardFacts(now = Date.now()): boolean {
  ...
  const recent = Math.max(changedAt <= now ? changedAt : -Infinity, shared <= now ? shared : -Infinity);
