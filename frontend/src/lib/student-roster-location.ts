...
export function saveRosterReturn(state: RosterReturnState) {
  try {
    sessionStorage.setItem(
      STORAGE_KEY,
      JSON.stringify({ ...state, history: state.history.slice(-20) }),
    );
  } catch {
    /* Storage can be disabled. Canonical filters still survive. */
  }
}
    ...
    if (
      ...
      !value.history.every(
        (entry) =>
          ...
          [entry[1].nextCursor, entry[1].previousCursor, entry[1].requestCursor].every(
            (cursor) => cursor === null || (typeof cursor === "string" && cursor.length <= 8192),
          ),
      ) ||
      (value.cursor !== null && (typeof value.cursor !== "string" || value.cursor.length > 8192)) ||
      !Number.isFinite(value.scroll) ||
      value.scroll < 0 ||
      value.scroll > 1_000_000 ||
      typeof value.focusId !== "string" ||
      value.focusId.length > 100
    )
      return null;
    return value;
  } catch {
...
export function consumeRosterReturn(restored: RosterReturnState): void {
  try {
    ...
    /* Optional return context must not block navigation. */
  }
}
