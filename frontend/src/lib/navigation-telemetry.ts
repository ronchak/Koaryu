...
type Attempt = {
  route: PerformanceMetric["route"];
  navigation: PerformanceMetric["navigation"];
  started: number;
  stages: Set<string>;
};

/** The clock starts at user intent, before middleware or route loading. */
export function createNavigationTimer(
  emit: (event: PerformanceMetric) => void,
  now = () => performance.now(),
) {
    ...
    start(
      ...
      if (active)
        emit({
          ...
          value: Math.max(0, now() - active.started),
          outcome: "superseded",
        });
...
export function configurePerformanceCollection(build: string | null, environment: string) {
  ...
  sampled =
    ...
    productionPerformanceEnabled(environment, process.env.NEXT_PUBLIC_PREVIEW_MODE === "true") &&
    Boolean(build) &&
    Math.random() < 0.1;
}
...
export function flushPerformanceMetrics() {
  ...
  void fetch("/api/performance", {
    ...
    body: JSON.stringify({ version, events: batch }),
    keepalive: true,
...
export function startMeasuredNavigation(
  path: string,
  navigation: PerformanceMetric["navigation"],
  started?: number,
) {
  ...
  try {
    ...
    performance.mark("koaryu.navigation.intent", {
      startTime: started ?? performance.now(),
      detail: { route, navigation },
    });
  } catch {
    /* Native marks are optional diagnostics. */
  }
