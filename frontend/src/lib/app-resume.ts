...
export function createResumeCheck({
  ...
  return {
    check(): Promise<void> {
      ...
      pending = (async () => {
        if (!loaded.commit_sha) {
          lastCheck = now();
          if (!disposed) onCurrent();
          return;
        }
        try {
          const current = await readVersion();
          if (disposed || !validAppVersion(current) || current.environment !== loaded.environment)
            return;
          lastCheck = now();
