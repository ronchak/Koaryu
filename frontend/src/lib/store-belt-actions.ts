...
export function useStoreBeltActions({
  ...
  const pendingRankTransition = useCallback(
    (kind: RankTransitionKind, data: PromoteStudent | DemoteStudent) => {
      const key = `${kind}:${data.student_id}`;
      const fingerprint = rankTransitionFingerprint(data);
      const existing =
        pendingRankTransitionsRef.current.get(key) ??
  ...
  const setCurrentLadder = useCallback(
  ...
  const setBeltRanks = useCallback(
    async (ranks: BeltRank[], options: { ladderId: string; subRankTerm?: string }) => {
      ...
      if (isPreviewMode) {
        ...
        const nextPreviewLadder = buildPreviewBeltLadderFromRanks(beltLaddersRef.current, ranks, {
      ...
      const pendingSync =
        pendingLadderSyncsRef.current.get(ladder.id) ??
      ...
      if (!canCommitLiveMutation(liveRequest)) {
        throw Object.assign(new Error("Program ranks were saved in the previous workspace."), {
          committed: true,
        });
      }
  ...
  const loadPromotionHistory = useCallback(
    async (
      ...
      return loadPromotionHistoryWithCache({
        ...
        beginLiveAuthRequest,
        fetchPromotionHistory: (requestedStudentId, authToken) =>
          api.get<Promotion[]>(
  ...
  const commitPromotionHistoryItem = useCallback(
    (studentId: string, item: Promotion) => {
      commitPromotionHistoryCache(
        studentId,
        buildPromotionHistoryWithPrependedItem(promotionHistoryCacheRef.current, studentId, item),
      );
    },
    [commitPromotionHistoryCache, promotionHistoryCacheRef],
  );

  const commitLivePromotionHistoryItem = useCallback(
    (studentId: string, item: Promotion) => {
  ...
  const demoteStudent = useCallback(
    async (data: DemoteStudent) => {
      if (isPreviewMode) {
