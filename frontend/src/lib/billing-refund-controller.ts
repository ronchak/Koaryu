...
export function useBillingRefundController({
  ...
  const scope =
    identity && identityKey && canRefundPayments && token && !isPreviewMode
      ? `${identityKey}:${identity.userId}:${identity.studioId}`
      : null;
  ...
  function isPaymentRefundBlocked(paymentId: string) {
    ...
    return (
      blockedAttempts.has(attemptIdentity) ||
  ...
  async function runRefund(
    ...
    amountCents: number,
    reason: RefundReason,
    recovery?: RefundAttempt,
  ) {
    if (!identity || !token || !scope || scopeRef.current !== scope || activeOperationRef.current)
      return;
    ...
    try {
      if (
        recovery &&
        readRefundAttempt(identity, payment.id, storage)?.requestKey !== recovery.requestKey
      ) {
        throw new Error("The saved refund request changed. Reload Billing before continuing.");
      }
      requestKey = resolveRefundRequestKey(
        identity,
        payment.id,
        amountCents,
        reason,
      ...
      if (!accepted) {
        ...
        if (!isMatchingRefundResponse(result, identity, payment.id, amountCents)) {
          throw new Error(
            "The refund result could not be verified. Recover the original request before starting another refund.",
          );
        }
        ...
        showRefundMessage(
          result.status === "failed"
            ? "The refund failed. No completed refund is confirmed."
        ...
        if (!markRefundAccepted(identity, payment.id, requestKey, result.id, storage)) {
          setError(
            "The refund result was received, but its recovery confirmation could not be saved. Recover the original request before starting another refund.",
          );
          return;
        }
      } else {
        showRefundMessage(
          "The original refund request is recorded. Refreshing its payment balance.",
        );
      }
      ...
      if (result.status !== "updated") {
        setError(
          "The refund request is recorded, but its payment balance could not refresh. Use Refresh payment before starting another refund.",
        );
        return;
      }
      if (!clearRefundRequestKey(identity, payment.id, storage, requestKey)) {
        setError(
          "The payment balance refreshed, but its saved recovery state could not be cleared. Reload Billing before taking another action.",
        );
      }
    } catch (error) {
      if (!current()) return;
      ...
      if (
        ...
        !recovery &&
        requestKey &&
        (isTerminalBillingIdempotencyError(error) || isDefinitiveRefundRejection(error))
      ) {
        if (!clearRefundRequestKey(identity, payment.id, storage, requestKey)) {
          setError(
            "The refund was rejected, but its saved request state could not be cleared. Reload Billing before correcting and retrying.",
          );
          return;
        }
      }
  ...
  async function recoverRefund(payment: BillingPayment) {
    if (!canRefundPayments || isPreviewMode || !identity || isPaymentRefundBlocked(payment.id))
      return;
    try {
      ...
      setError(
        "The saved refund request could not be read. Reload Billing before taking another action.",
      );
    }
  }
