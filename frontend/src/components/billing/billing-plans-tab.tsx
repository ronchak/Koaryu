            ...
            >
              <div className="min-w-0">
                ...
                <p className="mt-1 text-xs text-muted">
                  {plan.description || intervalLabel(plan.billing_interval)}
                </p>
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {plan.programs.length ? (
                    plan.programs.map((program) => (
                      <ProgramChip key={program.program_id} program={program} />
                    ))
                  ) : (
                ...
                {plan.pending_reason ? (
                  <p className="mt-2 text-xs text-warning">{plan.pending_reason}</p>
                ) : null}
              </div>
              ...
              <div className="max-w-[220px] text-right text-xs text-muted">
                <p className="mb-1 text-left text-xs font-medium text-muted md:hidden">
                  Provider references
                </p>
