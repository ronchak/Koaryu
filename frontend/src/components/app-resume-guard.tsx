...
export function AppResumeGuard({ loaded }: { loaded: AppVersion }) {
  ...
  useEffect(() => {
    ...
    const onPageShow = (event: PageTransitionEvent) => {
      if (event.persisted) void checker.check();
    };
    const onVisibility = () => {
    ...
    const onOnline = () => {
      if (!document.hidden) void checker.check();
    };
  ...
  return (
    <aside
      role="status"
      className="fixed bottom-4 left-1/2 z-[80] flex w-[calc(100%-2rem)] max-w-lg -translate-x-1/2 items-center gap-4 rounded-lg border border-border bg-surface p-4 text-sm text-text-primary shadow-lg"
    >
      ...
      <button
        ...
        disabled={pending > 0}
        onClick={() => {
          if (!pendingCommands()) window.location.reload();
        }}
