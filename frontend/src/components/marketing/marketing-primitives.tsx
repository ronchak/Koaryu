...
export interface MarketingBrandLinkProps extends Omit<MarketingLinkProps, "children"> {
  children?: ReactNode;
}
...
export type MarketingMenuButtonProps = Omit<ButtonHTMLAttributes<HTMLButtonElement>, "children">;
...
export function MarketingBrandLink({
  ...
  return (
    <Link {...props} aria-label={ariaLabel} className={joinClassNames(styles.wordmark, className)}>
      {children}
    </Link>
  );
}

export function MarketingNavLink({ children, className, ...props }: MarketingLinkProps) {
  return (
...
export function MarketingActionLink({
  ...
  const variantClass = variant === "primary" ? styles.primaryAction : styles.secondaryAction;

  return (
    <Link {...props} className={joinClassNames(styles.actionLink, variantClass, className)}>
...
export function MarketingMenuButton({
  ...
  return (
    <button
      ...
      type={type}
      aria-expanded={ariaExpanded}
      aria-label={ariaLabel ?? (ariaExpanded ? "Close navigation" : "Open navigation")}
