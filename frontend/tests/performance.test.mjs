...
describe("committed dashboard readiness", () => {
  it("waits for a paint opportunity, deduplicates a generation, and drops superseded callbacks", async () => {
    const { beginDashboardNavigation, markDashboardReadiness } =
      await import("../src/lib/performance.ts");
    ...
    try {
      markDashboardReadiness("dashboard", 10, {
        shell: true,
        identity: true,
        useful: true,
        complete: false,
      ...
      frame();
      assert.equal(
        marks.some((entry) => entry.name === "koaryu.visible.complete"),
        false,
      );
      ...
      assert.deepEqual(Object.keys(marks[0].detail).sort(), [
        "identity_generation",
      ...
      assert.equal(
        complete[0].detail.navigation_generation,
        marks.filter((entry) => entry.name === "koaryu.navigation.started").at(-1).detail
          .navigation_generation,
      );
    } finally {
