...
const homeSource = readFileSync(
  new URL("../src/components/dashboard/dashboard-home.tsx", import.meta.url),
  "utf8",
);
const controllerSource = readFileSync(
  new URL("../src/lib/dashboard-page-controller.ts", import.meta.url),
  "utf8",
);
const captureSource = readFileSync(
  new URL("../scripts/capture-dashboard-performance.mjs", import.meta.url),
  "utf8",
);
...
function deferredCapture() {
  ...
  page.evaluate = async (read) => {
    if (read.name === "readVisualReadiness")
      return { ready: true, image_failed: false, lcp: 2, cls: 0 };
    if (evaluateCalls++ === 0) {
      return [
        "navigation.started",
        "visible.shell",
  ...
  const capture = captureDashboardPerformance(
    {
      ...
      backendApi: "https://backend.invalid/api/v1",
  ...
  return {
    ...
    sizes,
    lateSizes,
    snapshotWait: snapshotWait.promise,
...
describe("privacy-safe performance evidence", () => {
  it("captures a late native LCP after complete-data marks while the image is still loading", async () => {
    ...
    try {
      ...
      capture = captureDashboardPerformance(
        {
          route: "billing",
          frontendOrigin: origin,
          backendApi: origin + "/api/v1",
          storageState: "/unused-test-state",
        },
        {
          verifyDeployment: async () => ({
            verified: true,
      ...
      await page.waitForFunction(
        () =>
          performance.getEntriesByName("koaryu.visible.complete").length &&
          globalThis.__koaryuEvidence?.lcp !== null,
      );
      ...
      assert.equal(
        settled,
        false,
        "an unfinished image must prevent even a quiet early LCP from being accepted",
      );
      ...
      assert.ok(
        evidence.web_vitals.largest_contentful_paint_ms > evidence.selected_required_data_ms,
      );
      assert.deepEqual(evidence.web_vitals.observation, {
        status: "stabilized",
        ...WEB_VITALS_STABILIZATION,
      });
      assert.equal(
        evidence.requests.filter(
          (entry) => entry.initiator === "image" && entry.outcome === "complete",
        ).length,
        1,
      );
    } finally {
  ...
  it("bounds visual stabilization and rejects instead of returning an early metric", async () => {
    ...
    await assert.rejects(
      stabilizeWebVitals(page, () => ({ pending: 1, revision: 1 }), {
        timeout_ms: 100,
        quiet_window_ms: 20,
      }),
    ...
    await assert.rejects(
      stabilizeWebVitals(
        ...
        { timeout_ms: 100, quiet_window_ms: 20 },
      ),
      /stabilization timed out/,
    );
    await assert.rejects(
      stabilizeWebVitals({ evaluate: async () => ({ image_failed: true }) }, () => ({
        pending: 0,
        revision: 1,
      })),
      /visible image decoding failed/,
    );
  });
  ...
  it("restarts the quiet interval for a later LCP and a rendering request between samples", async () => {
    ...
    let revision = 0;
    const started = performance.now();
    const result = await stabilizeWebVitals(
      {
        evaluate: async () => {
          reads += 1;
      ...
      () => ({ pending: 0, revision }),
      { timeout_ms: 1_000, quiet_window_ms: 60 },
    );
  ...
  it("freezes an exhaustive request inventory before deferred size lookups settle", async () => {
    ...
    assert.deepEqual(
      pending.map((entry) => [entry.resource, entry.status]),
      [
        ["other", 202],
        ["dashboard-summary", 0],
      ],
  ...
  it("verifies the exact SHA before launching a browser", async () => {
    const order = [];
    const browser = {};
    const result = await openVerifiedBrowser(
  ...
  it("requires a ready dashboard, both successful resources, finite metrics, and zero blocks", () => {
    ...
    assert.throws(
      () =>
        validateCapturedEvidence({
          ...evidence,
          server_timing: evidence.server_timing.filter(
            (entry) => entry.resource !== "dashboard-summary",
          ),
        }),
        ...
        validateCapturedEvidence({
          ...
          server_timing: evidence.server_timing.map((entry) =>
            entry.resource === "dashboard-summary"
              ? {
                  ...entry,
                  server_timing: [{ name: "koaryu_summary_total", duration_ms: Infinity }],
                }
              : entry,
          ),
        }),
  ...
  it("reads committed marks from the current route generation rather than observer wait duration", async () => {
    ...
    const marks = [
      ...
      "visible.useful",
      "visible.complete",
      "visible.legacy-complete",
    ].map((stage, index) => ({
      name: `koaryu.${stage}`,
      startTime: index * 10,
      detail: { route: "dashboard", identity_generation: 2, navigation_generation: 3 },
    }));
    const page = {
      locator: (selector) => ({
        waitFor: async ({ state }) => {
