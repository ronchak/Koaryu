...
test("navigation IDs fit Vercel and distinguish environment and deployment at the same commit", () => {
  const env = { VERCEL_GIT_COMMIT_SHA: "a".repeat(40), VERCEL_ENV: "production", VERCEL_URL: "first.example.test" };
  ...
  assert.equal(getNavigationDeploymentId({ ...env, NEXT_DEPLOYMENT_ID: "provider-id" }), "provider-id");
