...
async function mountEditor(browser, { emptyFirst = true, mode = "production" } = {}) {
    ...
    ({ emptyFirst }) => {
      ...
      f.ladders = ["A", "B"].map((name) => ({
        ...
        ranks:
          name === "A" && emptyFirst
            ? []
            : ["White", "Blue"].map((color, index) => ({
                id: `${name}-${color}`,
                ladder_id: `ladder-${name}`,
                studio_id: "studio-a",
                name: `${color} ${name}`,
                color_hex: "#ffffff",
                is_tip: false,
      ...
      f.api = {
        get: async (path, token) => {
          ...
          if (path === "/dashboard/workspace")
            return { auth: f.auth, studio: { name: "Synthetic", timezone: "UTC" } };
          if (path.startsWith("/dashboard/bootstrap"))
            return {
              auth: f.auth,
              studio_name: "Synthetic",
              programs: f.programs,
        ...
        post: async (path, body, token) => {
          ...
          ladder.ranks = body.ranks.map((rank, index) => ({
            ...rank,
            id: rank.id ?? `saved-${index}`,
            ladder_id: ladder.id,
            studio_id: "studio-a",
          }));
  ...
  try {
    await page.waitForFunction(
      () => fixture.store?.identityReady && fixture.store.programsLoaded,
      undefined,
      { timeout: 5000 },
    );
  } catch (error) {
...
for (const mode of ["production", "development"]) {
  test(`rank edits preserve the selected ladder's whole draft (${mode})`, async () => {
    ...
    try {
      ...
      assert.deepEqual(
        first.body.ranks.map((rank) => rank.id),
        ["B-White", "B-Blue"],
      );
      ...
      await page.getByRole("button", { name: "Save ranks", exact: true }).click();
      await page.waitForFunction(
        () => fixture.writes.length === 2 && fixture.store.beltRanks[0]?.id === "B-Blue",
      );
      const second = await page.evaluate(() => fixture.writes[1]);
      assert.deepEqual(
        second.body.ranks.map((rank) => rank.id),
        ["B-Blue", "B-White"],
      );
...
test("a pending rank save owns editing and survives credential renewal", async () => {
  ...
  try {
    ...
    for (const button of await page
      .getByRole("button", { name: "Add degree", exact: true })
      .all()) {
      await expect(button).toBeDisabled();
    }
    await expect(page.getByLabel("Program", { exact: true })).toBeDisabled();
    assert.equal(
      await page.locator('[data-belt-drag-handle="A-Blue"]').getAttribute("draggable"),
      "false",
    );
    // A queued drop may still be delivered after dragging was disabled.
    const dataTransfer = await page.evaluateHandle(() => new DataTransfer());
    await page
      .locator('[data-belt-drag-handle="A-Blue"]')
    ...
    assert.deepEqual(await page.evaluate(() => fixture.store.beltRanks.map((rank) => rank.id)), [
      "A-White",
      "A-Blue",
    ]);
    ...
    assert.deepEqual(
      await page.evaluate(() => fixture.store.beltRanks),
      [],
      "a previous identity's acknowledged save cannot restore private state",
    );
...
test("rejected saves retain the draft, while committed refresh failures do not invite another save", async () => {
  ...
  try {
    ...
    await page.getByRole("button", { name: "Save ranks", exact: true }).click();
    await expect(
      page.getByText("Could not save ladder changes. Please try again.", { exact: true }),
    ).toBeVisible();
    await expect(page.getByRole("button", { name: "Save ranks", exact: true })).toBeEnabled();
    assert.equal(
      await page.getByRole("button", { name: "Edit sub-rank term" }).innerText(),
      "Degree",
    );
    ...
    await page.getByRole("button", { name: "Save ranks", exact: true }).click();
    await expect(
      page.getByText("Program ranks saved. Refresh needed to show the latest student ranks.", {
        exact: true,
      }),
    ).toBeVisible();
    await expect(page.getByRole("button", { name: "Save ranks", exact: true })).toHaveCount(0);
    ...
    assert.equal(
      ...
      before,
      "a stale draft target must be rejected before transport",
    );
  } finally {
    await browser.close();
  }
