...
const { fetchStudentPage, StudentRosterCursorError } =
...
describe("student roster API transport", () => {
  it("preserves a FastAPI cursor detail through api.get and fetchStudentPage", async () => {
    ...
    await assert.rejects(fetchStudentPage("token", { page: 2, cursor: "opaque" }), (error) => {
    ...
    await assert.rejects(api.get("/students?page=2", "token"), (error) => {
  ...
  it("rejects malformed or non-409 cursor details without changing ordinary messages", async () => {
    ...
    await assert.rejects(fetchStudentPage("token"), (error) => {
      assert.ok(error instanceof ApiError);
