...
function renderGate(
  profile,
  programs = { programsLoaded: true, programsLoadError: null, refreshPrograms: async () => [] },
) {
  ...
  const imports = {
    ...
    "@/components/dashboard-identity-skeleton": {
      DashboardIdentitySkeleton: ({ error }) =>
        error ? "identity-error-retry" : "neutral-workspace",
    },
    "@/components/sidebar": { Sidebar: () => "tenant-navigation" },
    "@/components/theme-provider": { useTheme: () => ({ navigationPlacement: "side" }) },
    "@/components/account/legal-name-blocking-screen": {
      LegalNameBlockingScreen: () => "legal-name-form",
    },
    "@/lib/store": {
      StoreProvider: passthrough,
      useStudioStore: () => profile,
  ...
  return renderToStaticMarkup(
    React.createElement(
      ...
      null,
      React.createElement("p", null, "protected-records"),
    ),
  );
}
const profile = {
...
test("program metadata warnings stay behind identity and legal-name gates", () => {
  const unavailablePrograms = {
    programsLoaded: false,
    programsLoadError: "Metadata unavailable",
    refreshPrograms: async () => [],
  };
  assert.doesNotMatch(renderGate(profile, unavailablePrograms), /Program options are unavailable/);
  assert.doesNotMatch(
    renderGate(
      { ...profile, identityReady: true, staffProfilesAvailable: true },
      unavailablePrograms,
    ),
    /Program options are unavailable/,
  );
  assert.match(
    renderGate({ ...profile, identityReady: true, currentRole: "admin" }, unavailablePrograms),
    /Program options are unavailable/,
  );
});
