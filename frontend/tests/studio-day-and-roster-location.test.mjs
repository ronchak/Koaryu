...
test("return destinations are internal, bounded roster queries", () => {
  for (const href of ["https://evil.test", "//evil.test", "/students/../../evil", "/students\\evil", "/students#x"]) {
    assert.equal(safeStudentsReturn(href), "/students");
  }
  assert.equal(safeStudentsReturn("/students?status=active&token=secret&sort=name"), "/students?status=active&sort=name");
});
test("roster return context is bounded, expiring, and scoped to identity", () => {
  const items = new Map();
  globalThis.sessionStorage = { getItem: key => items.get(key), setItem: (key, value) => items.set(key, value) };
  const state = { scope: "user:studio:admin:1", href: "/students?status=active", page: 3, cursor: "signed-cursor", history: [], scroll: 120, focusId: "student-1", savedAt: Date.now() };
