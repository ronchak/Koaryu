...
const bundle = (mode, options = {}) =>
  buildBundle(mode, { ...options, legacyBootstrapFixture: true });
    ...
    try {
      ...
      await page.evaluate(() => {
        ...
        const fixture = (window.fixture = {
          ...
          attendanceRows: [],
          attendanceWaiters: [],
          holdAttendance: false,
          session: {
            access_token: "fixture-token-a",
            user: { id: "user-a", email: "a@example.test", user_metadata: {} },
          },
          maxListeners: 0,
          listeners,
        });
        fixture.emit = (event, session) => {
        ...
        fixture.supabase = {
          auth: {
            ...
            onAuthStateChange: (callback) => {
              ...
              queueMicrotask(() => {
                if (listeners.has(callback)) callback("INITIAL_SESSION", fixture.session);
              });
              fixture.maxListeners = Math.max(fixture.maxListeners, listeners.size);
          ...
          get: async (path, token) => {
            ...
            if (path === "/dashboard/bootstrap?allow_partial=true") {
              ...
              if (fixture.holdBootstrap)
                await new Promise((resolve) => fixture.bootstrapWaiters.push(resolve));
              if (fixture.bootstrapError)
                throw Object.assign(new Error(fixture.bootstrapError.message), {
                  status: fixture.bootstrapError.status,
                });
              return {
                auth: {
                ...
                studio_name: `Studio ${session.user.id}`,
                students: [],
                programs: [],
                leads: [],
                belt_ladders: [],
                ...
                sessions: [
                  {
                    id: `session-${date}`,
                    date,
                    name: `Class ${date} ${token}`,
                    start_time: "09:00",
                    end_time: "10:00",
                    attendance_count: fixture.attendanceRows.filter(
                      (row) => row.session_id === `session-${date}`,
                    ).length,
                  },
                ],
                templates: [],
                attendance: fixture.attendanceRows.filter(
                  (row) => row.session_id === `session-${date}`,
                ),
              };
              if (fixture.holdSchedule)
                await new Promise((resolve) => fixture.scheduleWaiters.push(resolve));
              return snapshot;
            }
            if (path.startsWith("/programs")) {
              if (fixture.holdPrograms)
                await new Promise((resolve) => fixture.programWaiters.push(resolve));
              return [];
            }
        ...
        fixture.api.post = async (path, body, token) => {
          if (path === "/schedule/attendance") {
            fixture.requests.push({ path, token });
            if (fixture.holdAttendance)
              await new Promise((resolve) => fixture.attendanceWaiters.push(resolve));
            const record = {
              ...body,
              id: "attendance-persisted",
              studio_id: "fixture-studio",
              checked_in_at: "2026-09-04T12:00:00Z",
      ...
      const initial = await page.evaluate(() => ({
        ...
        observations: window.fixture.observations,
        listeners: window.fixture.listeners.size,
        max: window.fixture.maxListeners,
      }));
      assert.deepEqual(errors, []);
      assert.equal(
        initial.requests.filter((r) => r.path === "/dashboard/bootstrap?allow_partial=true").length,
        1,
        "role commit must not bootstrap a second time",
      );
      ...
      assert.ok(
        initial.observations.slice(firstReady).every((o) => o.ready),
        "role commit cannot reset identity readiness",
      );
      assert.equal(
        initial.requests.filter((r) => r.path.startsWith("/programs")).length,
        0,
        "metadata bootstrap must not globally enrich program usage",
      );
      await page.evaluate(() =>
        fixture.emit("TOKEN_REFRESHED", {
          ...fixture.session,
          access_token: "fixture-token-renewed",
        }),
      );
      ...
      assert.equal(
        await page.evaluate(
          () =>
            fixture.requests.filter((r) => r.path === "/dashboard/bootstrap?allow_partial=true")
              .length,
        ),
        1,
      );
      ...
      await page.waitForFunction(
        () => fixture.store.currentRole === null && fixture.store.currentStudioId === null,
      );
      await page.evaluate(() =>
        fixture.emit("SIGNED_IN", {
          access_token: "fixture-token-b",
          user: { id: "user-b", email: "b@example.test", user_metadata: {} },
        }),
      );
      ...
      await page.evaluate(() => {
        ...
        fixture.emit("SIGNED_IN", {
          access_token: "fixture-token-c",
          user: { id: "user-c", email: "c@example.test", user_metadata: {} },
        });
      });
      ...
      await page.evaluate(() => {
        ...
        fixture.emit("SIGNED_IN", {
          access_token: "fixture-token-d",
          user: { id: "user-d", email: "d@example.test", user_metadata: {} },
        });
      });
      ...
      assert.equal(
        await page.evaluate(() => fixture.store.currentStudioId),
        "studio-user-d",
        "superseded bootstrap must never commit",
      );
      ...
      await page.evaluate(() => {
        fixture.studioOverride = "studio-changed";
        fixture.emit("USER_UPDATED", fixture.session);
      });
      await page.waitForFunction(
        () =>
          fixture.store.staffProfilesAvailable &&
          fixture.store.currentStudioId === "studio-changed" &&
          !fixture.store.programsUsageLoaded,
      );
      await page.evaluate(() => {
      ...
      await page.evaluate(async () => {
        fixture.programWaiters[0]();
        await fixture.oldPrograms;
      });
      assert.equal(await page.evaluate(() => fixture.store.programsUsageLoaded), false);
      await page.evaluate(async () => {
        fixture.programWaiters[1]();
        await fixture.newPrograms;
      });
      ...
      const windowsBefore = await page.evaluate(
        () => fixture.requests.filter((r) => r.path.startsWith("/schedule/window")).length,
      );
      await page.evaluate(async () => {
        await Promise.all([
      ...
      assert.equal(
        await page.evaluate(() =>
          fixture.store.sessions.some((s) => s.id === "session-2026-10-01"),
        ),
        true,
      );
      await page.evaluate(() => {
        fixture.holdSchedule = true;
        fixture.oldRange = fixture.store
          .refreshScheduleRange("2026-11-01", "2026-11-07", "materialize")
          .catch((error) => error.message);
      });
      await page.waitForFunction(() => fixture.scheduleWaiters.length === 1);
      await page.evaluate(() => {
        fixture.newRange = fixture.store.refreshScheduleRange(
          "2026-12-01",
          "2026-12-07",
          "materialize",
        );
      ...
      const staleRange = await page.evaluate(async () => {
        await fixture.newRange;
        return fixture.oldRange;
      });
      assert.match(staleRange, /superseded/);
      await page.waitForFunction(() =>
        fixture.store.sessions.some((s) => s.id === "session-2026-12-01"),
      );
      assert.equal(
        await page.evaluate(() =>
          fixture.store.sessions.some((s) => s.id === "session-2026-11-01"),
        ),
        false,
      );
      ...
      await page.evaluate(() => {
        ...
        fixture.attendanceRaceRange = fixture.store
          .refreshScheduleRange("2026-12-01", "2026-12-07", "materialize")
          .catch((error) => error.message);
      });
      await page.waitForFunction(() => fixture.scheduleWaiters.length === 2);
      await page.evaluate(() => {
        fixture.attendanceMutation = fixture.store.toggleCheckIn(
          "session-2026-12-01",
          "student-fixture",
          "Fixture Student",
        );
        fixture.afterAttendanceRange = fixture.store.refreshScheduleRange(
          "2027-01-01",
          "2027-01-07",
          "materialize",
        );
      ...
      assert.equal(
        await page.evaluate(() =>
          fixture.requests.some((r) => r.path.includes("start_date=2027-01-01")),
        ),
        false,
        "range waits for attendance mutation settlement",
      );
      await page.evaluate(async () => {
        fixture.attendanceWaiters[0]();
        await Promise.all([
          fixture.attendanceMutation,
          fixture.afterAttendanceRange,
          fixture.attendanceRaceRange,
        ]);
      });
      await page.waitForFunction(() =>
        fixture.store.attendance.some((row) => row.id === "attendance-persisted"),
      );
      assert.equal(
        await page.evaluate(() =>
          fixture.store.sessions.some((row) => row.id === "session-2027-01-01"),
        ),
        true,
      );
      assert.equal(
        await page.evaluate(
          () =>
            fixture.store.sessions.find((row) => row.id === "session-2026-12-01")?.attendance_count,
        ),
        1,
        "refreshing another window preserves the changed attendance count",
      );
      ...
      await page.evaluate(() => {
        fixture.emit("TOKEN_REFRESHED", {
          ...fixture.session,
          access_token: "calendar-initial-renewal",
        });
        fixture.holdSchedule = false;
        fixture.scheduleWaiters[0]();
      });
      await page.waitForFunction(
        () => fixture.controller.hasLoadedRange && !fixture.controller.isRefreshingRange,
        undefined,
        { timeout: 3000 },
      );
      ...
      assert.equal(
        await page.evaluate(
          () =>
            fixture.requests.at(-1).path.startsWith("/schedule/window/materialize") &&
            fixture.requests.at(-1).token === "calendar-initial-renewal",
        ),
        true,
        "renewal preserves calendar materialization intent",
      );

      // Navigation, token renewal, and a pending attendance write share one queue.
      const attendanceWritesBefore = await page.evaluate(
        () => fixture.requests.filter((r) => r.path === "/schedule/attendance").length,
      );
      await page.evaluate(() => {
        fixture.scheduleWaiters = [];
      ...
      await page.waitForFunction(() => fixture.scheduleWaiters.length === 1);
      const navigationDate = await page.evaluate(() =>
        new URL(fixture.requests.at(-1).path, "http://fixture.local").searchParams.get(
          "start_date",
        ),
      );
      await page.evaluate((date) => {
        fixture.renewalAttendance = fixture.store.toggleCheckIn(
          `session-${date}`,
          "renewal-student",
          "Renewal Student",
        );
        fixture.emit("TOKEN_REFRESHED", {
          ...fixture.session,
          access_token: "calendar-navigation-renewal",
        });
        fixture.holdSchedule = false;
        fixture.scheduleWaiters[0]();
      }, navigationDate);
      await page.waitForFunction(() => fixture.attendanceWaiters.length === 1);
      const pendingWindowCount = await page.evaluate(
        () => fixture.requests.filter((r) => r.path.startsWith("/schedule/window")).length,
      );
      await page.evaluate(
        () => new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve))),
      ...
      assert.equal(
        await page.evaluate(() => fixture.controller.hasLoadedRange),
        false,
        "an unsettled attendance write cannot open the range",
      );
      assert.equal(
        await page.evaluate(
          () => fixture.requests.filter((r) => r.path.startsWith("/schedule/window")).length,
      ...
      await page.evaluate(async () => {
        fixture.holdAttendance = false;
        fixture.attendanceWaiters[0]();
        await fixture.renewalAttendance;
      });
      await page.waitForFunction(
        () => fixture.controller.hasLoadedRange && !fixture.controller.isRefreshingRange,
        undefined,
      ...
      assert.deepEqual(
        ...
        [1, `Class ${navigationDate} calendar-navigation-renewal`, null],
      );
      assert.equal(
        await page.evaluate(
          () => fixture.requests.filter((r) => r.path === "/schedule/attendance").length,
        ),
        attendanceWritesBefore + 1,
        "attendance writes are never replayed after token renewal",
      );
      ...
      await page.waitForFunction(() => fixture.scheduleWaiters.length === 1);
      const abandonedDate = await page.evaluate(() =>
        new URL(fixture.requests.at(-1).path, "http://fixture.local").searchParams.get(
          "start_date",
        ),
      );
      await page.evaluate(() => {
        fixture.emit("TOKEN_REFRESHED", {
          ...fixture.session,
          access_token: "calendar-new-range-renewal",
        });
        fixture.controller.onNavigate(1);
      });
      await page.evaluate(
      ...
      assert.equal(
        await page.evaluate(
          (date) => fixture.store.sessions.some((row) => row.date === date),
          abandonedDate,
        ),
        false,
        "the abandoned transport snapshot cannot replace the latest selected range",
      );
      assert.equal(await page.evaluate(() => fixture.controller.scheduleLoadError), null);
      ...
      await page.evaluate(() => {
        ...
        fixture.api.get = async (path, token) => {
          if (path.startsWith("/schedule/window/materialize") && fixture.renewalAttempts < 3) {
            ...
            fixture.emit("TOKEN_REFRESHED", {
              ...fixture.session,
              access_token: `bounded-renewal-${fixture.renewalAttempts}`,
            });
          }
        ...
        fixture.store.refreshScheduleRange("2028-01-01", "2028-01-07", "materialize").then(
          ...
          (error) => {
            fixture.boundedRangeResult = error.message;
          },
        );
      });
      ...
      await page.evaluate(() => {
        fixture.api.get = fixture.originalScheduleGet;
      });
      await page.evaluate(() => {
        fixture.scheduleWaiters = [];
        fixture.holdSchedule = true;
        fixture.store.refreshScheduleRange("2029-01-01", "2029-01-07", "materialize").then(
          ...
          (error) => {
            fixture.oldIdentityRangeResult = error.message;
          },
        );
      });
      await page.waitForFunction(() => fixture.scheduleWaiters.length === 1);
      await page.evaluate(() => {
        fixture.emit("TOKEN_REFRESHED", {
          ...fixture.session,
          access_token: "calendar-before-identity-change",
        });
      ...
      await page.waitForFunction(
        () =>
          fixture.store.staffProfilesAvailable &&
          fixture.store.currentStudioId === "studio-calendar-replacement",
      );
      await page.evaluate(() => {
        fixture.holdSchedule = false;
        fixture.scheduleWaiters[0]();
      });
      await page.waitForFunction(() => fixture.oldIdentityRangeResult, undefined, {
        timeout: 3000,
      });
      assert.match(await page.evaluate(() => fixture.oldIdentityRangeResult), /superseded/);
      assert.equal(
        await page.evaluate(() => fixture.store.sessions.some((row) => row.date === "2029-01-01")),
        false,
        "token renewal never authorizes a caller to survive a studio identity reset",
      );
      // Generic outages never fan out into the legacy loading architecture.
      for (const failure of [
        { status: 503, message: "Unavailable" },
        { status: 404, message: "Business record missing" },
        { message: "Request timed out" },
      ]) {
        const before = await page.evaluate(() => fixture.requests.length);
        await page.evaluate((failure) => {
          fixture.bootstrapError = failure;
          fixture.store.retryInitialization();
        }, failure);
        await page.waitForFunction(() => Boolean(fixture.store.identityLoadError));
        assert.deepEqual(
          await page.evaluate(
            (before) => fixture.requests.slice(before).map((r) => r.path),
            before,
          ),
          ["/dashboard/bootstrap?allow_partial=true"],
        );
        await page.evaluate(() => {
          fixture.bootstrapError = null;
          fixture.store.retryInitialization();
        });
        await page.waitForFunction(
          () => fixture.store.staffProfilesAvailable && !fixture.store.identityLoadError,
        );
        await page.evaluate(
          () =>
            new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve))),
...
for (const mode of ["production", "development"]) {
  test(`preview hydration opens the real Dashboard layout identity gate (${mode})`, async () => {
    ...
    try {
      ...
      await page.waitForFunction(() => fixture.store?.staffProfilesAvailable, undefined, {
        timeout: 3000,
      });
      assert.deepEqual(errors, []);
      assert.deepEqual(
          ...
          get: async (path) => {
            if (path === "/dashboard/bootstrap?allow_partial=true") {
              await new Promise((resolve) => {
                fixture.releaseBootstrap = resolve;
              });
              return {
                auth: {
                  user,
                  membership_status: "active",
                  role: "admin",
                  studio_id: "legacy-fixture-studio",
...
for (const role of ["admin", "front_desk"]) {
  test(`mounted fresh Leads completes only its accessible datasets (${role})`, async () => {
    ...
    try {
          ...
          get: async (path) => {
            ...
            if (path === "/dashboard/bootstrap?allow_partial=true")
              return {
                auth: {
                  membership_status: "active",
                  studio_id: "leads-studio",
                  role,
                  staff_profiles_available: true,
                  user,
                },
                studio_name: "Leads studio",
                students: [],
                programs: [],
                leads: [],
                belt_ladders: [],
      ...
      if (role === "admin") {
        await page.waitForFunction(() => fixture.staffWaiters.length === 1, null, {
          timeout: 3000,
        });
        assert.equal(
          await page.evaluate(() => fixture.readiness.at(-1).complete),
          false,
          "staff cannot block useful lead content, but complete waits for it",
        );
        ...
        assert.equal(
          await page.evaluate(() => fixture.readiness.at(-1).complete),
          false,
          "staff failure is not complete data",
        );
        await page.evaluate(() => {
          fixture.failStaff = false;
        });
      ...
      assert.equal(
        await page.evaluate(
          () => fixture.requests.filter((path) => path.startsWith("/staff")).length,
        ),
        role === "admin" ? 2 : 0,
      );
    } finally {
  ...
  test(`mounted route-owned staff, program and summary reads recover after token rotation (${outcome})`, async () => {
    ...
    try {
      ...
      await page.evaluate((outcome) => {
        ...
        fixture.emit = (event, session) => {
          fixture.session = session;
          for (const listener of fixture.listeners) listener(event, session);
        };
        fixture.supabase = {
          auth: {
            ...
            onAuthStateChange: (callback) => {
              ...
              return {
                data: { subscription: { unsubscribe: () => fixture.listeners.delete(callback) } },
              };
            },
          },
        };
      ...
      await page.evaluate(() => {
        void fixture.store.refreshDashboardSummary().catch(() => {});
      });
      ...
      await page.waitForFunction(
        () =>
          ...
          fixture.store.programsUsageLoaded &&
          fixture.store.dashboardSummaryLoaded &&
          fixture.readiness.at(-1)?.complete,
      );
      assert.equal(
        await page.evaluate(() => fixture.store.identityGeneration),
        generation,
        "token rotation must preserve identity and shell",
      );
      assert.deepEqual(
        await page.evaluate(() => [fixture.store.staffMembers, fixture.store.programs]),
        [[], []],
        "obsolete results never commit",
      );
      assert.deepEqual(
        await page.evaluate(() => [
          fixture.store.staffLoadError,
          fixture.store.programsUsageLoadError,
        ]),
        [null, null],
      );
      assert.equal(
      ...
      assert.deepEqual(
        await page.evaluate(() =>
          fixture.requests.filter((r) => r.path.startsWith("/staff")).map((r) => r.token),
        ),
        ["route-old-token", "route-new-token"],
      );
      assert.deepEqual(
        await page.evaluate(() =>
          fixture.requests.filter((r) => r.path.startsWith("/programs")).map((r) => r.token),
        ),
        ["route-old-token", "route-new-token"],
      );
      ...
      assert.equal(
        ...
        1,
        "summary timing includes every auth replay",
      );
      assert.deepEqual(
        await page.evaluate(() =>
          fixture.requests
  ...
  test(`mounted Dashboard redirects after null ${event} and discards pending identity reads`, async () => {
    ...
    try {
      ...
      await page.evaluate((event) => {
        ...
        fixture.emit = () => {
          for (const listener of fixture.listeners) listener(event, null);
        };
        fixture.supabase = {
          auth: {
            getSession: async () => {
              ...
              if (event === "INITIAL_SESSION") {
                if (fixture.sessionReads > 1) return { data: { session: null }, error: null };
                await new Promise((resolve) => fixture.waiters.push(resolve));
              }
      ...
      await page.waitForFunction(() => fixture.redirects.includes("/login"), null, {
        timeout: 3000,
      });
      await page.evaluate(() => {
        for (const resolve of fixture.waiters) resolve();
    ...
    try {
      ...
      await page.evaluate((legalNamePresent) => {
        const user = {
          ...
          legal_first_name: legalNamePresent ? "Recovery" : null,
          legal_last_name: legalNamePresent ? "User" : null,
        };
        const session = { access_token: "recovery-token", user };
        const profile = {
          user,
          membership_status: "active",
          studio_id: "recovery-studio",
          role: "admin",
          staff_profiles_available: true,
        };
        const fixture = (window.fixture = {
        ...
        fixture.api = {
          get: async (path) => {
            ...
            if (path === "/platform-billing/status")
              return {
                status: "canceled",
                comped: false,
            ...
            if (path === "/billing/system/status")
              return { mutation_capabilities: { core_subscription: true } };
      ...
      await page.addScriptTag({
        content: bundle("production", { layout: true, subscriptionPage: true }),
      });
      await page.waitForFunction(() => fixture.store?.identityReady);
      await page.evaluate(() => {
        fixture.staffRead = fixture.store.refreshStaff();
        fixture.programRead = fixture.store.refreshPrograms();
      });
      ...
      await page.evaluate(async () => {
        for (const resolve of fixture.waiters) resolve();
        await Promise.all([fixture.staffRead, fixture.programRead]);
      });
      assert.deepEqual(
        await page.evaluate(() => [fixture.store.staffMembers, fixture.store.programs]),
        [[], []],
        ...
        await page.evaluate(
          () =>
            fixture.requests.filter(
              (path) => path.startsWith("/staff") || path.startsWith("/programs"),
            ).length,
        ),
        2,
        "revoked reads cannot replay or commit",
      );
      await page.evaluate(() => fixture.root.unmount());
    } finally {
      await browser.close();
...
test("unknown identity after bootstrap402 renders a retryable error and recovers after profile verification", async () => {
  ...
  try {
    ...
    await page.evaluate(() => {
      const user = {
        id: "unknown-recovery-user",
        email: "unknown@example.test",
        legal_first_name: "Verified",
      ...
      const profile = {
        user,
        membership_status: "active",
        studio_id: "unknown-recovery-studio",
        role: "admin",
        staff_profiles_available: true,
      };
      const fixture = (window.fixture = {
      ...
      fixture.api = {
        get: async (path) => {
          ...
          if (path === "/auth/me") {
            if (fixture.failProfile) throw new Error("Profile unavailable");
            return profile;
          }
          if (path === "/platform-billing/status")
            return {
    ...
    assert.match(
      ...
      /account and studio access could not be verified/,
    );
    assert.equal(await page.evaluate(() => fixture.store.identityReady), false);
    assert.equal(await page.locator('[data-recovery-page="true"]').count(), 0);
    assert.equal(await page.locator('[data-preview-sidebar="ready"]').count(), 0);
    await page.evaluate(() => {
      fixture.failProfile = false;
    });
    ...
    assert.equal(
      await page.evaluate(
        () =>
          fixture.requests.filter((path) => path === "/dashboard/bootstrap?allow_partial=true")
            .length,
      ),
      2,
    );
  ...
  test(`SDK session refresh outage stays retryable despite null INITIAL_SESSION (${sessionFailure})`, async () => {
    ...
    try {
      ...
      await page.evaluate((sessionFailure) => {
        const user = {
          id: "refresh-outage-user",
          email: "refresh@example.test",
          legal_first_name: "Refresh",
          legal_last_name: "User",
        };
        ...
        const profile = {
          user,
          membership_status: "active",
          studio_id: "refresh-outage-studio",
          role: "admin",
          staff_profiles_available: true,
        };
        const fixture = (window.fixture = {
          ...
          auth: {
            getSession: async () => {
              ...
              if (fixture.failSession) {
                const error = Object.assign(new Error("Refresh service unavailable"), {
            ...
            onAuthStateChange: (callback) => {
              ...
              queueMicrotask(() => {
                if (
                  fixture.listeners.has(callback) &&
                  (sessionFailure === "returned" || !fixture.failSession)
                )
                  callback("INITIAL_SESSION", fixture.failSession ? null : session);
              });
              return {
                data: { subscription: { unsubscribe: () => fixture.listeners.delete(callback) } },
              };
      ...
      assert.equal(await page.locator('[data-preview-sidebar="ready"]').count(), 0);
      assert.deepEqual(
        await page.evaluate(() => fixture.redirects ?? []),
        [],
        "a retryable session error is not sign-out",
      );
      assert.deepEqual(
        await page.evaluate(() => fixture.requests),
        [],
        "unverified identity must not fetch protected datasets",
      );
      assert.equal(
        await page.evaluate(() => fixture.sessionReads),
        sessionFailure === "returned" ? 2 : 1,
        "only null INITIAL_SESSION requires one bounded confirmation probe",
      );
      assert.equal(
        await page.evaluate(() => localStorage.getItem("fixture-sdk-session")),
        "retained",
      );
      assert.match(
        await page.evaluate(() => document.cookie),
        /koaryu-active-studio=refresh-outage-studio/,
      );
      await page.evaluate(() => {
        fixture.failSession = false;
      });
      await page.getByRole("button", { name: "Retry workspace" }).click();
      ...
      assert.deepEqual(
        await page.evaluate(() => [
          fixture.store.identityReady,
          fixture.store.currentUserId,
          fixture.store.token,
        ]),
        [false, "", null],
        "an outage on recheck hides the prior identity and invalidates app requests",
      );
      assert.equal(await page.locator('[data-preview-sidebar="ready"]').count(), 0);
...
for (const failedDataset of ["leads", "students", "programs", "belts", "studio"]) {
  test(`partial bootstrap keeps useful routes open when ${failedDataset} fails`, async () => {
    ...
    try {
        ...
        fixture.api = {
          get: async (path) => {
            ...
            if (path === "/dashboard/bootstrap?allow_partial=true")
              return {
                auth: profile,
                studio_name: fixture.failedDataset === "studio" ? null : "Healthy studio",
                ...
                programs:
                  fixture.failedDataset === "programs"
                    ? []
                    : [
                        {
                          id: "healthy-program",
                ...
                belt_ladders: [],
                primary_belt_ladder: null,
                dataset_errors: fixture.failedDataset
                  ? {
                      [fixture.failedDataset]: `${fixture.failedDataset} projection failed. Please retry.`,
                    }
                  : {},
              };
            ...
            if (path.startsWith("/programs?")) {
              ...
              return [
                { id: "healthy-program", name: "Healthy program", sort_order: 0, is_system: false },
              ];
            }
      ...
      page.on("pageerror", (error) => errors.push(error.message));
      await page.addScriptTag({
        content: bundle("production", {
          layout: true,
          dashboardController: true,
          beltPage: failedDataset === "belts" || failedDataset === "programs",
        }),
      });
      ...
      if (failedDataset === "belts") {
        ...
        assert.equal(
          await page.evaluate(() => fixture.dashboard.widgetViewModels.promotions_due.metric),
          undefined,
        );
        assert.equal(
          await page.evaluate(() => fixture.dashboard.widgetViewModels.setup_progress.state),
          "error",
        );
      }
      ...
      if (failedDataset === "programs" || failedDataset === "leads") {
        ...
        assert.equal(
          await page.evaluate(() => fixture.store.programsLoadError),
          failedDataset === "programs" ? "Programs retry unavailable" : null,
        );
        assert.equal(
          await page.evaluate(() => fixture.store.programsUsageLoadError),
          "Programs retry unavailable",
        );
        assert.equal(
          await page.evaluate(() => fixture.store.programsLoaded),
          failedDataset !== "programs",
        ...
        assert.equal(
          await page.getByRole("button", { name: "Retry programs", exact: true }).count(),
          failedDataset === "programs" ? 1 : 0,
          "usage-only failures stay out of the global metadata warning",
        );
        if (failedDataset === "programs") {
          await page.evaluate(() => {
            fixture.programsRetryFails = false;
          });
          await page.getByRole("button", { name: "Retry programs", exact: true }).click();
          await page.waitForFunction(
            () => fixture.store.programsLoaded && !fixture.store.programsLoadError,
          );
          assert.equal(
            await page.getByRole("button", { name: "Retry programs", exact: true }).count(),
            0,
    ...
    for (const mutationFinish of ["before-bootstrap", "after-bootstrap"]) {
      test(`bootstrap preserves ${operation} begun ${mutationStart} and settled ${mutationFinish}`, async () => {
        ...
        try {
              ...
              get: async (path) => {
                ...
                if (path.startsWith("/students"))
                  return {
                    items: [
                      {
                        id: "converted-student",
                        legal_first_name: "Converted",
                        legal_last_name: "Student",
                        status: "active",
                      },
                    ],
            ...
            fixture.mutate = async () => {
              ...
              if (operation === "add") {
                ...
                result = {
                  ...
                  first_name: "Changed",
                  ...(operation === "convert"
                    ? { stage: "enrolled", converted_student_id: "converted-student" }
                    : {}),
                };
                fixture.dbLeads = [result];
              }
              await new Promise((resolve) => {
                fixture.releaseMutation = resolve;
              });
              return result;
            };
              ...
              fixture.mutation =
                operation === "add"
                  ...
                  : operation === "update"
                    ...
                    : operation === "delete"
                      ? fixture.store.deleteLead("existing-lead")
          ...
          await page.waitForFunction(
            () => fixture.bootstrapCalls === 2 && Boolean(fixture.releaseBootstrap),
          );
          ...
          if (mutationFinish === "before-bootstrap") {
            ...
            await page.waitForFunction(
              () => JSON.stringify(fixture.store.leads) === JSON.stringify(fixture.dbLeads),
            );
            await page.evaluate(() => fixture.releaseBootstrap());
          } else {
            await page.evaluate(() => fixture.releaseBootstrap());
            await page.waitForFunction(() => fixture.store.identityReady);
            assert.deepEqual(
              await page.evaluate(() => fixture.store.leads),
              [await page.evaluate(() => fixture.initialLead)],
              "a pending mutation excludes the bootstrap snapshot even if the server already committed it",
            );
            await page.evaluate(async () => {
              fixture.releaseMutation();
              await fixture.mutation;
          ...
          assert.equal(
            await page.evaluate(() => fixture.mutationCalls),
            1,
            "bootstrap cannot cancel or replay successful writes",
          );
          if (operation === "convert") {
            assert.equal(
              await page.evaluate(() => fixture.store.leads[0]?.converted_student_id),
              "converted-student",
            );
            assert.equal(
              await page.evaluate(() =>
                fixture.store.students.some((student) => student.id === "converted-student"),
              ),
              true,
              "student revision protection survives the lead guard",
            );
          }
          if (
            operation === "update" &&
            mutationStart === "before-retry" &&
            mutationFinish === "before-bootstrap"
          ) {
            const message = await page.evaluate(async () => {
              fixture.api.patch = async () => {
                throw new Error("Lead write failed");
              };
              return fixture.store
                .updateLead("existing-lead", { first_name: "Rejected" })
                .catch((error) => error.message);
            });
            assert.equal(
              message,
              "Lead write failed",
              "failed writes must settle their pending scope before the next retry",
            );
          }
          await page.evaluate(() => fixture.store.refreshLeads().catch(() => undefined));
          await page.waitForFunction(
            () =>
              fixture.store.leadsLoadError === "Leads refresh failed" && fixture.store.leadsLoaded,
          );
          await page.evaluate(() => {
            fixture.holdBootstrap = false;
            fixture.store.retryInitialization();
          ...
          await page.waitForFunction(
            () =>
              fixture.bootstrapCalls === 3 &&
              fixture.store.identityReady &&
              fixture.store.leadsLoaded &&
              !fixture.store.leadsLoadError,
          );
          assert.deepEqual(
            await page.evaluate(() => fixture.store.leads),
            await page.evaluate(() => fixture.dbLeads),
            "an uncontested retry still restores the complete dataset",
          );
          await page.evaluate(() => fixture.root.unmount());
        } finally {
          await browser.close();
...
test("real bootstrap transport survives its server budget and bounds a stalled body", async () => {
  ...
  try {
    ...
    await page.evaluate(() => {
      const user = {
        id: "bootstrap-budget-user",
        email: "budget@example.test",
        legal_first_name: "Budget",
        legal_last_name: "User",
      };
      ...
      window.fetch = (url, { signal }) => {
        ...
        if (path.includes("/dashboard/bootstrap"))
          return Promise.resolve(Response.json(fixture.payload));
        if (path.includes("/schedule/window"))
          return Promise.resolve(Response.json({ sessions: [], templates: [], attendance: [] }));
    ...
    assert.deepEqual(
      await page.evaluate(() => [
        fixture.store.identityReady,
        fixture.store.identityLoadError,
        fixture.aborts.length,
      ]),
      [false, null, 0],
      "pending bootstrap headers must outlive the generic 12s cutoff",
    );
    ...
    assert.deepEqual(
      ...
      [false, null, 0],
      "the body remains within budget at 30.5s",
    );
    await page.evaluate(() => fixture.requests[0].sendBody(fixture.payload));
    await page.waitForFunction(
      () => fixture.store.identityReady && fixture.store.scheduleStatus === "ready",
      null,
      { timeout: 3000 },
    );
    assert.equal(await page.locator('[data-preview-sidebar="ready"]').count(), 1);
    ...
    await page.evaluate(() => {
      fixture.leadRead = fixture.store.refreshLeads().catch((error) => {
        fixture.leadError = error.message;
      });
    });
    ...
    assert.equal(
      await page.evaluate(() => fixture.leadError),
    ...
    assert.equal(await page.evaluate(() => fixture.store.identityReady), false);
    assert.equal(
      await page.evaluate(() => fixture.aborts.length),
      2,
      "the stalled bootstrap body aborts at its own 35s deadline",
    );
    assert.equal(await page.locator('[data-preview-sidebar="ready"]').count(), 0);
