...
import { markDashboardFactsChanged, needsFreshDashboardFacts } from "../src/lib/dashboard-freshness.ts";

test("route bootstrap requirements omit unrelated feature collections", () => {
  for (const route of ["schedule", "settings"]) assert.deepEqual([...bootstrapDatasets(initialBootstrapView(`/${route}`))], ["studio", "programs"]);
  for (const route of ["leads", "reports"]) assert.deepEqual([...bootstrapDatasets(initialBootstrapView(`/${route}`))], ["studio", "programs", "leads"]);
  assert.deepEqual([...bootstrapDatasets(initialBootstrapView("/belt-tracker"))], ["studio", "programs", "belts"]);
