...
describe("legal-name decision and normalization model", () => {
  ...
  it("trims surrounding whitespace and collapses internal runs to ASCII spaces", () => {
    ...
    assert.deepEqual(normalizeLegalNameDraft({ firstName: "  Ari  ", lastName: "\tvan\nLane " }), {
      firstName: "Ari",
      lastName: "van Lane",
    });
...
describe("legal-name dashboard blocking contract", () => {
  it("derives the gate from store values at render and suppresses protected dashboard content", () => {
    assert.match(
      layoutSource,
      /shouldBlockForLegalName\(\{[\s\S]*?staffProfilesAvailable,[\s\S]*?firstName: legalFirstName,[\s\S]*?lastName: legalLastName/,
    );
    assert.match(
      layoutSource,
      /isLegalNameBlocked \? \([\s\S]*?<LegalNameBlockingScreen onSignOut=\{handleSignOut\}/,
    );
    assert.match(
      layoutSource,
      /\) : \([\s\S]*?<Sidebar[\s\S]*?<DashboardRouteTransition>\{children\}<\/DashboardRouteTransition>/,
    );
...
describe("legal-name blocking form contract", () => {
  it("uses required labeled inputs, exact store action, pending/error state, and no navigation", () => {
    ...
    assert.match(
      blockingSource,
      /label="Legal first name"[\s\S]*?autoComplete="given-name"[\s\S]*?required/,
    );
    assert.match(
      blockingSource,
      /label="Legal last name"[\s\S]*?autoComplete="family-name"[\s\S]*?required/,
    );
    assert.match(
      blockingSource,
    ...
    assert.match(
      blockingSource,
      /setIsSubmitting\(true\)[\s\S]*?finally \{[\s\S]*?setIsSubmitting\(false\)/,
    );
    assert.match(
      blockingSource,
      /catch \(submitError: unknown\)[\s\S]*?setError\(submitError instanceof Error/,
    );
