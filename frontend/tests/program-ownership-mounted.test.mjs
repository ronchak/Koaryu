  ...
  await page.evaluate(
    ({ holdBootstrap }) => {
      ...
      f.feature = () => ({
        ...
        students: [],
        leads: [],
        programs: structuredClone(f.rows),
        belt_ladders: [],
      ...
      f.pendingWrite = (method, path, body, token) =>
      ...
      f.api = {
        get: async (path, token) => {
          ...
          if (path === "/dashboard/workspace") {
            ...
            if (f.holdWorkspace)
              return new Promise((resolve, reject) =>
                f.workspaces.push({ snapshot, resolve, reject }),
              );
            return snapshot;
          }
          if (path.startsWith("/dashboard/bootstrap")) {
            ...
            if (f.holdBootstrap)
              return new Promise((resolve, reject) =>
                f.features.push({ resolve, reject, snapshot }),
              );
            return snapshot;
          }
          ...
          if (path.startsWith("/students?"))
            return { items: [], has_next: false, page_ordinal: 1, page_size: 200, total: 0 };
...
test("program reads cannot replace a confirmed archive or certify data during a pending write", async () => {
  for (const readDuringWrite of [false, true]) {
    ...
    try {
      ...
      if (readDuringWrite) {
        ...
        await flush(page);
        assert.equal(
          await page.evaluate(() => fixture.store.programsUsageLoaded),
          false,
          "a read overlapping a write cannot certify usage",
        );
      }
      await page.evaluate(() => {
      ...
      await flush(page);
      assert.equal(
        await page.evaluate(() => fixture.store.programs.find((p) => p.id === "p1").archived_at),
      ...
      assert.equal(
        await page.evaluate(() => fixture.store.programs.find((p) => p.id === "p1").archived_at),
        "2026-09-08T00:00:00Z",
        "old usage must not undo the acknowledged archive",
      );
  ...
  try {
    ...
    await page.evaluate(async () => {
      fixture.writes[0].reject(new Error("Archive denied"));
...
test("confirmed program writes survive token renewal and simultaneous acknowledgements retain both rows", async () => {
  ...
  try {
    ...
    assert.equal(
      await page.evaluate(() => fixture.store.programs.find((p) => p.id === "p3")?.name),
      "Created",
    );
    assert.deepEqual(
      await page.evaluate(() => fixture.writes.map((w) => w.token)),
      ["token-a", "token-a"],
    ...
    assert.ok(
      (
        await page.evaluate(() =>
          fixture.requests.filter((r) => r.path.startsWith("/belts/ladders")),
        )
      ).some((r) => r.token === "token-b"),
    );
    await page.evaluate(async () => {
...
test("newest program reads own rows and errors while equivalent current reads still share transport", async () => {
  for (const oldFailure of [false, true]) {
    ...
    try {
      await page.evaluate(() => {
        fixture.old = fixture.store
          .refreshPrograms({ includeArchived: true })
          .catch((error) => error.message);
        fixture.shared = fixture.store
          .refreshPrograms({ includeArchived: true })
          .catch((error) => error.message);
      ...
      await page.waitForFunction(() => fixture.reads.length === 2);
      await page.evaluate(async () => {
        fixture.reads[1].resolve([fixture.program("new", "Newest")]);
...
test("program bootstrap cannot overwrite a later write, usage snapshot, or successful load flags", async () => {
    ...
    try {
        ...
        if (outcome === "transport-error") old.reject(new Error("Old bootstrap failed"));
          ...
          old.resolve(
            outcome === "dataset-error"
              ? { ...old.snapshot, dataset_errors: { programs: "Old program error" } }
              : old.snapshot,
          );
      }, outcome);
      await flush(page);
      assert.deepEqual(await page.evaluate(() => fixture.store.programs.map((p) => p.id).sort()), [
        "p1",
        "p2",
        "p3",
      ]);
      assert.deepEqual(
        await page.evaluate(() => [
          ...
          fixture.store.programsLoadError,
  ...
  try {
    ...
    await page.waitForFunction(
      () => fixture.workspaces.length === 1 && fixture.writes.length === 1,
    );
    await page.evaluate(async () => {
    ...
    assert.equal(
      ...
      true,
      "old workspace failure cannot replace a newer program commit",
    );
    assert.equal(await page.evaluate(() => fixture.store.programsLoadError), null);
    assert.equal(
      await page.evaluate(() => fixture.store.programs.find((p) => p.id === "p3")?.name),
      "Saved while retrying",
    );
    assert.equal(
      await page.evaluate(() => fixture.store.identityReady),
      false,
      "workspace identity failure must still close the access gate",
    );
    assert.match(
      await page.evaluate(() => fixture.store.identityLoadError),
      /Workspace retry failed/,
    );
  } finally {
    await page.close();
  }
...
test("an acknowledged CSV import gets a program read begun after its receipt", async () => {
  ...
  try {
    ...
    assert.equal(
      ...
      2,
      "an earlier GET cannot stand in for post-import reconciliation",
    );
    await page.evaluate(async () => {
      fixture.reads[0].resolve(fixture.reads[0].snapshot);
      fixture.reads[1].resolve(fixture.reads[1].snapshot);
  ...
  for (const replaceIdentity of [false, true]) {
    ...
    try {
      ...
      if (replaceIdentity) {
        ...
        await page.waitForFunction(
          () => fixture.store.currentStudioId === "studio-b" && fixture.store.programsLoaded,
        );
      } else {
        await page.evaluate(() => {
