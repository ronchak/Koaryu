...
} from "../scripts/performance-capture-policy.mjs";
const policy = {
  frontendOrigin: "https://frontend.invalid",
  backendOrigin: "https://backend.invalid",
  supabaseOrigin: "https://auth.invalid",
...
const check = (path, method = "GET", functional = false, origin = policy.backendOrigin) =>
  blockedRequestCategory({ url: origin + path, method }, { ...policy, functional });
...
test("disposable functional mode permits only exact refresh/materialization POST destinations", () => {
  ...
  assert.equal(
    check("/auth/v1/token?grant_type=refresh_token", "POST", true, policy.supabaseOrigin),
    null,
  );
  assert.equal(
    check("/auth/v1/token?grant_type=password", "POST", true, policy.supabaseOrigin),
    "write_methods",
  );
...
test("visible mark sanitation drops unknown routes and identifiers, retaining numeric generations only", () => {
  ...
  assert.deepEqual(sanitizeVisibleMarks([entry], "billing"), [
    {
      stage: "useful",
      route: "billing",
      identity_generation: 2,
  ...
  assert.deepEqual(
    sanitizeVisibleMarks([{ ...entry, detail: { ...entry.detail, route: "private" } }], "billing"),
    [],
  );
});
