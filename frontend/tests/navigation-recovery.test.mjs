...
test("auth provider retry recovers without treating outage as missing credentials", async () => {
  ...
  const result = await requestAuthUser(async () => ++reads === 1
    ? { data: { user: null }, error: { status: 503 } } : { data: { user } }, new AbortController().signal);
  assert.equal(result, user);
  assert.equal(reads, 2);
  await assert.rejects(requestAuthUser(async () => ({ data: { user: null }, error: { status: 503 } }), new AbortController().signal), AuthProviderUnavailable);
});

test("auth rejection is not retried and a stalled SDK call respects cancellation", async () => {
  let reads = 0;
  assert.equal(await requestAuthUser(async () => { reads++; return { data: { user: null }, error: { status: 401 } }; }, new AbortController().signal), null);
...
test("restoration deduplicates requests, refreshes current data, and only notifies for a new build", async () => {
  let now = 0, reads = 0, refreshes = 0, updates = 0, resolve;
  const checker = createResumeCheck({ loaded, now: () => now,
    readVersion: () => { reads++; return new Promise(r => { resolve = r; }); },
    onCurrent: () => refreshes++, onUpdate: () => updates++,
  });
  const first = checker.check();
  assert.equal(first, checker.check());
  resolve(current); await first;
  await checker.check();
  assert.equal(reads, 1); assert.equal(refreshes, 1); assert.equal(updates, 0);
  now = 31000;
  const second = checker.check(); resolve({ ...current, commit_sha: "b".repeat(40) }); await second;
  assert.equal(updates, 1); assert.equal(refreshes, 1);
});

test("offline, malformed, other-environment and late version responses cannot replace the app", async () => {
  for (const response of [null, { ...current, environment: "staging" }, { ...current, commit_sha: "bad" }, new Error("Offline")]) {
    const checker = createResumeCheck({ loaded,
      readVersion: async () => { if (response instanceof Error) throw response; return response; },
      onCurrent: () => assert.fail(), onUpdate: () => assert.fail(),
    });
    await checker.check();
  }
  let resolve;
...
test("reload protection tracks all concurrent commands until each settles", () => {
  const first = beginPendingCommand(), second = beginPendingCommand();
  assert.equal(pendingCommands(), 2); first(); first();
  assert.equal(pendingCommands(), 1); second(); assert.equal(pendingCommands(), 0);
});
