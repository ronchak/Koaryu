...
describe("report export catalog", () => {
  ...
  it("matches the backend live role contract and excludes every deferred billing ID", () => {
    ...
    const backendLive = backendManifest.filter((report) => report.availability === "available");
    ...
    assert.deepEqual(
      [...backendLive.filter((report) => report.min_role === "front_desk")]
    ...
    assert.equal(
      backendManifest
        .filter((report) => report.availability === "deferred_billing")
