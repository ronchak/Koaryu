...
export function bundle(mode, { preview = false, pagedRoster = true, layout = false, leadsPage = false, leadController = false, programsSection = false, subscriptionPage = false, scheduleController = false, scheduleForm = false, dashboardController = false, beltPage = false, realApi = false, detailController = false, rosterController = false, studentForm = false, legacyBootstrapFixture = false } = {}) {
  ...
  return `(()=>{${legacyAdapter}const process={env:{NODE_ENV:${mode === "development" ? '"development"' : '"production"'},NEXT_PUBLIC_STUDENTS_PAGED_ROSTER:${pagedRoster ? '"true"' : '"false"'},NEXT_PUBLIC_PREVIEW_MODE:${preview ? '"true"' : '"false"'}}};const modules=[${modules.join(",")}],cache={};function require(id){if(cache[id])return cache[id].exports;const module=cache[id]={exports:{}};modules[id](module,module.exports,require);return module.exports;}const React=require(${react});const {StoreProvider,useStore}=require(${store});${scheduleObserver}${dashboardObserver}${detailObserver}${rosterObserver}${formObserver}${leadObserver}function Observer(){const store=useStore();window.fixture.store=store;React.useEffect(()=>{window.fixture.observations.push({role:store.currentRole,ready:store.staffProfilesAvailable,user:store.currentUserId,studio:store.currentStudioId});});return React.createElement('output',null,store.staffProfilesAvailable?'ready':'pending');}window.fixture.root=require(${dom}).createRoot(document.getElementById('root'));window.fixture.root.render(React.createElement(${mode === "development" ? "React.StrictMode" : "React.Fragment"},null,React.createElement(${provider},null,React.createElement(Observer),${leads === null ? "null" : `React.createElement(require(${leads}).default)`},${programs === null ? "null" : `React.createElement(require(${programs}).ProgramsSection)`},${subscription === null ? "null" : `React.createElement(require(${subscription}).default)`},${schedule === null ? "null" : "React.createElement(ScheduleMount)"},${dashboard === null ? "null" : "React.createElement(DashboardObserver)"},${leadHook === null ? "null" : "React.createElement(LeadControllerFixture)"},${form === null ? "null" : "React.createElement(StudentFormFixture)"},${roster === null ? "null" : "React.createElement(RosterMount)"},${detail === null ? "null" : "React.createElement(DetailMount)"},${belt === null ? "null" : `React.createElement(require(${belt}).default)`})));})();`;
}
