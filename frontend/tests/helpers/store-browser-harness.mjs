...
export function bundle(mode, { preview = false, layout = false, leadsPage = false, programsSection = false, subscriptionPage = false, scheduleController = false, dashboardController = false, beltPage = false, realApi = false, detailController = false, rosterController = false, studentForm = false, legacyBootstrapFixture = false } = {}) {
  if (!["production", "development"].includes(mode) || typeof preview !== "boolean") throw new Error("Unsupported fixture environment");
  const modules = [];
  const ids = new Map();
  const stubs = {
    "next/navigation": `exports.usePathname=()=>window.fixture.pathname??'/dashboard'; exports.useParams=()=>({id:window.fixture.studentId??'student-1'}); exports.useSearchParams=()=>new URLSearchParams(window.fixture.search??window.location.search); const router={replace(path){(window.fixture.redirects??=[]).push(path)},push(path){(window.fixture.redirects??=[]).push(path)}}; exports.useRouter=()=>router;`,
  ...
  return `(()=>{${legacyAdapter}const process={env:{NODE_ENV:${mode === "development" ? '"development"' : '"production"'},NEXT_PUBLIC_PREVIEW_MODE:${preview ? '"true"' : '"false"'}}};const modules=[${modules.join(",")}],cache={};function require(id){if(cache[id])return cache[id].exports;const module=cache[id]={exports:{}};modules[id](module,module.exports,require);return module.exports;}const React=require(${react});const {StoreProvider,useStore}=require(${store});${scheduleObserver}${dashboardObserver}${detailObserver}${rosterObserver}${formObserver}function Observer(){const store=useStore();window.fixture.store=store;React.useEffect(()=>{window.fixture.observations.push({role:store.currentRole,ready:store.staffProfilesAvailable,user:store.currentUserId,studio:store.currentStudioId});});return React.createElement('output',null,store.staffProfilesAvailable?'ready':'pending');}window.fixture.root=require(${dom}).createRoot(document.getElementById('root'));window.fixture.root.render(React.createElement(${mode === "development" ? "React.StrictMode" : "React.Fragment"},null,React.createElement(${provider},null,React.createElement(Observer),${leads === null ? "null" : `React.createElement(require(${leads}).default)`},${programs === null ? "null" : `React.createElement(require(${programs}).ProgramsSection)`},${subscription === null ? "null" : `React.createElement(require(${subscription}).default)`},${schedule === null ? "null" : "React.createElement(ScheduleMount)"},${dashboard === null ? "null" : "React.createElement(DashboardObserver)"},${form === null ? "null" : "React.createElement(StudentFormFixture)"},${roster === null ? "null" : "React.createElement(RosterMount)"},${detail === null ? "null" : "React.createElement(DetailMount)"},${belt === null ? "null" : `React.createElement(require(${belt}).default)`})));})();`;
}
