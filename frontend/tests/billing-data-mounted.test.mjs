...
function bundle({ realApi = false, refunds = false, pageController = false } = {}) {
  ...
  if (refunds || pageController)
    stubs["lucide-react"] = `module.exports=new Proxy({},{get:()=>()=>null});`;
  if (pageController) {
    stubs["next/link"] =
  ...
  const refundObserver = refunds
    ? `const refunds=require(${refund}).useBillingRefundController({...window.fixture.refundOptions,refreshBilling:state.refreshBilling,refreshPaymentAfterRefund:state.refreshPaymentAfterRefund});window.fixture.refunds=refunds;`
    : "";
  const rendered = refunds
...
async function mountBillingFixture(browser) {
  ...
  await page.evaluate(() => {
    ...
    f.options = {
      activeTab: "overview",
      identityKey: "user:studio:admin:1",
      canManageKoaryuSubscription: true,
      canViewStudioBilling: true,
      isPreviewMode: false,
      shouldSettleEarly: false,
      token: "token-a",
    ...
    f.api = {
      get: async (path, token) => {
        ...
        if (path === f.subscriptionRequired)
          throw Object.assign(new Error("Subscription required"), { status: 402 });
        if (path === "/billing/landing")
          return {
            studio_id: identity,
            system_status: { payment_account: { studio_id: identity }, workflow_capabilities: [] },
            platform_status: null,
            financial_access:
              f.financialAccess ?? (f.denied ? "subscription_required" : "available"),
            aggregates: f.denied
              ? null
              : { active_student_count: 8, payment_cohort: { payment_count: 1001 } },
            errors: f.warnings ?? [],
          };
        if (path.includes("/page")) {
          ...
          return {
            items: [{ id: identity }],
            next_cursor: more && !path.includes("?") ? "older" : null,
            complete: !more || path.includes("?"),
          };
        }
...
async function mountRefundFixture(browser) {
  ...
  await page.evaluate(() => {
    ...
    f.options = {
      activeTab: "reports",
      identityKey: "admin:studio:admin:1",
      identity: { userId: "admin", studioId: "studio" },
      canManageKoaryuSubscription: true,
      canViewStudioBilling: true,
      isPreviewMode: false,
      shouldSettleEarly: false,
    ...
    f.refundOptions = {
      enabledWorkflowIds: new Set(["payment.refund"]),
      identity: { userId: "admin", studioId: "studio" },
      identityKey: f.options.identityKey,
      isPreviewMode: false,
      role: "admin",
      token: "token-a",
      setError: f.options.setError,
      setMessage: f.options.setMessage,
    };
    f.payment = {
      id: "payment-1",
      studio_id: "studio",
      payer_id: "payer-1",
      stripe_charge_id: "charge-1",
      stripe_account_id: "account-1",
      status: "succeeded",
      amount_cents: 5000,
      ...
      payment_method_type: "Test card",
    };
    f.api = {
      get: async (path, token) => {
        f.requests.push({ path, token });
        const snapshot =
          path === "/billing/landing"
            ? {
                financial_access: "available",
                errors: [],
                system_status: {
                  workflow_capabilities: [{ workflow_id: "payment.refund", enabled: true }],
                },
                aggregates: { payment_cohort: { payment_count: 1 } },
              }
            : path === "/billing/payers"
              ? []
              : path === "/billing/plans"
                ...
                : path === "/billing/payments/current-month-cohort"
                  ? { payment_count: 1, net_amount_cents: f.payment.net_collected_amount_cents }
                  : path === "/billing/payments/page"
                    ? {
                        items: structuredClone(
                          (f.pageRows ?? [f.payment]).map((row) =>
                            row.id === f.payment.id ? f.payment : row,
                          ),
                        ),
                        next_cursor: f.more ? "older" : null,
                        complete: !f.more,
                      }
                    : path === "/billing/payments/page?cursor=older"
                      ? {
                          items: structuredClone(
                            (f.historyRows ?? []).map((row) =>
                              row.id === f.payment.id ? f.payment : row,
                            ),
                          ),
                          next_cursor: null,
                      ...
                      : path === `/billing/payments/${f.payment.id}`
                        ? structuredClone(f.targetOverride ?? f.payment)
                        : null;
        if (path === f.heldGet) await new Promise((resolve) => f.waiters.push({ path, resolve }));
        if (
          path === `/billing/payments/${f.payment.id}` &&
          f.targetStatus &&
          (!f.deniedToken || f.deniedToken === token)
        )
          throw new f.ApiError("Target access denied", f.targetStatus);
        if (path.startsWith("/billing/payments/") && f.failPayments)
          throw new Error("Payment read failed");
        if (snapshot) return snapshot;
        throw new Error(`Unexpected read ${path}`);
      },
      post: async (path, body, token, options) => {
        f.posts.push({ path, body, token, options });
        if (f.holdBeforeCommit)
          await new Promise((resolve) => f.waiters.push({ path: "refund-commit", resolve }));
        if (!f.refundResponse) {
          f.payment = {
            ...f.payment,
            refunded_amount_cents: body.amount_cents,
            net_collected_amount_cents: 5000 - body.amount_cents,
            refundable_amount_cents: 5000 - body.amount_cents,
          };
          f.refundResponse = {
            id: "refund-1",
            studio_id: f.payment.studio_id,
    ...
    f.receipts = () =>
      Object.entries(localStorage).filter(([key]) =>
        key.startsWith("koaryu.billing.payment-refund.v1"),
      );
    window.confirm = (message) => {
      f.confirmations = (f.confirmations ?? []).concat(message);
      return !f.cancelConfirm;
    };
  });
  await page.addScriptTag({ content: bundle({ refunds: true }) });
  await page.waitForFunction(
    () => fixture.state?.payments.length === 1 && fixture.refunds?.refundActionReady,
  );
  return page;
}
...
async function mountExternalPaymentFixture(
  ...
  await page.evaluate(
    ({ role, preview, workflow, storageFault }) => {
      ...
      f.saved = () =>
        Object.entries(localStorage).filter(([key]) => key.startsWith("koaryu.external-payment:"));
      f.storageFault = (mode) => {
        ...
        if (mode === "get")
          proto.getItem = function (key) {
            if (key.startsWith("koaryu.external-payment:")) {
              f.externalStorageReads++;
              throw new Error("Storage denied");
            }
            return f.storageOriginals.getItem.call(this, key);
          };
        if (["set", "silent", "corrupt-write"].includes(mode))
          proto.setItem = function (key, value) {
            if (!key.startsWith("koaryu.external-payment:"))
              return f.storageOriginals.setItem.call(this, key, value);
            if (mode === "set") throw new Error("Storage full");
            if (mode === "corrupt-write")
              return f.storageOriginals.setItem.call(this, key, "broken");
          };
        if (["remove", "silent-remove"].includes(mode))
          proto.removeItem = function (key) {
            if (!key.startsWith("koaryu.external-payment:"))
              return f.storageOriginals.removeItem.call(this, key);
            if (mode === "remove") throw new Error("Storage denied");
          };
      };
      if (storageFault === "missing")
      ...
      else if (storageFault === "corrupt")
        localStorage.setItem("koaryu.external-payment:admin:studio", "broken");
      ...
      f.api = {
        get: async (path, token) => {
          ...
          if (path === "/billing/landing")
            return {
              studio_id: f.pageOptions.studioStore.currentStudioId,
              financial_access: "available",
              errors: [],
              system_status: {
                workflow_capabilities: [
                  { workflow_id: "payment.external.record", enabled: f.workflow },
                ],
              },
              aggregates: { payment_cohort: { payment_count: Object.keys(f.receipts).length } },
            };
          ...
          if (path === "/billing/payments/current-month-cohort")
            return {
              payment_count: Object.keys(f.receipts).length,
              net_amount_cents: 7525,
        ...
        post: async (path, body, token, options) => {
          ...
          f.receipts[requestKey] ??= {
            id: `payment-${Object.keys(f.receipts).length + 1}`,
            studio_id: identity.currentStudioId,
            ...structuredClone(body),
            status: "externally_recorded",
            payment_method_type: "external",
            gross_paid_amount_cents: 7525,
            net_collected_amount_cents: 7525,
            refunded_amount_cents: 0,
            disputed_amount_cents: 0,
            refundable_amount_cents: 0,
            created_at: "2026-09-09T00:00:00Z",
          };
          const result = Object.hasOwn(f, "responseOverride")
            ? f.responseOverride
            : structuredClone(f.receipts[requestKey]);
...
test("external payment keeps the exact durable request through lost response, edits, remount and retry", async () => {
  ...
  try {
    ...
    assert.deepEqual(JSON.parse(first.saved[0][1]), {
      version: 1,
    ...
    for (const label of ["Payer", "Amount", "Method", "Note"])
      assert.equal(await page.getByLabel(label, { exact: true }).isDisabled(), true);
    await page.evaluate(() => {
      fixture.actions.onExternalPayerChange("payer-2");
      fixture.actions.onExternalAmountChange("99");
      fixture.actions.onExternalMethodChange("Cash");
      fixture.actions.onExternalNoteChange("replacement");
    });
    await settleExternalPage(page);
    assert.deepEqual(
      await page.evaluate(() => [
        fixture.actions.externalPayerId,
        fixture.actions.externalAmount,
        fixture.actions.externalMethod,
        fixture.actions.externalNote,
      ]),
      ["payer-1", "75.25", "Check", "paid at front desk"],
    );
    ...
    await page.evaluate(() => {
      fixture.loseResponse = false;
    });
    await page.getByRole("button", { name: "Retry payment", exact: true }).click();
    await page.waitForFunction(
      () => fixture.page.message === "External payment recorded." && !fixture.page.isLoading,
    );
    assert.deepEqual(await page.evaluate(() => fixture.posts.map((p) => [p.requestKey, p.body])), [
      [first.requestKey, first.body],
      [first.requestKey, first.body],
    ]);
    ...
    assert.equal(
      await page.getByText("External payment recorded.", { exact: true }).count(),
      0,
      "a new invalid draft cannot retain the previous payment's success notice",
    );
    assert.deepEqual(
      await page.evaluate(() => [fixture.posts.length, fixture.saved().length]),
      [2, 0],
    );
  } finally {
    await browser.close();
...
test("external payment rejects invalid drafts before capture and blocks unsafe storage", async () => {
  ...
  try {
    ...
    for (const [field, value] of [
      ...
      assert.deepEqual(
        await invalid.evaluate(() => [fixture.posts.length, fixture.saved().length]),
        [0, 0],
        `${field} must fail before durable capture`,
      );
    }
    await fillExternalPayment(invalid);
    ...
    for (const fault of ["missing", "get", "corrupt", "set", "silent", "corrupt-write"]) {
      ...
      assert.equal(
        await page.getByRole("button", { name: "Record", exact: true }).isDisabled(),
        true,
        fault,
      );
      ...
      assert.match(
        ...
        /storage is unavailable|saved payment request cannot be verified/i,
      );
      await page.close();
    }
  } finally {
  ...
  try {
    for (const response of [
      ...
      await page.evaluate((response) => {
        fixture.responseOverride =
          typeof response === "string"
            ? {
                id: "payment-1",
                studio_id: response === "wrong-studio" ? "other" : "studio",
                payer_id: "payer-1",
                amount_cents: response === "wrong-amount" ? 1 : 7525,
                currency: "usd",
                external_method: "Check",
                note: response === "wrong-note" ? "different" : "paid at front desk",
                status: "externally_recorded",
                payment_method_type: "external",
              }
            : response;
      ...
      await page.evaluate(() => {
        delete fixture.responseOverride;
      });
      await page.getByRole("button", { name: "Retry payment", exact: true }).click();
      await page.waitForFunction(
        () => fixture.saved().length === 0 && !fixture.actions.isActionLoading,
      );
      assert.equal(await page.evaluate(() => Object.keys(fixture.receipts).length), 1);
      assert.equal(
        await page.evaluate(() => fixture.posts[0].requestKey === fixture.posts[1].requestKey),
        true,
      );
      await page.close();
    }
...
test("external payment confirmation survives real loader failure and page Refresh only reads", async () => {
  ...
  try {
    ...
    await page.getByRole("button", { name: "Record", exact: true }).click();
    await page.waitForFunction(
      () => fixture.page.error.includes("Payment read failed") && !fixture.page.isLoading,
    );
    assert.equal(await page.evaluate(() => fixture.page.message), "External payment recorded.");
    assert.equal(await page.evaluate(() => fixture.saved().length), 0);
    ...
    await page.waitForFunction(
      () => !fixture.page.isLoading && fixture.page.tabContentProps.billingPayments.length === 1,
    );
  ...
  try {
    for (const change of ["token", "user", "studio", "role", "generation", "signout", "unmount"]) {
      ...
      await page.evaluate((change) => {
        ...
        if (change === "signout") {
          config.token = null;
          studioStore.currentUserId = null;
          studioStore.currentRole = null;
          studioStore.identityGeneration++;
        }
        f.pageOptions = { ...f.pageOptions, config, studioStore };
        f.render();
      }, change);
      await settleExternalPage(page);
      if (!["token", "unmount"].includes(change)) {
        assert.equal(
          await page.evaluate(() => fixture.actions.isActionLoading),
          false,
          `${change} releases the old claim`,
        );
        assert.equal(
          await page.evaluate(() => fixture.actions.claimAction("record-external")),
          true,
        );
        await settleExternalPage(page);
      }
      ...
      if (change === "token") {
        await page.waitForFunction(
          () => !fixture.actions.isActionLoading && !fixture.page.isLoading,
        );
        assert.equal(await page.evaluate(() => fixture.saved().length), 0);
        assert.equal(await page.evaluate(() => fixture.page.message), "External payment recorded.");
        assert.ok(
          await page.evaluate(
            (n) => fixture.requests.slice(n).every((r) => r.token === "renewed"),
            before,
          ),
        );
        ...
        assert.deepEqual(
          await page.evaluate(() => fixture.saved()),
          saved,
          `${change} cannot retire the original scope's attempt`,
        );
        if (change !== "unmount") {
          assert.equal(await page.evaluate(() => fixture.page.message), "");
          assert.equal(await page.evaluate(() => fixture.page.error), "");
          assert.equal(
            await page.evaluate(() => fixture.actions.isLoadingAction("record-external")),
            true,
            `${change} stale release cannot clear a new claim of the same action`,
          );
        }
      }
      assert.equal(await page.evaluate(() => fixture.posts.length), 1);
...
test("external payment page enforces front desk, preview, denied-role and exact-workflow gates", async () => {
  ...
  try {
    ...
    assert.ok(
      await front.evaluate(() => fixture.requests.some((r) => r.path === "/billing/landing")),
    );
    assert.deepEqual(
      await front.evaluate(() => [...new Set(fixture.requests.map((r) => r.path))].sort()),
      ["/billing/landing", "/billing/payers", "/billing/payments/page"],
    );
    await front.close();

    const preview = await mountExternalPaymentFixture(browser, {
      preview: true,
      storageFault: "get",
    });
    const payer = await preview
      .getByLabel("Payer", { exact: true })
      .locator("option")
      .nth(1)
      .getAttribute("value");
    ...
    await preview.waitForFunction(
      () => fixture.page.message === "Demo external payment recorded locally.",
    );
    assert.deepEqual(
      await preview.evaluate(() => [
        fixture.requests.length,
        fixture.posts.length,
        fixture.externalStorageReads,
      ]),
      [0, 0, 0],
    );
    await preview.close();
...
test("confirmed refund retains its receipt when the real payment loader fails, then recovers with only a read", async () => {
  ...
  try {
    ...
    await page.evaluate(() =>
      fixture.refunds.refundPayment(fixture.state.payments[0], "12.50", "requested_by_customer"),
    );
    assert.equal(await page.evaluate(() => fixture.posts.length), 1);
    assert.match(await page.evaluate(() => fixture.message), /Refund (submitted|accepted)/);
    assert.equal(
      await page.evaluate(() => fixture.receipts().length),
      1,
      "failed payment refresh must retain the original refund request",
    );
    const key = await page.evaluate(() => JSON.parse(fixture.receipts()[0][1]).requestKey);
    assert.equal(
      key,
      await page.evaluate(() => fixture.posts[0].options.headers["Idempotency-Key"]),
    );
    assert.equal(
      await page.evaluate(() => fixture.state.payments[0].refundable_amount_cents),
      5000,
    );
    await page.evaluate(() => {
      fixture.failPayments = false;
    });
    await page.getByRole("button", { name: "Refresh payment", exact: true }).click();
    await page.waitForFunction(() => fixture.receipts().length === 0);
    assert.equal(
      await page.evaluate(() => fixture.posts.length),
      1,
      "confirmed recovery must not submit another refund",
    );
    assert.equal(
      await page.evaluate(() => fixture.state.payments[0].refundable_amount_cents),
      3750,
    ...
    assert.ok(
      await page.evaluate(() =>
        fixture.requests.some((r) => r.path === "/billing/payments/payment-1"),
      ),
    );
...
test("refund target refresh preserves older pages and rejects held first-page and history snapshots", async () => {
  ...
  try {
    for (const held of ["/billing/payments/page", "/billing/payments/page?cursor=older"]) {
      ...
      await page.evaluate(() => {
        fixture.pageRows = [
          { ...fixture.payment, id: "newer-payment", payment_method_type: "Newer card" },
        ];
        fixture.historyRows = [fixture.payment];
        fixture.more = true;
        return fixture.state.refreshBilling();
      });
      await page.evaluate(() => fixture.state.loadMoreHistory());
      assert.deepEqual(await page.evaluate(() => fixture.state.payments.map((p) => p.id)), [
        "newer-payment",
        "payment-1",
      ]);
      if (held.includes("?")) {
        await page.evaluate(() => {
          fixture.holdBeforeCommit = true;
          fixture.failAfterPost = false;
          fixture.pendingRefund = fixture.refunds.refundPayment(
            fixture.state.payments[1],
            "12.50",
      ...
      await page.evaluate((held) => {
        ...
        fixture.failAfterPost = false;
        fixture.staleRead = held.includes("?")
          ? fixture.state.loadMoreHistory()
          : fixture.state.refreshBilling();
      }, held);
      await page.waitForFunction((held) => fixture.waiters.some((w) => w.path === held), held);
      if (held.includes("?")) {
        await page.evaluate(() => {
          fixture.waiters.find((w) => w.path === "refund-commit").resolve();
          return fixture.pendingRefund;
        });
      } else {
        await page.evaluate(() =>
          fixture.refunds.refundPayment(
            fixture.state.payments.find((p) => p.id === "payment-1"),
            "12.50",
            "requested_by_customer",
          ),
        );
      ...
      await page.evaluate(() => {
        fixture.heldGet = null;
        fixture.waiters.splice(0).forEach((w) => w.resolve());
        return fixture.staleRead;
      });
      assert.equal(
        await page.evaluate(
          () => fixture.state.payments.find((p) => p.id === "payment-1")?.refundable_amount_cents,
        ),
        held.includes("?") ? undefined : 3750,
      );
      assert.equal(
        await page.evaluate(() => fixture.state.isLoading || fixture.state.isLoadingMore),
        false,
      );
      const before = await page.evaluate(() => fixture.requests.length);
      ...
      assert.ok(
        await page.evaluate(
          (n) => fixture.requests.slice(n).some((r) => r.path === "/billing/payments/page"),
          before,
        ),
        "discarded page must not become retained fresh data",
      );
      assert.equal(await page.evaluate(() => fixture.posts.length), 1);
      await page.close();
    }
...
test("refund completion uses renewed credentials but drops old identity, capability and unmounted settlements", async () => {
  ...
  try {
    ...
    await renewed.evaluate(() => {
      fixture.failAfterPost = false;
      fixture.heldGet = "/billing/payments/payment-1";
      fixture.pendingRefund = fixture.refunds.refundPayment(
        fixture.payment,
        "12.50",
        "requested_by_customer",
    ...
    await renewed.evaluate(() => {
      fixture.options = { ...fixture.options, token: "renewed" };
      fixture.refundOptions = { ...fixture.refundOptions, token: "renewed" };
      fixture.targetStatus = 401;
      fixture.deniedToken = "token-a";
      fixture.render();
    });
    await renewed.evaluate(
    ...
    assert.deepEqual(
      await renewed.evaluate(() =>
        fixture.requests
          .filter((r) => r.path === "/billing/payments/payment-1")
          .map((r) => r.token),
      ),
      ["token-a", "renewed"],
    );
    assert.equal(
      await renewed.evaluate(() => fixture.posts.length),
      1,
      "only the rejected read may replay after token renewal",
    );
    await renewed.close();

    const postRenewal = await mountRefundFixture(browser);
    await postRenewal.evaluate(() => {
      fixture.failAfterPost = false;
      fixture.heldPost = true;
      fixture.pendingRefund = fixture.refunds.refundPayment(
        fixture.payment,
        "12.50",
        "requested_by_customer",
    ...
    await postRenewal.evaluate(() => {
      fixture.waiters[0].resolve();
      return fixture.pendingRefund;
    });
    assert.equal(await postRenewal.evaluate(() => fixture.receipts().length), 0);
    assert.equal(
      await postRenewal.evaluate(
        () => fixture.requests.find((r) => r.path === "/billing/payments/payment-1").token,
      ),
      "renewed",
    );
    ...
    for (const change of ["signout", "other-studio", "role", "capability", "unmount"]) {
      ...
      await page.evaluate((change) => {
        ...
        if (change === "capability")
          fixture.refundOptions = { ...fixture.refundOptions, enabledWorkflowIds: new Set() };
        else {
          const identity =
            change === "signout"
              ? null
              : { userId: "replacement", studioId: change === "other-studio" ? "other" : "studio" };
          const key = identity ? `${identity.userId}:${identity.studioId}:instructor:2` : null;
          fixture.options = {
            ...fixture.options,
            identity,
            identityKey: key,
            token: identity ? "replacement-token" : null,
            canViewStudioBilling: false,
          };
          fixture.refundOptions = {
            ...fixture.refundOptions,
            identity,
            identityKey: key,
            role: "instructor",
      ...
      if (change !== "unmount")
        assert.equal(
          await page.evaluate(() => fixture.message),
          "",
          `${change} must remove the old refund's message`,
        );
      await page.evaluate(() => {
        fixture.heldGet = null;
        fixture.waiters.splice(0).forEach((w) => w.resolve());
      ...
      assert.equal(
        await page.evaluate(() => fixture.receipts().length),
        1,
        `${change} retains original recovery state`,
      );
      if (!["unmount", "capability"].includes(change))
...
test("an old refund's finally cannot release the replacement identity's pending operation", async () => {
  ...
  try {
    ...
    await page.evaluate(() => {
      fixture.failAfterPost = false;
      fixture.heldPost = true;
      fixture.oldRefund = fixture.refunds.refundPayment(
        fixture.payment,
        "12.50",
        "requested_by_customer",
      );
    ...
    await page.evaluate(() => {
      ...
      fixture.options = {
        ...fixture.options,
        identity,
        identityKey: "new-admin:new-studio:admin:2",
        token: "new-token",
      };
      fixture.refundOptions = {
        ...fixture.refundOptions,
        identity,
        identityKey: fixture.options.identityKey,
        token: "new-token",
      };
      fixture.payment = {
        ...fixture.payment,
        ...
        stripe_charge_id: "charge-2",
        refunded_amount_cents: 0,
        net_collected_amount_cents: 5000,
        refundable_amount_cents: 5000,
      };
      fixture.refundResponse = null;
      fixture.render();
    });
    await page.waitForFunction(() => fixture.state.payments[0]?.id === "payment-2");
    await page.evaluate(() => {
      fixture.newRefund = fixture.refunds.refundPayment(
        fixture.state.payments[0],
        "10.00",
        "requested_by_customer",
      );
    });
    ...
    await page.evaluate(() => {
      fixture.waiters[1].resolve();
      return fixture.newRefund;
    });
    assert.equal(await page.evaluate(() => fixture.refunds.activePaymentId), null);
    assert.equal(
      await page.evaluate(() => fixture.receipts().length),
      1,
      "only the unresolved old identity's receipt remains",
    );
...
test("accepted refund recovery survives remount at zero balance without another provider command", async () => {
  ...
  try {
    const page = await mountRefundFixture(browser);
    await page.evaluate(() =>
      fixture.refunds.refundPayment(fixture.payment, "50.00", "requested_by_customer"),
    );
...
test("unverified refund or target results retain the exact receipt, and current access denial clears financial data", async () => {
  ...
  try {
    for (const fault of [
      ...
      assert.equal(
        await page.evaluate(() => JSON.parse(fixture.receipts()[0][1]).requestKey),
        await page.evaluate(() => fixture.posts[0].options.headers["Idempotency-Key"]),
      );
      if ([401, 402, 403].includes(fault))
        assert.deepEqual(await page.evaluate(() => fixture.state.payments), []);
      if (String(fault).startsWith("refund"))
        assert.equal(await page.evaluate(() => fixture.message), "");
      await page.close();
  ...
  try {
    for (const failure of ["marker", "removal"]) {
      ...
      await page.evaluate((failure) => {
        fixture.failAfterPost = false;
        fixture.originalSet = Storage.prototype.setItem;
        fixture.originalRemove = Storage.prototype.removeItem;
        if (failure === "marker")
          Storage.prototype.setItem = function (key, value) {
            if (value.includes("acceptedRefundId")) throw new Error("storage full");
            fixture.originalSet.call(this, key, value);
          };
      ...
      assert.match(
        ...
        failure === "marker" ? /could not be saved/ : /could not be cleared/,
      );
      await page.evaluate(() => {
        Storage.prototype.setItem = fixture.originalSet;
        Storage.prototype.removeItem = fixture.originalRemove;
        fixture.root.unmount();
      });
      await page.addScriptTag({ content: bundle({ refunds: true }) });
      await page.waitForFunction(() => fixture.state.payments[0]?.refundable_amount_cents === 0);
      await page
        .getByRole("button", {
          name: failure === "marker" ? "Retry original refund" : "Refresh payment",
          exact: true,
        })
        .click();
      await page.waitForFunction(() => fixture.receipts().length === 0);
      ...
      assert.ok(
        posts.every(
          (post) =>
            post.options.headers["Idempotency-Key"] === original.requestKey &&
            post.body.amount_cents === 5000,
        ),
      );
      await page.close();
    }
    ...
    const page = await mountRefundFixture(browser);
    await page.evaluate(() => {
      fixture.postStatus = 503;
      return fixture.refunds.refundPayment(fixture.payment, "50.00", "requested_by_customer");
    });
    const key = await page.evaluate(() => JSON.parse(fixture.receipts()[0][1]).requestKey);
    await page.evaluate(() => {
      fixture.postStatus = 401;
      fixture.failPayments = false;
      return fixture.refunds.recoverRefund(fixture.payment);
    });
    assert.equal(
      await page.evaluate(() => JSON.parse(fixture.receipts()[0][1]).requestKey),
      key,
      "a rejection of a replay does not disprove the earlier accepted request",
    );
    await page.evaluate(() => {
      fixture.postStatus = null;
      fixture.failAfterPost = false;
      return fixture.refunds.recoverRefund(fixture.payment);
    });
    assert.equal(await page.evaluate(() => fixture.receipts().length), 0);
    assert.ok(
      await page.evaluate(() =>
        fixture.posts.every(
          (p) =>
            p.options.headers["Idempotency-Key"] ===
            fixture.posts[0].options.headers["Idempotency-Key"],
        ),
      ),
    );
  } finally {
...
test("refund completion preserves a newer receipt and settles through tab changes without stranding other reads", async () => {
  ...
  try {
    ...
    await page.waitForFunction(() => fixture.waiters.length === 1);
    await page.evaluate(() => {
      const [key, raw] = fixture.receipts()[0];
      localStorage.setItem(
        key,
        JSON.stringify({ ...JSON.parse(raw), requestKey: "newer-request" }),
      );
      fixture.options = { ...fixture.options, activeTab: "plans" };
      fixture.heldGet = "/billing/plans";
      fixture.render();
    });
    await page.waitForFunction(() => fixture.waiters.length === 2);
    await page.evaluate(() => {
      fixture.waiters.find((w) => w.path === "/billing/payments/payment-1").resolve();
      return fixture.pendingRefund;
    });
    assert.equal(
      await page.evaluate(() => JSON.parse(fixture.receipts()[0][1]).requestKey),
    ...
    await page.evaluate(() => {
      fixture.heldGet = null;
      fixture.waiters.find((w) => w.path === "/billing/plans").resolve();
    });
    await page.waitForFunction(() => fixture.state.hasBillingLoadSettled);
...
test("known target access loss prevents older landing and unrelated tab reads from restoring financial access", async () => {
  ...
  try {
    for (const held of ["/billing/landing", "/billing/plans"]) {
      ...
      await page.waitForFunction((held) => fixture.waiters.some((w) => w.path === held), held);
      await page.evaluate(() => {
        fixture.targetStatus = 403;
        fixture.waiters.find((w) => w.path.endsWith("/refund")).resolve();
        return fixture.pendingRefund;
      });
      assert.equal(await page.evaluate(() => fixture.state.isLoading), false);
      ...
      await page.evaluate((held) => {
        fixture.heldGet = null;
        fixture.waiters.find((w) => w.path === held).resolve();
        return fixture.staleRead;
      }, held);
      await page.evaluate(
        () => new Promise((r) => requestAnimationFrame(() => requestAnimationFrame(r))),
      ...
      assert.deepEqual(
        await page.evaluate(() => [
          fixture.state.landing,
          fixture.state.billingSystemStatus,
          fixture.state.plans,
          fixture.state.payments,
  ...
  try {
    ...
    for (const gate of ["role", "capability", "preview", "storage"]) {
      await page.evaluate((gate) => {
        fixture.refundOptions = {
          ...fixture.refundOptions,
          role: gate === "role" ? "instructor" : "admin",
          enabledWorkflowIds: new Set(gate === "capability" ? [] : ["payment.refund"]),
          isPreviewMode: gate === "preview",
        };
        if (gate === "storage")
          Object.defineProperty(window, "localStorage", {
            get() {
      ...
      await page.evaluate(() =>
        fixture.refunds.refundPayment(fixture.payment, "12.50", "requested_by_customer"),
      );
    ...
    for (const status of ["failed", "canceled", "pending", "requires_action", "reconciliation"]) {
      ...
      await payment.evaluate((status) => {
        ...
        fixture.refundResponse = {
          ...
          payment_id: "payment-1",
          amount_cents: 1250,
          status: status === "reconciliation" ? "pending" : status,
          reconciliation_required: status === "reconciliation",
        };
        if (["pending", "requires_action"].includes(status))
          fixture.payment.refundable_amount_cents = 3750;
        return fixture.refunds.refundPayment(fixture.payment, "12.50", "requested_by_customer");
      }, status);
      if (status === "reconciliation") {
        ...
        assert.equal(
          await payment
            .getByRole("button", { name: /Issue refund|Retry original refund|Refresh payment/ })
            .count(),
          0,
        );
      } else {
        ...
        assert.match(
          await payment.evaluate(() => fixture.message),
          status === "failed"
            ? /refund failed/
            : status === "canceled"
              ? /refund was canceled/
...
test("Billing landing waits for its server budget and body but still bounds stalled requests", async () => {
  ...
  try {
    ...
    await page.waitForFunction(() => fixture.requests.length === 1 || fixture.error, undefined, {
      timeout: 3000,
    });
    ...
    assert.deepEqual(
      await page.evaluate(() => [
        fixture.state.hasBillingLoadSettled,
        fixture.error,
        fixture.aborts,
      ]),
      [false, "", 0],
      "a valid composed read must survive the generic 12s deadline",
    );
    ...
    assert.deepEqual(
      ...
      [false, "", 0],
      "body transfer has room beyond the server's 30s deadline",
    );
    await page.evaluate(() =>
      fixture.sendBody({
        studio_id: "studio",
        ...
        financial_access: "available",
        errors: [],
        aggregates: { active_student_count: 8 },
      }),
    );
    await page.waitForFunction(() => fixture.state.hasBillingLoadSettled);
    assert.equal(
      await page.evaluate(() => fixture.state.landing.aggregates.active_student_count),
      8,
    );
    assert.equal(await page.evaluate(() => fixture.error), "");
    ...
    assert.equal(
      await page.evaluate(() => fixture.aborts),
      1,
      "a stalled body is canceled when the bounded landing deadline expires",
    );
    ...
    await page.evaluate(() => {
      fixture.sendHeaders(504);
      fixture.sendBody({ detail: "Provider operation timed out." });
    });
    await page.waitForFunction(
      () =>
        fixture.state.hasBillingLoadSettled && fixture.error === "Provider operation timed out.",
    );
    assert.equal(
      await page.evaluate(() => fixture.aborts),
      1,
      "the server's timeout detail arrives before the browser deadline",
    );
  } finally {
    await browser.close();
  }
...
test("mounted Billing landing, tab retention, mutations and identity isolation", async () => {
  ...
  try {
    ...
    await page.waitForFunction(() => fixture.state?.hasBillingLoadSettled);
    assert.deepEqual(await page.evaluate(() => fixture.requests.map((r) => r.path)), [
      "/billing/landing",
    ]);
    await page.evaluate(() => {
    ...
    await page.waitForFunction(() => fixture.waiters.length === 1);
    assert.deepEqual(
      await page.evaluate(() => fixture.commits[0]),
      { tab: "plans", settled: false, loading: true, requestCount: 1 },
      "an unseen tab is loading in its first commit before its effect starts the request",
    );
    await page.evaluate(() => {
      fixture.held = null;
    ...
    await page.waitForFunction(
      () => fixture.state.plans.length === 1 && fixture.state.hasBillingLoadSettled,
    );
    const before = await page.evaluate(() => fixture.requests.length);
    await page.evaluate(() => {
      fixture.options = { ...fixture.options, activeTab: "overview", token: "renewed" };
      fixture.render();
    });
    await page.evaluate(
      () => new Promise((r) => requestAnimationFrame(() => requestAnimationFrame(r))),
    ...
    assert.deepEqual(
      await page.evaluate(() => fixture.commits[0]),
      { tab: "plans", settled: true, loading: false, requestCount: before },
      "a cached tab is ready in its first commit",
    );
    assert.equal(
      await page.evaluate(() => fixture.requests.length),
      before,
      "fresh revisit and token renewal retain data",
    );
    await page.evaluate(() => fixture.state.refreshBilling());
    assert.equal(
      await page.evaluate(
        () => fixture.requests.filter((r) => r.path === "/billing/landing").length,
      ),
      2,
      "mutation refresh invalidates landing",
    );
    await page.evaluate(() => {
    ...
    await page.waitForFunction(
      () => fixture.state.payers.length === 1 && fixture.state.payments.length === 1,
    );
    assert.ok(
      await page.evaluate(() => fixture.requests.some((r) => r.path === "/billing/payments/page")),
      "reports preserve refund/payment history",
    );
    await page.evaluate(() => {
      fixture.more = true;
      return fixture.state.refreshBilling();
    ...
    assert.equal(
      ...
      true,
      "Reports has another payment page",
    );
    ...
    assert.equal(
      await page.evaluate(() => fixture.state.hasMoreHistory),
      false,
      "a retained payment cursor cannot advertise invisible history on Invoices",
    );
    assert.equal(
      await page.evaluate(
        (n) => fixture.requests.slice(n).some((r) => r.path.startsWith("/billing/payments/")),
        beforeInvoices,
      ),
      false,
      "Invoices does not fetch payments it cannot render",
    );
    ...
    assert.equal(
      await page.evaluate(() => fixture.requests.length),
      beforeNoOp,
      "Invoices with no invoice cursor has no history work",
    );
    await page.evaluate(() => {
      fixture.moreInvoices = true;
      return fixture.state.refreshBilling();
    });
    ...
    await page.evaluate(() => fixture.state.loadMoreHistory());
    assert.deepEqual(
      await page.evaluate((n) => fixture.requests.slice(n).map((r) => r.path), beforeInvoicePage),
      ["/billing/invoices/page?cursor=older"],
    );
    ...
    assert.equal(
      await page.evaluate(() => fixture.state.hasMoreHistory),
      true,
      "Reports retains its own payment cursor",
    );
    const beforePaymentPage = await page.evaluate(() => fixture.requests.length);
    await page.evaluate(() => fixture.state.loadMoreHistory());
    assert.deepEqual(
      await page.evaluate((n) => fixture.requests.slice(n).map((r) => r.path), beforePaymentPage),
      ["/billing/payments/page?cursor=older"],
    );
    assert.equal(await page.evaluate(() => fixture.state.hasMoreHistory), false);
    await page.evaluate(() => {
      fixture.more = false;
    ...
    await page.evaluate(() => {
      fixture.fail = "/billing/payers";
      fixture.commits = [];
      fixture.options = { ...fixture.options, activeTab: "families" };
      fixture.render();
    });
    await page.waitForFunction(
      () => fixture.state.hasBillingLoadSettled && fixture.error === "Required dataset failed",
    );
    assert.equal(
      await page.evaluate(() => fixture.commits[0].settled),
      false,
      "a failed tab also begins honestly pending",
    );
    assert.equal(
      await page.evaluate(() => fixture.state.isLoading),
      false,
      "a required failure settles so its error is visible",
    );
    await page.evaluate(() => {
      fixture.fail = null;
      fixture.options = { ...fixture.options, activeTab: "reports" };
    ...
    assert.equal(
      await page.evaluate(() => fixture.commits[0].settled),
      false,
      "a previous failed read is not retained as successful cached data",
    );
    await page.evaluate(() => {
      fixture.fail = null;
      fixture.options = { ...fixture.options, activeTab: "reports" };
    ...
    await page.evaluate(() => {
      fixture.held = "/billing/payments/page?cursor=older";
      void fixture.state.loadMoreHistory();
    });
    await page.waitForFunction(() => fixture.waiters.length === 1 && fixture.state.isLoadingMore);
    await page.evaluate(() => {
      fixture.options = { ...fixture.options, activeTab: "overview" };
      fixture.render();
    });
    ...
    assert.equal(
      await page.evaluate(() => fixture.state.isLoadingMore),
      false,
      "superseded pagination must not leave button disabled",
    );
    await page.evaluate(() => {
      fixture.waiters.splice(0).forEach((resolve) => resolve());
      fixture.held = null;
    ...
    assert.deepEqual(
      await page.evaluate(() => fixture.state.payers),
      [],
      "verified subscription denial clears previously visible financial records",
    );
    assert.deepEqual(await page.evaluate(() => fixture.state.payments), []);
    assert.equal(await page.evaluate(() => fixture.state.hasMoreHistory), false);
    assert.ok(
      await page.evaluate(() => fixture.state.billingSystemStatus),
      "diagnostics survive financial denial",
    );
    await page.evaluate(() => {
    ...
    await page.waitForFunction(() => fixture.waiters.length === 1);
    assert.deepEqual(
      await page.evaluate(() => fixture.state.plans),
      [],
      "new identity cannot expose old plans during its pending read",
    );
    await page.evaluate(() => {
      fixture.options = {
        ...fixture.options,
    ...
    assert.deepEqual(
      await page.evaluate(() => fixture.state.plans),
      [],
      "superseded request cannot repopulate signout",
    );
    await page.evaluate(() => {
      fixture.held = null;
      fixture.denied = true;
      fixture.options = {
        ...fixture.options,
        identityKey: "front:studio:front_desk:3",
        token: "front-token",
        canViewStudioBilling: true,
    ...
    await page.waitForFunction(
      () => fixture.state.landing?.financial_access === "subscription_required",
    );
    ...
    assert.deepEqual(
      await page.evaluate((n) => fixture.requests.slice(n).map((r) => r.path), deniedStart),
      ["/billing/landing"],
      "denied landing cannot launch a financial tab read",
    );
    assert.equal(
      await page.evaluate(() => fixture.redirected),
      undefined,
      "diagnostics remain visible on denied subscription",
    );
  } finally {
    await browser.close();
  }
  ...
  try {
    ...
    const visit = async (tab) => {
      await page.evaluate((tab) => {
        fixture.options = { ...fixture.options, activeTab: tab };
        fixture.render();
      }, tab);
      await page.evaluate(
        () => new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve))),
    ...
    assert.equal(
      await page.evaluate(() => fixture.requests.length),
      beforeOverview,
      "Overview uses its retained successful data",
    );
    assert.equal(
      await page.evaluate(() => fixture.error),
      "",
      "a Families failure cannot leak into cached Overview",
    );

    await page.evaluate(() => {
      fixture.fail = null;
      fixture.warnings = ["Stripe diagnostics are unavailable."];
      return fixture.state.refreshBilling();
    });
    ...
    assert.equal(
      await page.evaluate(() => fixture.error),
      "Stripe diagnostics are unavailable.",
      "retained landing warnings survive a successful tab fetch",
    );
    await page.evaluate(() => {
      fixture.fail = "/billing/payers";
    });
    ...
    assert.equal(
      await page.evaluate(() => fixture.error),
      "Stripe diagnostics are unavailable. Required dataset failed",
    );
    ...
    assert.equal(
      await page.evaluate(() => fixture.error),
      "Stripe diagnostics are unavailable.",
      "cached Overview restores landing warnings without an unrelated tab failure",
    );
    await page.evaluate(() => {
      fixture.fail = null;
    });
    ...
    assert.equal(
      await page.evaluate(() => fixture.error),
      "Stripe diagnostics are unavailable.",
      "successful retry clears only the tab failure",
    );
    ...
    await visit("reports");
    await page.evaluate(() => {
      fixture.fail = "/billing/payments/page?cursor=older";
      return fixture.state.loadMoreHistory();
    });
    assert.equal(
      await page.evaluate(() => fixture.error),
      "Stripe diagnostics are unavailable. Required dataset failed",
    ...
    assert.equal(
      await page.evaluate(() => fixture.error),
      "Stripe diagnostics are unavailable. Required dataset failed",
      "cached history retains its own failed pagination diagnostic",
    );
    await page.evaluate(() => {
      fixture.fail = null;
      return fixture.state.loadMoreHistory();
    ...
    assert.equal(
      await page.evaluate(() => fixture.error),
      "Stripe diagnostics are unavailable.",
      "successful history retry clears its failure",
    );
    ...
    assert.equal(
      await page.evaluate(() => fixture.error),
      "",
      "forced mutation refresh clears resolved landing warnings",
    );
    await visit("overview");
    assert.equal(
      await page.evaluate(() => fixture.error),
      "",
      "invalidated tabs do not restore old warnings",
    );
    await page.evaluate(() => {
      fixture.warnings = ["Old identity warning."];
      return fixture.state.refreshBilling();
    });
    await page.evaluate(() => {
      fixture.warnings = [];
    ...
    assert.equal(
      await page.evaluate(() => fixture.error),
      "",
      "an identity change clears diagnostics before its response arrives",
    );
    await page.evaluate(() => {
      fixture.held = null;
      fixture.waiters.splice(0).forEach((resolve) => resolve());
    ...
    await page.waitForFunction(() => fixture.state.hasBillingLoadSettled);
    await page.evaluate(() => {
      fixture.warnings = ["Current identity warning."];
      return fixture.state.refreshBilling();
    });
    await page.evaluate(() => {
      fixture.subscriptionRequired = "/billing/payers";
    });
    await visit("families");
    ...
    assert.equal(
      await page.evaluate(() => fixture.error),
      "",
      "subscription denial discards retained diagnostics with financial data",
    );
    assert.equal(await page.evaluate(() => fixture.state.landing), null);
    await page.evaluate(() => {
      fixture.options = {
    ...
    assert.equal(
      await page.evaluate(() => fixture.error),
      "",
      "signout cannot retain another access scope diagnostic",
    );
  } finally {
    await browser.close();
  }
...
test("mounted Billing keeps denied landing diagnostics during Connect reads without crossing access scopes", async () => {
  ...
  try {
    for (const [access, warnings, expected] of [
      ...
      [
        "unavailable",
        ["Financial verification is unavailable."],
        "Financial verification is unavailable.",
      ],
      ...
      await page.evaluate(
        ({ access, warnings }) => {
          fixture.financialAccess = access;
      ...
      assert.ok(
        await page.evaluate(() => fixture.state.billingSystemStatus),
        "denied financial data retains account diagnostics",
      );
      await page.evaluate(() => {
        fixture.held = "/billing/connect/status";
        fixture.fail = "/billing/connect/status";
      ...
      assert.equal(
        await page.evaluate(() => fixture.error),
        expected,
        "pending Connect status preserves the financial access explanation",
      );
      await page.evaluate(() => {
        fixture.held = null;
        fixture.waiters.splice(0).forEach((resolve) => resolve());
      ...
      assert.equal(
        await page.evaluate(() => fixture.error),
        `${expected} Required dataset failed`,
        "a failed Connect read includes the financial access explanation",
      );
      await page.evaluate(() => {
        fixture.financialAccess = "available";
        fixture.warnings = [];
        fixture.fail = null;
        fixture.held = "/billing/landing";
        fixture.options = { ...fixture.options, identityKey: "new:studio:admin:2" };
        fixture.render();
      });
      await page.waitForFunction(() => fixture.waiters.length === 1);
      assert.equal(
        await page.evaluate(() => fixture.error),
        "",
        "another access scope cannot inherit financial or Connect errors",
      );
      await page.evaluate(() => {
        fixture.held = null;
        fixture.waiters.splice(0).forEach((resolve) => resolve());
