...
import { ACCOUNT_ARCHIVED_ROUTE, resolveMembershipRoute } from "../src/lib/auth-route-model.ts";
...
describe("membership route model", () => {
  it("keeps archived sessions on the stable archived account route and out of tenant routes", () => {
    assert.equal(
      route({ membershipStatus: "archived", pathname: "/login", isAuthRoute: true }),
      ACCOUNT_ARCHIVED_ROUTE,
    );
    assert.equal(
      route({ membershipStatus: "archived", pathname: "/onboarding", isOnboardingRoute: true }),
      ACCOUNT_ARCHIVED_ROUTE,
    );
    assert.equal(
      route({ membershipStatus: "archived", pathname: "/dashboard" }),
      ACCOUNT_ARCHIVED_ROUTE,
    );
  ...
  it("routes active and no-membership visits to the archived route without a loop", () => {
    assert.equal(
      route({ membershipStatus: "active", pathname: ACCOUNT_ARCHIVED_ROUTE }),
      "/dashboard",
    );
    assert.equal(
      route({ membershipStatus: "none", hasStudio: false, pathname: ACCOUNT_ARCHIVED_ROUTE }),
      "/onboarding",
    );
    assert.equal(
      route({ membershipStatus: "active", pathname: "/login", isAuthRoute: true }),
      "/dashboard",
    );
    assert.equal(
      route({ membershipStatus: "none", hasStudio: false, pathname: "/login", isAuthRoute: true }),
      "/onboarding",
    );
  });
  ...
  it("preserves existing active and no-membership onboarding behavior", () => {
    assert.equal(
      route({ membershipStatus: "active", pathname: "/onboarding", isOnboardingRoute: true }),
      "/dashboard",
    );
    assert.equal(
      route({
    ...
    assert.equal(
      route({ membershipStatus: "none", hasStudio: false, pathname: "/dashboard" }),
      "/onboarding",
    );
